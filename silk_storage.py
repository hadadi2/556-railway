...
def persistence_violation(status: dict, allow_nonmount: bool = False) -> "str | None":
    """القاعدةُ الواحدة لمصيدة القرص الدائم — يقرؤها حارسُ الإقلاع و`/health` (مراجعة R9؛
    كانت مكرّرةً في `api.py` فيمكن أن ينحرف أحدُهما عن الآخر ⇒ إقلاعٌ أخضر ثم 503).
    `unconfigured` لا مخزنَ موجَّه؛ `unwritable` تعذّر المجسّ؛ `nonmount` ليس وحدةً مركَّبة
    (إلا مع `SILK_ALLOW_NONMOUNT_PERSIST`)؛ None = سليم."""
    if not status.get("configured"):
        return "unconfigured"
    if not status.get("writable"):
        return "unwritable"
    if not status.get("is_mount") and not allow_nonmount:
        return "nonmount"
    return None
...
_RESEARCH_STATUSES = frozenset({"running", "completed", "failed"})
_initialized: set[str] = set()


def _now_iso() -> str:
    """طابعٌ زمنيّ واعٍ بـUTC (`+00:00`) — R5 (تدقيق 2026-09-01، DB-12).

    كانت الطوابع محلّيةً ساذجة بينما دلوُ `silk_usage._today()` بـUTC، فتشغيلةٌ
    تُنشأ قرب منتصف الليل تُغادر دلوَها ولا يُصالَح حجزُها الدولاريّ أبداً.
    الصفوفُ القديمة (ساذجة، وقتُ النشر UTC) تبقى قابلةً للمقارنة نصّياً على
    البادئة المشتركة.
    """
...
def _open(path: str):
    """اتصالٌ يُغلَق دائماً — `sqlite3.Connection` كمدير سياق **يلتزم ولا يُغلق**.

    رابعُ موضعٍ من العائلة نفسها (`silk_ops_log`، `silk_usage`، `silk_store`):
    اتصالٌ باقٍ مفتوحاً يحجز الملفَّ على ويندوز، فينهار تنظيفُ المجلّدات
    المؤقّتة بـ`WinError 32` ويتعثّر اختبارٌ يقيس شيئاً آخر تماماً. القفلُ
    البنيويّ على العائلة في `tests/test_sqlite_connection_hygiene.py`.
    """
    conn = _connect(path)
    try:
        with conn:
...
def _connect(path: str) -> sqlite3.Connection:
    ...
    المُوصِّل المشترك (`silk_sqlite.connect`، تدقيق البند ٢) يحمل مهلة القفل
    الصريحة كي لا تُرمى «database is locked» على كاتبٍ متزامن مشروع.
    """
    import silk_sqlite
    return silk_sqlite.connect(path, foreign_keys=True)
...
def init_db(path: str | None = None, *, force: bool = False) -> None:
    ...
    # R5 (DB-11): مرّةٌ واحدة لكل مسارٍ في العملية (كان يُعاد في ١١ موضعاً لكل
    # نداء)؛ `force=True` يُعيد الفحص، وملفٌّ حُذف ثم أُعيد يُبنى من جديد.
    key = os.path.abspath(path)
    if not force and key in _initialized and os.path.exists(path):
        return
    with _open(path) as conn:
        ...
        # الموجة p6 (T1 — تدقيق التنفيذ 2026-08-22): نقاط تفتيش **المراحل**
        # (محلل/حكم/روابط/مسوّدة كاتب جزئية) — شقيق `research_missions`؛
        # الفرع ب الحيّ: ثمن المحلل كان يعيش في الذاكرة حتى الحفظ النهائي.
        # جدول إضافي جديد — لا ALTER على الجداول القائمة، لا صفّ يُمَسّ.
        conn.execute(
            "CREATE TABLE IF NOT EXISTS research_stages ("
            "analysis_id INTEGER NOT NULL, stage TEXT NOT NULL, "
            "status TEXT NOT NULL, payload_json TEXT, market_iso3 TEXT, "
            "completed_at TEXT, PRIMARY KEY (analysis_id, stage))"
        )
        ...
        # جيل المطالبة يمنع إعادة استعمال لقطة قديمة حتى لو عادت الحالة إلى
        # failed في الثانية نفسها. Additive attempt-generation fence.
        if "resume_generation" not in existing:
            conn.execute("ALTER TABLE analyses ADD COLUMN resume_generation "
                         "INTEGER NOT NULL DEFAULT 0")
        # R5 (DB-7): فهارسُ أنماط الاستعلام الفعلية — بعد ALTER كي توجد الأعمدة:
        # قارئُ الأسواق (`market_scores_for_analyses`) وحاصدُ اليتامى
        # (`reap_orphan_research_runs`) كانا مسحَين كاملَين.
        conn.execute("CREATE INDEX IF NOT EXISTS idx_market_scores_analysis "
                     "ON market_scores(analysis_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_analyses_status_updated "
...
def save_analysis(result: dict, path: str | None = None,
    ...
    with _open(path) as conn:
        if analysis_id is not None:
            ...
            # R5 (API-17): إعادةُ الحفظ بنفس المعرّف (اكتمال التشغيلة، إعادةُ
            # التوليد، إثراءُ المستوردين، نثرُ التصدير) كانت تُضاعِف صفوفَ الأسواق.
            conn.execute("DELETE FROM market_scores WHERE analysis_id = ?",
                         (analysis_id,))
        else:
            cur = conn.execute(
...
def create_research_run(product: str, market_iso3: str, hs_code: str | None,
    ...
    with _open(path) as conn:
        ...
        from silk_request_identity import allocated
        allocated(conn, int(cur.lastrowid))
        return int(cur.lastrowid)


def claim_research_resume(analysis_id: int, *, expected: dict,
                          path: str | None = None) -> bool:
    """طالِب بالاستئناف ذرّياً بعد القبول؛ لا كتابة إن تغيّرت لقطة الطالب.

    Status + heartbeat + generation fence across processes, not a Python lock.
    Reset the reconciliation flag in the same transaction as the claim.
    """
    path = path or _db_path()
    init_db(path)
    with _open(path) as conn:
        ...
        row = conn.execute(
            "SELECT status, updated_at, resume_generation, progress_json "
            "FROM analyses WHERE id = ? AND kind = 'research'", (analysis_id,)
        ).fetchone()
        if row is None or any(row[k] != expected.get(k) for k in
                              ("status", "updated_at", "resume_generation")):
            return False
        progress = json.loads(row["progress_json"] or "{}")
        progress["usd_reconciled"] = False
        conn.execute(
            "UPDATE analyses SET status = 'running', updated_at = ?, "
            "resume_generation = resume_generation + 1, progress_json = ? WHERE id = ?",
            (_now_iso(), json.dumps(progress, ensure_ascii=False), analysis_id))
    return True
...
def update_research_status(analysis_id: int, status: str,
    ...
    if status not in _RESEARCH_STATUSES:          # R5 (DB-13): لا CHECK إضافيّ
        raise ValueError(f"unknown research status: {status!r}")
    path = path or _db_path()
    now = _now_iso()
    with _open(path) as conn:
        conn.execute(
...
def update_research_progress(analysis_id: int, path: str | None = None,
    ...
    with _open(path) as conn:
        ...
        for k, v in fields.items():
            ...
            current[k] = v
        # R2b (API-2): كلُّ لقطةِ تقدّم نبضةٌ — الحاصدُ يقرأ `updated_at` وكان
        # لا يتحرّك إلا بتغيّر الحالة، فتشغيلةٌ حيّة طويلة تُحصَد «عالقةً».
        conn.execute(
            "UPDATE analyses SET progress_json = ?, updated_at = ? WHERE id = ?",
            (json.dumps(current, ensure_ascii=False, default=_json_default),
             _now_iso(), analysis_id))
...
def _reconcile_leaked_usd(analysis_id: int, created_at: object,
    ...
    # درس 188 (F15): UTC كـsilk_usage — كان `date.today()` محلياً فقد يختلف
    # حدُّ اليوم عن دلو الدفتر المدفوع فيُخصَم من دلوٍ خطأ.
    today = silk_usage._today()
    if (str(created_at or ""))[:10] != today:
        update_research_progress(analysis_id, path=path, usd_reconciled=True)
        return False
    # درس 188 (F9): المصدر الواحد للتقدير — لا حرفيّة 3.0 مكرّرة تتباعد.
    expected = silk_usage.expected_run_usd()
    try:
        actual = float((progress or {}).get("cost_usd_estimate") or 0.0)
    except (TypeError, ValueError):
        actual = 0.0
    silk_usage.reconcile_usd(reserved=expected, actual=actual)
    # R2b (API-2): ما صُولِح إليه يُحفَظ — فمصالحةُ الخطّ النهائية (إن أكمل بعد
    # حصدٍ مبكّر) تنطلق منه لا من التقدير، فينتهي الدفترُ عند الفعليّ الأخير.
    update_research_progress(analysis_id, path=path, usd_reconciled=True,
                             usd_reconciled_actual=actual)
    return True


def reconcile_run_usd_final(analysis_id: int, *, reserved: float, actual: float,
                            day: str | None = None, path: str | None = None) -> None:
    """المصالحةُ النهائية لتشغيلةٍ أكملت — R2b (API-2): إن سبقها حاصدٌ فصالَح إلى
    `usd_reconciled_actual`، يُصالَح من ذلك المبلغ إلى الفعليّ النهائي (لا من التقدير
    مرّةً ثانية = خصمٌ مزدوج). دلوُ يوم الحجز كما في الدرس ١٨٨."""
    import silk_usage
    progress = get_research_progress(analysis_id, path=path)
    prev = progress.get("usd_reconciled_actual")
    base = reserved
    if progress.get("usd_reconciled") and prev is not None:
        try:
            base = float(prev)
        except (TypeError, ValueError):
            base = reserved
    silk_usage.reconcile_usd(reserved=base, actual=actual, day=day)
    try:
        update_research_progress(analysis_id, path=path, usd_reconciled=True,
                                 usd_reconciled_actual=actual)
    except Exception as exc:  # noqa: BLE001 — R2b (مراجعة): الدفترُ كُتب؛ رفعٌ هنا
        # كان يدفع المُنادي إلى مصالحةٍ ثانية من الحجز الكامل (خصمٌ مزدوج).
        log.warning("usd_reconciled flag not written for %s after the ledger "
                    "write: %s", analysis_id, exc)
...
def mark_research_failed(analysis_id: int, error_message: str,
    ...
    init_db(path)
    now = _now_iso()
    # الموجة p6 (T1): **دمج لا استبدال** — كان الوسم يستبدل json_blob كاملاً
    # فيمحو نتيجةً مدفوعةً محفوظة (محلل/حكم/بعثات) إن فشل الذيل بعدها.
    # الآن: إن كان الصفّ يحمل قاموساً صالحاً تُضاف إليه حقول الفشل فوقه؛
    # NULL/فاسد => القاموس الصغير كما كان (لا اختلاق دمج على نصٍّ لا يُقرأ).
    # مراجعة §58 #13: القراءة والدمج والكتابة في **معاملة واحدة**
    # (BEGIN IMMEDIATE يأخذ قفل الكتابة قبل القراءة). باتصالين منفصلين كان
    # كاتبان متزامنان (الخيط الخلفي + المكنَس/الجسر) يقرآن البلوب نفسه فيمحو
    # اللاحقُ دمجَ السابق — وقد يمحو نتيجةً مدفوعة كُتبت بينهما: عينُ فقدان
    # البيانات الذي وُضِع الدمجُ لمنعه.
    with _open(path) as conn:
        conn.execute("BEGIN IMMEDIATE")
        row = conn.execute("SELECT json_blob FROM analyses WHERE id = ?",
                           (analysis_id,)).fetchone()
        merged: dict = {}
        if row is not None and row["json_blob"]:
            try:
                prior = json.loads(row["json_blob"])
                if isinstance(prior, dict):
                    merged = prior
            except Exception:  # noqa: BLE001 — نصّ فاسد => لا دمج، لا كسر
                merged = {}
        merged["status"] = "failed"
        merged["error"] = str(error_message)[:2000]
        merged["failed_at"] = now
        blob = json.dumps(merged, ensure_ascii=False, default=_json_default)
        conn.execute(
            "UPDATE analyses SET status = 'failed', json_blob = ?, "
            "updated_at = ? WHERE id = ?", (blob, now, analysis_id))


def orphan_stale_minutes(stale_minutes: int | None = None) -> int:
    """نافذةُ التقادم بالدقائق — `SILK_ORPHAN_STALE_MINUTES` (٣٠)؛ القاعدةُ الواحدة
    للحاصد ولبوّابة 409 `resume_still_running` (مراجعة R2b)."""
    if stale_minutes is not None:
        return int(stale_minutes)
    try:
        return int(os.environ.get("SILK_ORPHAN_STALE_MINUTES", "30") or "30")
    except ValueError:
        return 30


def orphan_stale_cutoff(stale_minutes: int | None = None) -> str:
    """طابعُ الحدّ (UTC، ثوانٍ): صفٌّ `updated_at` أقدمُ منه عالق."""
    now = datetime.datetime.now(datetime.timezone.utc)
    return (now - datetime.timedelta(minutes=orphan_stale_minutes(stale_minutes))
...
def reap_orphan_research_runs(stale_minutes: int | None = None,
    ...
    **لماذا.** تعطّل العملية (إعادة نشر Railway، SIGKILL) لا يُشغِّل
    ...
    """
    stale = orphan_stale_minutes(stale_minutes)
    path = path or _db_path()
    if not os.path.exists(path):
        return []
    init_db(path)
    now = datetime.datetime.now(datetime.timezone.utc)   # R5 (DB-12): دلوٌ UTC
    cutoff = orphan_stale_cutoff(stale)
    ...
    # R2b (مراجعة): مقابضُ `silk_research_runtime` الحيّة تُستثنى — مرحلةُ المحلّل
    # قد تطول عن العتبة بلا نبضةٍ بينها، والحيّةُ في هذه العملية ليست يتيمة.
...
def get_research_run(analysis_id: int, path: str | None = None) -> dict | None:
    ...
    with _open(path) as conn:
        row = conn.execute(
            ...
            "kind, request_json, resume_generation FROM analyses WHERE id = ?", (analysis_id,)
        ).fetchone()
    if row is None:
        return None
    d = dict(row)
...
def save_mission_checkpoint(analysis_id: int, mission_key: str, report: object,
    ...
    with _open(path) as conn:
        ...
        # R2b (API-2): نقطةُ تفتيش البعثة نبضةٌ أيضاً (نفسُ الاتصال، نفسُ الالتزام).
...
def save_stage_checkpoint(analysis_id: int, stage: str, payload: object,
                          status: str = "succeeded",
                          market_iso3: str | None = None,
                          path: str | None = None) -> None:
    """خزّن نتيجة **مرحلة** فور عودتها (الموجة p6، T1) — نظير
    `save_mission_checkpoint` للمراحل غير البعثية: `analyst` / `verdict` /
    `leads` / `writer_partial`. الفرع ب الحيّ: نداء المحلل (أغلى نداء في
    التشغيلة) كان يعيش في الذاكرة حتى الحفظ النهائي فيُعاد دفعه عند أيّ فشل.
    `payload` يُسلسَل بنفس `_json_default` (DataPoint/AgentReport → dict).
    `market_iso3` يُختَم كما في البعثات فترفض `load_stage_checkpoints` صفّاً
    لسوقٍ آخر. يحدّث `analyses.updated_at` كي يراه المكنَس تشغيلةً حيّة."""
    path = path or _db_path()
    init_db(path)
    now = _now_iso()
    ...
    with _open(path) as conn:
        conn.execute(
            ...
            "(analysis_id, stage, status, payload_json, market_iso3, "
            "completed_at) VALUES (?, ?, ?, ?, ?, ?) "
            "ON CONFLICT(analysis_id, stage) DO UPDATE SET "
            "status = excluded.status, payload_json = excluded.payload_json, "
            "market_iso3 = excluded.market_iso3, "
            "completed_at = excluded.completed_at",
            (analysis_id, stage, status, blob, market_iso3, now))
        conn.execute("UPDATE analyses SET updated_at = ? WHERE id = ?",
                     (now, analysis_id))


def load_stage_checkpoints(analysis_id: int,
                           market_iso3: str | None = None,
                           path: str | None = None) -> dict:
    """نقاط تفتيش المراحل لتشغيلة — {stage: {"status", "payload",
    "completed_at"}}. قاموس فارغ بلا قاعدة/تشغيلة. نفس حارس السوق:
    صفّ مختوم بسوقٍ آخر يُرفَض بتحذير؛ صفّ بلا ختم يمرّ."""
    path = path or _db_path()
    if not os.path.exists(path):
        return {}
    init_db(path)
    with _open(path) as conn:
        rows = conn.execute(
            "SELECT stage, status, payload_json, market_iso3, completed_at "
            "FROM research_stages WHERE analysis_id = ?", (analysis_id,)
        ).fetchall()
    out: dict = {}
    for r in rows:
        stored_iso3 = r["market_iso3"]
        if market_iso3 and stored_iso3 and stored_iso3 != market_iso3:
            log.warning(
                "stage checkpoint %s/%s belongs to market %s, not requested "
                "%s — rejected (cross-market leak guard)",
                analysis_id, r["stage"], stored_iso3, market_iso3)
            continue
        try:
            payload = json.loads(r["payload_json"]) if r["payload_json"] else None
        except Exception as e:  # noqa: BLE001 — نقطة تفتيش فاسدة تُهمَل لا تكسر
            log.warning("corrupt stage checkpoint %s/%s ignored: %s",
                        analysis_id, r["stage"], e)
            continue
        out[r["stage"]] = {"status": r["status"], "payload": payload,
...
def set_outcome(analysis_id: int, outcome: str,
    ...
    with _open(path) as conn:
        cur = conn.execute(
            ...
            (outcome, datetime.datetime.now(datetime.timezone.utc).date().isoformat(),
             analysis_id),
        )
    return cur.rowcount > 0


def list_analyses(path: str | None = None,
    ...
    `limit` (إضافي، اختياري): يقصر الاستعلام نفسه على أحدث N — للمسارات
    الساخنة (بطاقة تكاليف الأدمِن) بدل تجسيد الجدول كاملاً ثم القصّ."""
    path = path or _db_path()
    if not os.path.exists(path):
        return []
    init_db(path)  # ترحيل آمن للقواعد الأقدم قبل قراءة عمودي outcome
    with _open(path) as conn:
        # درس 191 (§58 الملاحظة 5): `kind` مُسقَط في العرض — بدونه لا يستطيع
        # `silk_consistency.check_against_history` الترشيح بالنوع، فصفُّ
        # /analyze أحدث يحجب /research الأقدم فيعطّل فحص «ترقية حكم على أدلة
        # أسوأ» ويطبع «لا تشغيلة سابقة» كاذباً. إضافةٌ عرضية (كل مستهلك dict).
        sql = ("SELECT id, product, hs_code, year, created_at, preliminary, "
               "outcome, outcome_date, status, market_name, verdict_label, "
               "cost_usd, kind FROM analyses ORDER BY id DESC")
        if limit is not None:
            rows = conn.execute(sql + " LIMIT ?", (max(1, int(limit)),)
                                ).fetchall()
        else:
...
def market_scores_for_analyses(analysis_ids: list[int],
                               path: str | None = None) -> list[dict]:
    """صفوف `market_scores` لمجموعة تحاليل — تستهلكها نظرة عامة المنصّة
    (GET /platform/overview، قرار المالك 2026-08-18): تجميع الأسواق المدروسة
    لحسابٍ من درجات المحرّك المسطّحة بلا تفسير `json_blob` الكامل لكل صفّ.

    قراءة فقط؛ قاعدة غائبة أو قائمة فارغة = [] معلنة، لا اختلاق.
    """
    ids = [int(i) for i in (analysis_ids or [])][:200]
    if not ids:
        return []
    path = path or _db_path()
    if not os.path.exists(path):
    ...
    with _open(path) as conn:
        rows = conn.execute(
            "SELECT analysis_id, country, iso3, total_score, confidence "
            f"FROM market_scores WHERE analysis_id IN ({marks})", ids).fetchall()
