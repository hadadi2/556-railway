...
def _check_services(duration_s: float | None,
                    n_last: int = 60,
                    trace_id: str | None = None) -> tuple[list[dict], list[dict]]:
    """أعطال خدمات خارجية تخصّ هذه التشغيلة — مطابقة يقينية ثم تقريبية.

    منذ 2026-08-17 يختم `silk_ops_log.record_service_failure` الصفَّ بـ
    `trace_id` حين يقع العطل داخل سياق تتبّعٍ نشط، فالمطابقة الأولى **يقينية**:
    صفٌّ موسومٌ يُنسَب لتشغيلته حصراً (تطابُقٌ يُدرِج، واختلافٌ يستبعد ولو وقع
    داخل النافذة الزمنية). الصفوف غير الموسومة (مسارات بلا تتبّع، وصفوف ما قبل
    الوسم) تبقى على النافذة التقريبية القديمة — [الآن − (مدّة التشغيلة +
    هامش)، الآن] — مُفصَّحاً بها كتقريبٍ لا يقين. **مجمَّعٌ بخدمةٍ واحدة** —
    عدّة أعطالٍ لنفس الخدمة ضمن التشغيلة تُبلَّغ بندًا واحدًا لا قائمةً
    مكرَّرة (خدمةٌ واحدة فشلت ٥ مرّات تبقى ملاحظةً واحدة، لا خمس)."""
    try:
        import silk_ops_log
    ...
    for r in rows:
        ...
        # مطابقة يقينية أولاً (وسم 2026-08-17): صفٌّ يحمل `trace_id` يُنسَب
        # لتشغيلته **حصراً** — يُدرَج عند التطابق فقط، ويُستبعَد عن أي تشغيلة
        # أخرى (بما فيها تشغيلة بلا تتبّع أصلاً — كانت الصيغة الأولى تُسقِطه
        # في نافذتها الزمنية فيُنسَب عطل بحثٍ متزامن لتحليلٍ بريء: ملاحظة
        # المراجعة الذاتية §58). غير الموسوم يبقى على النافذة التقريبية.
        row_tid = str(ctx.get("trace_id") or "")
        if row_tid:
            if row_tid != (trace_id or ""):
                continue
        else:
            at = r.get("at") or ""
...
def _observe_unsafe(result: dict, kind: str, analysis_id: int | None) -> dict:
    ...
    _store(record)
    # التنبيه في خيط منفصل — إرسال SMTP المتزامن كان يعلّق ردّ التحليل حتى
    # دقيقة عند بطء المضيف (مهلة smtplib لكل طور — ملاحظة المراجعة §58)؛
    # الحارس «مراقبة لا بوّابة»: لا يُبطئ أي تشغيلة أبداً.
    if os.environ.get("SILK_WATCHDOG_ALERT_EMAIL", "").strip() \
            and record.get("overall") == RED:
        threading.Thread(target=_maybe_alert_red, args=(record,),
                         name="silk-watchdog-alert", daemon=True).start()
    return record


def _maybe_alert_red(record: dict) -> None:
    """تنبيه دافع عند سجلٍّ أحمر — يغلق فجوة «الحارس يرى ولا ينادي».

    كل مراقبة المنصّة كانت «سحباً»: خرق عقدٍ أحمر (تسريب عبر-سوقي، اختلاق،
    بوّابة معطّلة) يبقى صامتاً حتى يفتح المالكُ صفحة الحارس. هذا يرسل سطراً
    بريدياً واحداً عبر ناقل SMTP التشغيلي القائم
    (`silk_platform.smtp_transport.operator_config_from_env`).

    **صمّام مطفأ افتراضاً** (قاعدة الأعلام الجديدة — الدرس ٧٠): يعمل فقط حين
    يضبط المالك `SILK_WATCHDOG_ALERT_EMAIL` (عنوان الاستلام هو الصمّام نفسه)
    وتهيئة SMTP التشغيلية قائمة. قناة جانبية بحتة: لا ترفع أبداً، وعطلها
    يُسجَّل في سجلّ العمليات (عائلة الدرس ٢٦ — لا فشل صامتاً للمشغّل).
    """
    to_email = os.environ.get("SILK_WATCHDOG_ALERT_EMAIL", "").strip()
    if not to_email or record.get("overall") != RED:
        return
    try:
        from silk_platform import smtp_transport
        cfg = smtp_transport.operator_config_from_env()
        if cfg is None:
            return   # لا تهيئة تشغيلية — الصمّام مضبوط لكن الناقل غائب
        reds = [f["check"] for f in record.get("findings") or []
                if f.get("severity") == RED][:6]
        subject = (f"تنبيه حارس سِلك: سجلّ أحمر — تشغيلة "
                   f"#{record.get('analysis_id') or '—'}")
        body = ("رصد حارس المراقبة خرق عقدٍ أحمر.\n"
                f"التشغيلة: #{record.get('analysis_id') or '—'} "
                f"({record.get('kind')})\n"
                f"المنتج/السوق: {record.get('product') or '—'} / "
                f"{record.get('market') or '—'}\n"
                f"الفحوص الحمراء: {', '.join(reds) or '—'}\n\n"
                "التفصيل الكامل في صفحة «تقرير الحارس» على اللوحة "
                "(/watchdog — سطح مالك).")
        msg_id = (f"<watchdog-red-{record.get('analysis_id') or 'na'}-"
                  f"{record.get('created_at', '').replace(':', '')}"
                  "@silk-platform.local>")
    ...
    except Exception as e:  # noqa: BLE001 — قناة جانبية؛ عطلها يُعلَن لا يُرفَع
        try:
            import silk_ops_log
            silk_ops_log.record_service_failure(
                "watchdog_alert", f"red-alert email failed: {e}")
        except Exception:  # noqa: BLE001
            pass
