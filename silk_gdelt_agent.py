...
def gdelt_news(query: str, market: str = "", months: int = 12,
    ...
    try:
        resp = throttled_get(_ENDPOINT, params=params, headers=_HEADERS,
                             timeout=_TIMEOUT)
        # EXT-17: الإعادةُ صارت ملكَ المسار المقوّى وحدَه (`throttled_get`: تراجُعٌ أسّيّ
        # يحترم `Retry-After` + قاطعُ دائرة) — إعادةُ الوكيل فوقها كانت تضاعف النداءات.
