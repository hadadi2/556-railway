...
from silk_data_layer import DataPoint, _today, throttled_get  # EXT-17
...
def retail_prices(query: str, market: str | None = None) -> list[DataPoint]:
    ...
    try:
        r = throttled_get(_PRICE_URL, params=params, timeout=_TIMEOUT)
        r.raise_for_status()
        payload = r.json()
...
def _comparable_prices(findings, currency, pack_size):
    """أساس واحد للمقارنة والتوصية؛ لا مزج عملة أو تعبئة أو قيمة غير منتهية."""
    import math
    prices = []
    for dp in findings or []:
        v = dp.value
        if (not currency or not pack_size or not isinstance(v, dict)
                or str(v.get("currency") or "").upper() != currency.upper()
                or str(v.get("pack_size") or "").strip().casefold() != pack_size.strip().casefold()):
            continue
        try:
...
def compare_own_price(own_price: float | None, findings: list[DataPoint], *,
                      currency: str | None = None, pack_size: str | None = None) -> dict:
    ...
    """
    prices = _comparable_prices(findings, currency, pack_size)
    ...
    if not prices:
        return {"your_price": own_price, "listings_count": 0,
                ...
                "note": "لا توجد قوائم بوحدة تعبئة وعملة معلومتين مطابقتين لسعرك — "
...
def suggest_price(findings: list[DataPoint], cost_per_unit: float | None = None,
                  ...
                  shipping_per_unit: float | None = None, *,
                  currency: str | None = None, pack_size: str | None = None) -> dict:
    ...
    if not prices:
        return {**empty,
                "note": "لا قوائم أسعار مرصودة بوحدة وعملة متطابقتين يُشتق منها نطاق — التوصية "
    ...
    floor = None
    if all(v is not None for v in (cost_per_unit, tariff_pct, shipping_per_unit)):
        floor = float(cost_per_unit) * (1.0 + float(tariff_pct) / 100.0) \
            + float(shipping_per_unit)
        floor = round(floor, 2)
