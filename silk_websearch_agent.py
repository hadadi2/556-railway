...
def _web_search_primary(query: str, num: int = 5,
    ...
    # ذاكرة قصيرة العمر لنداء **مدفوع بالنداء** (تدقيق 2026-08-27، البند ٢٦):
    # إعادةُ تشغيل/استئناف تشغيلةٍ كانت تدفع ثمن استعلاماتٍ متطابقة حرفياً.
    # مطفأة افتراضياً (`SILK_SEARCH_CACHE_TTL_S=0`) كي لا يتغيّر السلوك بلا
    # قرار مالك؛ وحين تُفعَّل لا تُخزَّن إلا **نتيجةٌ ناجحة** — لا فشلٌ يُكرَّس.
    #
    # **الفحص قبل العدّ** (مراجعة §58): كان `live_fetches` يُزاد قبل فحص
    # الذاكرة، فإصابةُ ذاكرةٍ تُحتسَب «جلبةً حية» **و**«إصابة ذاكرة» معاً
    # فيتضخّم `data_economics` المعروض للمالك. نفس ترتيب `silk_cache.cached_get`.
    cached = _search_cache_read(body)
    if cached is not None:
        organic = cached.get("organic") or []
        return _organic_to_points(organic, q, geo_note, int(num),
                                  cached_at=cached.get("_silk_cached_at"))
    # العدّ (`live_fetches`) يقع الآن داخل `throttled_request` نفسه — **لكل
    # محاولة** لا لكل نداءٍ منطقي (مراجعة §58): إعادة المحاولة على 429 نداءٌ
    # مدفوعٌ ثانٍ، وعدُّه واحداً كان يُخفي حتى ثلاثة أرباع الإنفاق الفعلي.
    try:
        import silk_data_layer
        # المسار المقوّى (البند ٨): مباعدة/إعادة/قاطع دائرة — 429 عابرة من
        # Serper كانت تصير فجوةً معلنة في تقرير مدفوع بلا محاولةٍ ثانية.
        resp = silk_data_layer.throttled_request(
            "POST", _SERPER_URL,
        ...
        if organic:
            _search_cache_write(body, payload)
...
def _search_cache_ttl() -> int:
    """عمر ذاكرة نتائج البحث بالثواني — 0/غير مضبوط = مطفأة (السلوك القائم)."""
    try:
        return max(0, int(os.environ.get("SILK_SEARCH_CACHE_TTL_S", "0") or 0))
    except ValueError:
        return 0


def _search_cache_path(body: dict) -> str | None:
    """مسار ملف الذاكرة لهذا الاستعلام — None حين تكون الذاكرة مطفأة.

    المفتاح من **جسم الطلب** (q/num/gl/hl) لا من الـURL (كل استعلامات Serper
    على نفس المسار)؛ ولا يدخله المفتاح السرّي أبداً (خادم واحد، مستأجر واحد —
    نفس منطق `silk_cache.cached_get` مع ترويسة WTO).
    """
    ...
    try:
        import os.path
        import silk_cache
        return os.path.join(silk_cache._cache_dir(),
                            silk_cache._key(_SERPER_URL, body) + ".json")
    except Exception:  # noqa: BLE001 — الذاكرة تحسين لا شرط
        return None


def _search_cache_read(body: dict) -> dict | None:
    """اقرأ نتيجةً محفوظةً طازجة — None عند الغياب/التقادم/التلف."""
    import json
    import time
    path = _search_cache_path(body)
    if not path:
        return None
    try:
    ...
    try:
        import silk_context
        silk_context.count_data("cache_hits")
    except Exception:  # noqa: BLE001 — العدّ شفافية لا شرط
        pass
    return data


def _search_cache_write(body: dict, payload: dict) -> None:
    """احفظ **نتيجةً ناجحة** فقط — a failure is never cached."""
    import json
    path = _search_cache_path(body)
    if not path:
        return
    try:
        ...
        # تاريخ الجلب الأصلي يُحفَظ **داخل** الحمولة كي يعود مع كل إصابة
        # ذاكرة، فلا تُقدَّم نتيجةٌ مخزَّنة كأنها جُلبت اليوم (قاعدة الإسناد).
        payload = {**payload, "_silk_cached_at": _today()}
...
def _organic_to_points(organic: list, q: str, geo_note: str, num: int,
                       cached_at: str | None = None) -> list[DataPoint]:
    """حوّل نتائج Serper الخام إلى نقاط بيانات — one shape for live and cached.

    `cached_at`: تاريخ **الجلب الأصلي** حين تأتي النتيجة من الذاكرة. قاعدة
    الإسناد في `CLAUDE.md` صريحة: القيمة المخدومة من مخزن تحتفظ بـ
    `retrieved_at` الأصلي وتُعلن ذلك في ملاحظتها — **لا تُقدَّم كجلبٍ حيّ
    اليوم**. كانت النسخة الأولى تختم نتيجة الذاكرة بتاريخ اليوم (مراجعة §58).
    """
    stamp = cached_at or _today()
    src = "Web Search (Serper)" + (" — من الذاكرة" if cached_at else "")
    cache_note = (f" — من ذاكرة الطلبات، جُلبت أصلاً {cached_at}"
                  if cached_at else "")
    if not organic:
        return [DataPoint(None, "Web Search (Serper)", 0.0,
    ...
    for item in organic[: int(num)]:
        # **الموجة C (البند T-05).** الرابطُ كان يعيش داخل `value` وحدَه، ولا
        # يصل حقلَ `url` في عقد النقطة — فينتهي ادّعاءٌ مبنيٌّ على صفحةِ ويبٍ
        # **بلا رابطٍ ولا صفٍّ في المراجع**: العميلُ يقرأ نتيجةً ولا يستطيع
        # فتحَ مصدرها. والدليلُ الذي لا يُفتَح ليس دليلاً يُراجَع.
        findings.append(DataPoint(
            {"title": item.get("title", ""),
             "snippet": item.get("snippet", ""),
             "link": item.get("link", "")},
            src, 0.5,
            f"organic result for '{q}'{geo_note}{cache_note}", stamp,
            url=str(item.get("link") or ""), retrieval_method="llm_web"))
    return findings
    ...
    **تُرتَّب أولاً وتُوسَم دليلاً ثانوياً ◐** برابطها وثقة منخفضة (0.4)، ثم
    ...
    for dom in domains:
        ...
        for f in sub:
            ...
            seen_links.add(link)
            v = dict(f.value)
            # الرابطُ يُفقَد مرّةً ثانيةً هنا: إعادةُ البناء كانت تُسقِط كلّ
            # حقلٍ لم يُذكَر صراحةً. يُحمَل من الأصل أو من قيمته.
            preferred.append(DataPoint(
                v, f"{f.source} — {dom}", 0.4,
                f"{_PREFERRED_NOTE} · {dom}", f.retrieved_at,
                url=(getattr(f, "url", "") or str(v.get("link") or "")),
                retrieval_method=(getattr(f, "retrieval_method", "")
                                  or "llm_web")))
...
def web_search_shopping(query: str, gl: str | None = None,
    ...
    try:
        ...
        # العدّاد داخل المسار المقوّى (مراجعة §58) — كل محاولة تُحسب، فلا
        # إعادةُ محاولةٍ مدفوعةٌ تمرّ بلا أثر في `data_economics`.
        import silk_data_layer
        # المسار المقوّى كشقيقه `web_search` (مراجعة §58): هذا نداء **مدفوع
        # بالنداء** أيضاً، فتركه عارياً كان يعني أن ارتجاجة 429 عابرة تصير
        # فجوةً معلنة بلا محاولة ثانية — ويبقى خارج مباعدة المضيف التي صار
        # بقيّة المسار يعتمد عليها.
        resp = silk_data_layer.throttled_request(
            "POST", _SERPER_URL,
            headers={"X-API-KEY": key, "Content-Type": "application/json"},
            json_body=body, timeout=_TIMEOUT)
