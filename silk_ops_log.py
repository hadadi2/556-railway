...
def _open(path: str):
    ...
    على ويندوز يُبقي الاتصالُ المفتوح الملفَّ محجوزاً، فينهار تنظيفُ
    `TemporaryDirectory` بـ`WinError 32` في أيّ اختبارٍ يمرّ من هنا — عطلٌ
    يظهر بعيداً عن سببه (سجلٌّ تشخيصيّ يُفشِل اختبارَ شيءٍ آخر تماماً).
    """
    conn = _connect(path)
    try:
        with conn:
...
def record_service_failure(service: str, reason: str,
    ...
    # وسم هوية التشغيلة (2026-08-17): حين يقع العطل داخل سياق تتبّع بحثٍ نشط
    # يُختَم الصفّ بـ`trace_id` — فيصير إسناد حارس المراقبة للتشغيلة **يقينياً**
    # بدل النافذة الزمنية التقريبية (الضعف المُصرَّح به في
    # `silk_watchdog._check_services`: تشغيلات متزامنة قد تتشارك سطراً).
    # قناة جانبية بحتة: استيراد كسول، وأي عطل هنا لا يمسّ مسار الاستدعاء.
    try:
        import silk_trace
        tid = silk_trace.current_trace_id()
        if tid:
            ctx.setdefault("trace_id", tid)
    except Exception:  # noqa: BLE001 — الوسم تحسين، غيابه ليس عطلاً
        pass
