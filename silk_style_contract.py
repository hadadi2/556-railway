...
WRITING_STANDARD_RULE = (
    ...
    "الشحنة التجريبية (سعةٌ لوجستية لا حجمُ طلب)، استرداد كلفة الشحنة "
    "التجريبية ونقطة التعادل التشغيلي (مقياسان منفصلان لا اسمٌ واحد)، وتكلفة "
    "تمويل مخزون التجربة (مقياسٌ ثالث منفصل)، الزمن "
    "من القرار إلى أول فاتورة بالأسابيع، وأقصى خسارة إن فشل الدخول. المتعذر "
    "حسابه يُطبع بمعادلته وناقصه: «نقطة التعادل التشغيلي = التكاليف الثابتة "
    "÷ هامش المساهمة. المتاح: الهامش. الناقص: التكاليف الثابتة.» — أنفع من "
    "إسقاطه صامتاً.\n"
...
WRITING_STANDARD_RULE_EN = (
    ...
    "first shipment, trial shipment size (a logistics capacity, not order "
    "size), trial-shipment cash recovery and operating break-even as two "
    "separate measures, weeks from decision to first invoice, and maximum "
    "downside "
