...
def analyze(product_name: str, countries: list[dict] | None = None,
    ...
    # EXT-21 (تدقيق 2026-09-01): جدارٌ زمنيّ كلّي معلَن — `deadline_s` أو
    # `SILK_ANALYZE_DEADLINE_S`؛ بلا قيمة = السلوكُ القديم حرفياً. كلُّ طبقةِ إثراءٍ تفحص
    # المتبقّي قبل أن تبدأ، وما تُخطّي يُعلَن في `result["deadline"]["skipped_layers"]`.
    import time as _dtime
    if deadline_s is None:
        _env_dl = os.environ.get("SILK_ANALYZE_DEADLINE_S", "").strip()
        try:
            deadline_s = float(_env_dl) if _env_dl else None
        except ValueError:
            deadline_s = None
    # مراجعة EXT (§58): الجدارُ يدخل **سياقَ التشغيلة** (`silk_context.deadline_context`) لا
    # متغيّراً محلّياً — فتراه البعثاتُ والوكلاءُ وطبقاتُ الجلب المتداخلة عبر `remaining_s()`.
    import silk_context as _sctx
    _deadline_cm = _sctx.deadline_context(deadline_s)
    _deadline_cm.__enter__()
    _deadline_at = (_dtime.monotonic() + deadline_s) if deadline_s and deadline_s > 0 else None
    _skipped_layers: list[str] = []

    def _layer_ok(name: str) -> bool:
        left = _sctx.remaining_s()
        if left is not None and left <= 0:
            _skipped_layers.append(name)
            return False
        return True
    ...
    if _deadline_at is not None:
        result["deadline"] = {"seconds": deadline_s,
                              "exceeded": (_sctx.remaining_s() or 0) <= 0,
                              "skipped_layers": list(_skipped_layers)}
    _deadline_cm.__exit__(None, None, None)      # مراجعة EXT: لا يتسرّب خارج التحليل
    result["data_economics"] = _economics(_data_counter)
    if check_quality:
        _annotate_quality(result)
    if persist:
        ...
        try:
            ...
            for ind, label in _RISK_INDICATORS:
                ...
                if got and got.get("value") is not None:
                    try:  # إصابة مخزن تُحسب وتحتفظ بتاريخ جلبها الأصلي (صيد ٣)
                        import silk_context
                    ...
                    findings.append(DataPoint(
                        ...
                        got.get("retrieved_at") or _today()))
...
def _enrich_research(rows: list[dict], product_name: str, hs_code: str,
    ...
    # R1 (تدقيق 2026-09-01، CONC-1): نسخةُ سياقٍ لكل سوق من خيط المنادي —
    # وإلا فقد المنسّقُ داخل العامل حجبَ الإضافات ولوحةَ الوكلاء والعدّاد.
    if rows:
        import silk_context
        with ThreadPoolExecutor(max_workers=min(len(rows), 3)) as ex:
            list(silk_context.map_with_context(ex, _one, rows))
...
def _enrich_requirements(rows: list[dict], hs_code: str) -> None:
    ...
    for row in rows:
        try:
            ...
            # الموجة C (§٥): حالةُ الحواجز **بنيوياً** بجوار القائمة — لا
            # داخلها: `requirements` عقدُها «كلُّ قيمةٍ بندُ تحقّقٍ باتجاه»،
            # وحقنُ شكلٍ آخر فيها كسر أربعةَ اختباراتٍ قائمة فوراً.
            try:
                from silk_requirements_agent import regulatory_state
                row["regulatory"] = regulatory_state(
                    row.get("iso3"), hs_code)
            except Exception as _e:  # noqa: BLE001 — الحالةُ إضافةٌ لا شرط
                log.warning("regulatory state failed: %s", _e)
        except Exception as e:  # noqa: BLE001 — context layer must not crash analysis
...
def _persist(result: dict, db_path: str | None) -> None:
    ...
    try:
        ...
        # R2b (API-12): الفشلُ يُسمّى في النتيجة (اسمُ الصنف فقط — لا نصّ خام)
        # ويُسجَّل للمشغّل؛ `/analyze` يعيد `analysis_id: null` + `persist_error`.
        result["persist_error"] = type(e).__name__
        try:
            import silk_ops_log
            silk_ops_log.record_error("persist_failure",
                                      f"save_analysis failed: {type(e).__name__}",
                                      context={"product": result.get("product")})
        except Exception:  # noqa: BLE001 — سجلّ المشغّل قناة جانبية
            pass
