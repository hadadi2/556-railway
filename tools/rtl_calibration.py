...
def is_arabic_majority(text: str) -> bool:
    """سطرٌ عربيُّ الأغلبية — **تفويضٌ** إلى `silk_reports.is_arabic_majority`.

    كان التعريفُ مكرَّراً هنا وهناك؛ التكرارُ يعني انحرافَ المقياسين بصمت،
    والبوّابةُ الإنتاجية هي المرجع. Delegates to the production definition."""
    import silk_reports
    return silk_reports.is_arabic_majority(text)
...
def build_bracket_fixture(isolate: bool, bidi: bool = True) -> str:
    """ابنِ docx أسطره العربية تحوي أقواساً لاتينية.

    `isolate`: عزلُ RLM (WP-5) من عدمه. `bidi=False`: **العيّنةُ المقلوبةُ
    فعلاً** — فقرةٌ بلا `<w:bidi/>` أساسُها LTR، فتُصيَّر أقواسُها `()` بدل
    `)(`. بلا هذه العيّنة لا يُقاس تمييزُ المقياس أصلاً: عيّنتا C/D (كلتاهما
    bidi) خرجتا **متطابقتين** على المقياسين معاً — أي أنّ عزل RLM لا أثرَ له
    على اتجاه الأقواس في هذا الخطّ الإنتاجي، وهو نفسُه نتيجةُ قياسٍ تستحقّ
    أن تُطبَع لا أن تُفترَض. The genuinely-inverted cell of the matrix."""
    from docx import Document
    from docx.oxml import OxmlElement
    from docx.oxml.ns import qn
    import silk_reports
    ...
    for ln in _BRACKET_LINES:
        ...
        if bidi and ppr.find(qn("w:bidi")) is None:
            ppr.append(OxmlElement("w:bidi"))
        el = OxmlElement("w:jc")
        el.set(qn("w:val"), "start" if bidi else "left")
        ppr.append(el)
        rpr = r._r.get_or_add_rPr()
        if bidi and rpr.find(qn("w:rtl")) is None:
            rpr.append(OxmlElement("w:rtl"))
    tag = ("iso" if isolate else "raw") + ("" if bidi else "_nobidi")
...
def main() -> int:
    ...
    # WP-5: معايرةُ مقياسَي الأقواس جنباً إلى جنب — C (بعزل RLM) مقابل D
    # (خام). العمودُ «suspicious» هو المقياسُ **القديم** (تشخيصيّ فقط: أُثبِت
    # أنه مُعاكِسُ الإشارة — يرتفع على السليم وينخفض على المقلوب)، والعمودُ
    # «inverted» هو مقياسُ الهندسة الذي تعتمده البوّابةُ فعلاً.
    # نرفع العتبةَ مؤقتاً كي تُقاس كلُّ عيّنةٍ بلا رفع استثناء.
    os.environ["SILK_PDF_BRACKET_FAIL_MAX"] = "9999"
    try:
        import silk_reports as _R
        for label, iso, bidi in (("C(bracket,RLM-isolated)", True, True),
                                 ("D(bracket,raw)", False, True),
                                 ("E(bracket,NO-bidi=INVERTED)", False, False)):
            try:
                pdf = to_pdf(build_bracket_fixture(iso, bidi))
                n = bracket_suspicious_count(pdf)
                geo = _R.bracket_orientation_counts(pdf)
                shown = ("لم يقع قياس (لا pymupdf)" if geo is None else
                         f"ok={geo[0]} inverted={geo[1]} skipped={geo[2]}")
                print(f"{label}: suspicious(old)={n}  geometry: {shown}")
            except Exception as e:  # noqa: BLE001
                print(f"{label}: ERROR {e}")
    finally:
