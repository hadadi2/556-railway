...
def netherlands_research_blob() -> dict:
    ...
    missions = {
        ...
        # البند 5 (أمر إصلاح المحرّك): الملاحظة تحمل حجم العبوة والعملة —
        # كما تكتبها البعثة الحيّة فعلاً (الدرس ٩٦: المقعد يطابق ما
        # يستبدله)؛ بدونهما يُعلَّق الحل العكسي معلَناً وهذا مسار آخر.
        "pricing_scout": _m("أسعار رفّ 6.20–9.80€ (60/60)",
                            [_dp(7.49, "Google Maps",
                                 note="Albert Heijn سعر رف عبوة 1 كجم "
                                      "بسعر €7.49")]),
        # **الموجة C (البند E-10).** كان المفتاح `competition` بينما مفتاحُ
        # الإنتاج في `silk_missions.MISSIONS` هو `competitors`. فمدوّنةُ رُتبة
        # ٢ لا تطابق شكلَ البعثات الإنتاجيّ، ولا يُمرَّن مسارُ الاقتصاد
        # (HHI ⇒ بوّابةُ الإزاحة) في أيّ تشغيلةِ قبولٍ إطلاقاً — تُختبَر
        # المدوّنةُ نفسَها لا النظام.
        "competitors": _m("HHI 940 — سوق مفتّت", [_dp(940, note="HHI تركّز")]),
