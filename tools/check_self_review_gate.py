...
def check(body: str) -> list[str]:
    ...
    # R7 (تدقيق 2026-09-01، CI-7): القالبُ غيرُ المملوء كان يمرّ — العنوانُ والإعلانُ
    # موجودان في القالب نفسه. تعليقاتُ `<!-- … -->` = قالبٌ لم يُملأ.
    if re.search(r"<!--.*?-->", body, re.S):
        problems.append(
            "PR body still contains template placeholders (<!-- … -->) — fill the "
            "self-review section in; an untouched template is not a review.")
    return problems
