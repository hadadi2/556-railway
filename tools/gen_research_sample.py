...
# **الموجة B (البند E2).** الكتابةُ محروسةٌ بمتغيّر بيئة صريح. السببُ:
# `tests/test_wave11_identity_and_hardening.py` ينفّذ هذا الملفّ عبر
# `runpy.run_path` ليقرأ `result` منه، فكان **كلُّ تشغيلٍ للحزمة** يعيد كتابة
# `samples/research_report_latest.docx` ويُوسِّخ شجرةَ العمل (المحتوى مطابقٌ
# تماماً — كلُّ ١٩ مدخلةً بنفس الـCRC — والفرقُ طوابعُ الـzip وحدها). ضجيجٌ
# في كلّ فرق عمل، وخطرُ التزامٍ عرَضيّ، **ويُخفي تغيُّراً حقيقياً لو حدث**.
# التوليدُ المقصود يبقى كما هو: `python3 tools/gen_research_sample.py`.
_WRITE = (__name__ == "__main__"
          or os.environ.get("SILK_WRITE_SAMPLES", "").strip() == "1")
