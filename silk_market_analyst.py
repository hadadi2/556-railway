...
_ANALYST_MISSION = {
    ...
    "instructions": (
        ...
        # الموجة د-٣: المثالُ الأوّل كان «نسبة السكان المسلمين × واردات
        # المنتج تكشف حجم الطلب الحقيقي» — وهو **نقيضُ** القاعدة أسفلَ
        # الموجّه نفسِه («السكان ونسبة دينية لا يثبتان وجود مشترين»).
        # تعليمتان متعارضتان في نداءٍ واحد، والنموذجُ يختار؛ فحُذف المثال.
        "(أرضية لا سقف): الدخل للفرد × أسعار الرفّ يحدّد "
        ...
        "ثقافة الاستهلاك، حجم الاستيراد ونموه، السكان، الموسمية المرصودة، "
        "أي اتجاه بحث ذي صلة — تقدير **استدلالي "
        ...
        "**إن اكتملت مدخلات حساب مفيد للقرار، اعرض المعادلة والأرقام "
        "المستشهَد بها صراحة، مع تطابق المنتج والسوق والفترة والوحدات.** "
        "يُمنَع كتابة حجم طلب أو إيراد من افتراض استهلاك غير مسند. السكان "
        "ونسبة دينية لا يثبتان وجود مشترين لهذا المنتج. وجود رقمين وحده "
        "لا يكفي لحساب صالح؛ صرّح بالمدخل الناقص وبما يلزم للتحقق منه."),
}
...
def _tag_source_reports(mission_reports: dict[str, AgentReport]) -> list:
    ...
    for key, report in (mission_reports or {}).items():
        for dp in report.findings:
            tagged.append(replace(dp, note=f"[{key}] {dp.note}"))
    return tagged
...
def analyze_market(market: MarketRef, product: str,
    ...
    tagged = _tag_source_reports(price_reports(normalized_reports(mission_reports), product))
    ctx_parts = []
    ...
    # p6/T6 (الجذر، الفرع ب الحيّ): النداء الثقيل يُبثّ — مهلة خمول لا جدار
    # ٣٠٠ث، والجزء الواصل يُحلَّل عند الإجهاض بدل ضياعه. `timeout` يبقى
    # _LONG_TIMEOUT (الحقل المقفول في التتبّع؛ أرضية السقف الكلّي للبثّ).
    report = run_llm_agent(
        ...
        timeout=_LONG_TIMEOUT, model=_SMART_MODEL, stream=True)
    # تفصيل آخر خطأ/إجهاض في المزوّد يُقرأ فوراً (قبل أيّ نداء آخر) — يصل
    # التشخيص فلا يُخمَّن سبب «جزئي» أو «فارغ» من شكل النتيجة.
    import silk_llm_provider as _prov
    _llm_error = dict(_prov.last_error() or {}) or None
    ...
    diagnostics = {
        ...
        # p6/T6: بثٌّ أُجهِض لكن وصل جزء قابل للتحليل — نتائج جزئية معلَنة.
        "stream_aborted": bool(_llm_error) and bool(report.findings),
