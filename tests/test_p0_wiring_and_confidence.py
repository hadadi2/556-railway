...
def test_conditional_go_why_lists_only_true_reasons():
    ...
    # البند 2 (أمر إصلاح المحرّك): عمودٌ واحد لم يعد يُنتج درجةً أصلاً —
    # الحزمة توسَّع لثلاثة أعمدة (الحد الأدنى) وتبقى ثقتها دون 0.6 عبر
    # التغطية (0.8 × 3/5 = 0.48) فيبقى مقصد الاختبار كما هو: أسباب صادقة.
    b = _bundle(coverage=0.8)
    b["pillar_inputs"]["competition_intensity"] = {"hhi": 2000.0}
    b["pillar_inputs"]["risk"] = {"political_stability_wgi": 0.0,
                                  "logistics_lpi": 3.0}
    d = D.decide(b)
