...
def test_gdelt_429_twice_declares_a_distinct_gap_no_retry_spam():
    ...
    with patch("requests.get", side_effect=fake_get), \
        ...
        out = gd.gdelt_news("تمور", "Netherlands")
    # EXT-17 (تدقيق 2026-09-01): الإعادةُ صارت ملكَ المسار المقوّى وحدَه (`throttled_get`)
    # بدل إعادةٍ ثانية داخل الوكيل — نفسُ سياسة كلّ مصدرٍ آخر: محاولةٌ + `SILK_HTTP_RETRIES`.
    assert 2 <= calls["n"] <= 4, calls["n"]
...
def test_truncate_at_word_never_cuts_mid_word():
    # قفل محدَّث معلن (بلاغ حجب #12 — قاعدة «لا نقاط حذف» البنائية):
    # القصّ يُعلَن «(مختصر)» لا «…» — الذيل القديم كان يُطلِق بوابة نصّ
    # المُنتَج على docx العميل.
    ...
    assert cut.endswith(" (مختصر)") and not cut.endswith("…")
    body = cut[:-len(" (مختصر)")].rstrip()
...
def test_llm_runtime_citation_note_truncation_is_word_safe():
    ...
    assert cut.endswith(" (مختصر)") and not cut.endswith("…")
    assert not cut[:-len(" (مختصر)")].rstrip().endswith("كلم")  # لا قطع منتصف الكلمة
