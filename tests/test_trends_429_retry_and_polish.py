...
def test_all_pytrends_fetch_sites_route_through_retry():
    ...
    src = open(os.path.join(_ROOT, "silk_trends_agent.py"),
               encoding="utf-8").read()
    # EXT-12: الاهتمام/السلسلة/الموسمية صارت جلباً واحداً (`_interest_with_series`) —
    # موضعان: الجلبُ الموحَّد + السياق (related queries)، وكلاهما تحت `_retry_429`.
    assert src.count("_retry_429(") >= 2
    helper = src.split("def _interest_with_series(")[1].split("\ndef ")[0]
    assert "_retry_429(" in helper
    assert src.count("interest_over_time()") == 1, "جلبُ السلسلة خارج المسار الموحَّد"
