...
def test_client_structure_headings_in_order(tmp_path):
    ...
    # الترتيبُ يُقاس على **العناوين** لا على أوّل ورودٍ للنصّ في المستند.
    # `text.find` كان يلتقط ذكرَ اسم القسم داخل فقرةٍ سرديّة سابقة («مصادرُ
    # كل مؤشرٍ مجموعةٌ في قسم «المراجع» ختام التقرير») فيقلب الترتيبَ ظاهرياً
    # ويفشل الاختبارُ فشلاً كاذباً على إضافةٍ سليمة (الموجة B، البند T-11).
    from docx import Document
    heads = [p.text.strip() for p in Document(out).paragraphs
             if p.style.name.startswith("Heading") and p.text.strip()]
    positions = []
    for h in order:
        idx = next((i for i, t in enumerate(heads) if h in t), -1)
        assert idx >= 0, f"client section missing: {h}"
        positions.append(idx)
    assert positions == sorted(positions), (
        f"client sections out of order: {list(zip(order, positions))}")
...
def test_missions_table_replaced_by_methodology_paragraph(tmp_path):
    ...
    # أسلوبُ التحقّق. المرساةُ كانت «خضعت كل معلومة للتحقّق من مصدرها
    # العمومي المباشر» — وهو **وعدٌ لا تُنفّذه أيُّ طبقة** (الإسنادُ يُقاس على
    # مؤشّرات البعثات لا على أرقام المتن)، فأُبدِل في الموجة B (البند T-07)
    # بوصفٍ صادقٍ لما يجري فعلاً. الاختبارُ يقفل **وجودَ وصف الأسلوب** لا
    # صيغةَ الوعد الملغى.
    assert "مع تحليل تقاطعي" in text
    assert "فجوةً صريحة لا تقديراً" in text
    assert "خضعت كل معلومة للتحقّق" not in text, (
        "عاد الوعدُ غيرُ المُنفَّذ إلى فقرة المنهجية (البند T-07)")
...
def test_evidence_log_formats_numbers_and_drops_meaningless_rows(tmp_path):
    ...
    # هدف الدراسة الاحترافية (البند ٣): جملة معنى تحفظ الرقم بلا «HHI».
    assert R._client_readable_fact({"hhi": 2100.0}, "n") == \
        "تركّز المورّدين 2100 من 10000 — التوريد موزّع على منافسين كثر"
