...
def test_regenerate_report_calls_writer_once_with_reconstructed_reports():
    ...
    def fake_write_reviewed_report(mission_reports, analyst_summary, verdict,
                                   ...
                                   style=None, seed_draft=None, importer_leads=None, product_card=None,
                                   entry_decision=None, analyst_by_category=None):
        ...
        captured["seed_draft"] = seed_draft  # بلاغ تحليل 20: بذرة الاستئناف
...
def test_regenerate_report_seeds_writer_from_stored_partial():
    """بلاغ تحليل 20: إعادةُ التوليد على تشغيلةٍ **بلا تقرير كامل** + مسوّدةٍ
    محفوظة (`writer_partial`) تمرّر البذرةَ (`seed_draft`) للكاتب فيُكمِل الأقسامَ
    الباقية بدل توليد المسوّدة من الصفر (نداءٌ غالٍ). مع وجود تقريرٍ كاملٍ سابق
    لا تُبذَر (يُغطّيه الفرع الآخر)."""
    from silk_agents import AgentReport
    live_reports = {"trade_flow": AgentReport("LLMAgent:trade_flow", [],
                                              False, "ok")}
    captured = {}

    def fake_wrr(mission_reports, analyst_summary, verdict, product,
                 market_name, trace_id=None, hs_code=None,
                 hs_confirmation=None, style=None, seed_draft=None, importer_leads=None, product_card=None,
                                   entry_decision=None, analyst_by_category=None):
        captured["seed_draft"] = seed_draft
        return {"report": "## 1. الخلاصة التنفيذية\nاكتمل من البذرة.",
                "review_cycles": 1, "unresolved_notes": []}

    _stages = {"writer_partial": {"status": "partial",
                                  "payload": {"text": "## 1. جزء محفوظ سابق"},
    ...
    with _env(SILK_API_KEY="secret", SILK_RATE_LIMIT="0",
        ...
        with patch("silk_storage.get_analysis",
             ...
             patch("silk_storage.load_mission_checkpoints",
                   return_value=live_reports), \
             patch("silk_storage.load_stage_checkpoints", return_value=_stages), \
             patch("silk_ai_judge.write_reviewed_report", side_effect=fake_wrr), \
             patch("silk_storage.save_analysis",
                   side_effect=lambda *a, **k: 42):
            r = client.post("/analyses/42/report",
                            headers={"X-API-Key": "secret"})
    assert r.status_code == 200
    assert captured["seed_draft"] == "## 1. جزء محفوظ سابق"  # بُذِر من المخزَّن


def test_regenerate_report_seed_0_skips_seeding_for_reliable_fresh_draft():
    """بلاغ تحليل 20 (شبكةُ أمانِ الموثوقية): `?seed=0` يتخطّى بذرةَ
    `writer_partial` رغم وجودها — فيُعاد توليدُ مسوّدةٍ كاملةٍ من الصفر (مع
    ميزانية ٢٤ألف تكتمل ١١/١١ في نداءٍ واحدٍ يتفادى الإكمالَ الغامض) بدل تكرار
    الإكمال العالق على ٨/١١ في كلّ نداء."""
    from silk_agents import AgentReport
    live_reports = {"trade_flow": AgentReport("LLMAgent:trade_flow", [],
                                              False, "ok")}
    captured = {}
    ...
    with _env(SILK_API_KEY="secret", SILK_RATE_LIMIT="0",
        ...
        with patch("silk_storage.get_analysis",
             ...
             patch("silk_storage.save_analysis",
                   side_effect=lambda *a, **k: 42):
            r = client.post("/analyses/42/report?seed=0",
                            headers={"X-API-Key": "secret"})
    assert r.status_code == 200
    assert captured["seed_draft"] is None   # لم يُبذَر رغم وجود الجزء المخزَّن
