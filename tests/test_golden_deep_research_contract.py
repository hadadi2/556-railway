...
def test_golden_b_hs_gate_blocks_kuwait_peanut_butter_on_both_paths_live():
    ...
    for label, r in (("research", r_research), ("analyze", r_analyze)):
        ...
        # القفلُ على **العائلة** لا على تهجئةٍ واحدة: بعد موجة خطّ التصنيف
        # صار الرفضُ يسمّي سببَه بدقّة (`hs_catalog_conflict`: المُحلِّل يعيد
        # 200811 لهذا الاسم) بدل رمزٍ عامّ واحد. السلوكُ المحميّ هو الرفضُ
        # قبل الإنفاق على المسارين، لا اسمُ الرمز.
        from silk_platform.engine_bridge import _HS_GATE_CODES
        assert r.json()["detail"]["error"] in _HS_GATE_CODES, (
            label, r.json()["detail"]["error"])
