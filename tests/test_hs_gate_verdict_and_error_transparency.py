...
def test_research_path_carries_hs_confidence_into_the_result():
    ...
    # الدرس ١٩٩: حارسٌ يقفل **موقعَ** سطرٍ يمنع إعادةَ الهيكلة بلا أن يحمي
    # سلوكاً. الثقةُ لم تعد تُشتقّ من `dp` مباشرةً بل من خطّ التصنيف الواحد،
    # فالقفلُ صار على **الخاصيّة**: رقمٌ حقيقيّ يُحمَل إلى النتيجة، ويُمرَّر
    # للبوّابة قبل أيّ حجزٍ أو دولار.
    import silk_hs_pipeline as P
    from tests.api_source import api_layer
    src = api_layer()
    assert '"hs_confidence": hs_confidence,' in src
    assert "hs_confidence=hs_confidence" in src
    # والخاصيّةُ نفسُها مقيسةً: تصنيفٌ ناجحٌ يحمل ثقةً **رقمية** فوق العتبة،
    # ولا يعيد `None` التي وُلِدت منها «ثقة التصنيف —» أصلاً.
    out = P.classify("تمور")
...
def _env(**vals):
    """ضبطُ متغيّرات بيئةٍ مع استعادةٍ مضمونة — نفسُ نمط بقية السويت."""
...
def test_scanner_flags_same_code_but_below_the_new_threshold(tmp_path):
    ...
    البذرةُ السابقة («نفط خام» ⇒ 510111 الصوفُ الخام) كانت **تصادفاً حرفياً**
    من فرع الاحتواء الذي حُذف في موجة خطّ التصنيف: «خام» داخل اسمين لا
    علاقةَ بينهما. بذرةٌ تعتمد على عيبٍ أُغلِق تقيس العيبَ لا الحارس. الآن
    البذرةُ حتميّة: رمزٌ يُحلّ برتبة **مصطلح العائلة** (0.82) وعتبةٌ مرفوعة
    من البيئة فوقه — نفسُ الحالة المقصودة، بلا اعتمادٍ على مصادفة.
    """
    with _env(SILK_HS_MIN_CONFIDENCE="0.90"):
        out = _audit(_seeded_db(tmp_path, [("جبن شيدر", "040690")]))
        assert len(out) == 1
        assert out[0]["resolver_today"] == "040690"   # نفسُ الرمز — لا «خلاف»
        assert out[0]["resolver_confidence"] < 0.90
...
def test_seed_coverage_measures_the_root_cause_and_locks_it():
    ...
    **رمزَ إمدادٍ فقط**: لا يظهر إلا حين يُمرَّر صراحةً — وهناك عاشت حادثةُ
    ...
    # الأرقامُ كانت مقيسةً على البذرة **المتروكة** (159 من 5627) لأن
    # `seed_coverage` كانت تقرأ أعمدتها القديمة وتعيد صفراً عن الملفّ الحيّ —
    # مقياسٌ أعمى يُطمئِن كذباً (بلاغ المالك «تحديد HS بدقة»). بعد إصلاحه
    # يقيس المرجعَ الحيّ: **146 من 5613**. الرقمُ نزل لأن المقياس صار صادقاً
    # لا لأن التغطية تدهورت — ولذلك يُعاد ضبطُ القفل على المقيس فعلاً.
    assert cov["supply_only"] <= 5467, (
        f"عددُ رموز الإمداد-فقط ارتفع إلى {cov['supply_only']} — كلُّ صفٍّ "
        "جديدٍ بلا عربية يوسّع السطحَ الذي دخل منه 040110")
    assert cov["arabic_guarded"] >= 146
...
def test_040110_family_is_supply_only_by_construction():
    ...
    # المخطّط المُرحَّل (#139 المُكيَّف): الحارس العربي يعيش في `keywords_ar`.
    assert (rows["040120"].get("keywords_ar") or "").strip()
    for code in ("040110", "040140", "040150"):
        assert not (rows[code].get("keywords_ar") or "").strip(), code
...
def test_failed_narrative_is_marked_separately_from_the_verdict():
    ...
    # (الموجة ٠ — لغة المصنع تحكم لغة التقرير: التسمية صارت تمرّ بـ
    # `_verdict_label(tone, lang)` كي تخرج بلغة التقرير. **نفس الاشتقاق
    # الحتمي حرفياً** — `verdict_tone` وحده مصدرها، ولا علاقة للكاتب بها؛
    # والعربية داخل `_verdict_label` تبقى من `_VERDICT_LABELS_AR` نفسها.)
    assert '"verdict_label": _verdict_label(verdict_tone, lang),' in dr
    assert "_VERDICT_LABELS_AR[tone]" in src   # العربية من المصدر الواحد نفسه
