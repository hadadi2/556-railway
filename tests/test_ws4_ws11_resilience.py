...
def test_http_get_open_breaker_fails_fast_single_attempt(monkeypatch):
    ...
    calls["n"] = 0
    # EXT-1 (تدقيق 2026-09-01): القاطعُ المفتوح لا ينادي الشبكةَ إطلاقاً — `CircuitOpen`
    # (كان «الفشلُ السريع» نداءً حيّاً واحداً لكلّ نداء fan-out).
    import pytest
    with pytest.raises(silk_circuit.CircuitOpen):
        dl._http_get("https://x.example/api")
    assert calls["n"] == 0, "open breaker must not touch the network"
    silk_circuit.http_breaker.reset()
