...
def test_writer_escalation_is_bounded_and_traces_each_attempt():
    ...
    # الافتراضية (24000→48000، رُفعت لبلاغ تحليل 20: ١١ قسماً تحتاج ~٢٢ألف) يبلغ
    # التضعيفُ السقفَ الصلب بخطوة واحدة، فيتوقّف عند محاولتين (إعادة النداء بنفس
    # السقف لا تفيد) — كلاهما حدّ صريح. رموزٌ صريحة عبر الرمز لا الحرفيّة.
    assert aj._MAX_TOKENS_RETRIES == 3            # الحدّ الأقصى المعلن للمحاولات
    assert caps == [aj._WRITER_MAX_TOKENS, aj._MAX_TOKENS_CEILING]  # صعّد مرة ثم توقّف
...
def test_writer_base_budget_fits_eleven_sections_analysis20():
    """بلاغ تحليل 20: مسوّدةٌ بلغت max_tokens=16000 وأنتجت ٨ أقسام من ١١. فالميزانية
    الأساسية يجب أن تسع أحدَ عشر قسماً (~٢٢ألف رمز بأسلوب تلك التشغيلة) كي يكتمل
    التقرير في نداءٍ واحد بدل إجبار كلّ تقرير على الإكمال الهشّ. (السقفُ الصلب >
    الأساس محكومٌ في test_command6 — لا يُكرَّر هنا؛ وقراءةُ env مضمونةٌ ببناء
    الثابت من os.environ في رأس الوحدة.)"""
    import silk_ai_judge as aj
    assert aj._WRITER_MAX_TOKENS >= 22000, (
        f"ميزانية المسوّدة ({aj._WRITER_MAX_TOKENS}) دون ١١ قسماً — تُجبِر الإكمال")


def test_writer_escalation_meters_every_attempt_in_cost():
    """كل نداء كاتب يُحسَب وتُقاس رموزه (الذيل خارج سقف النداءات الكلي فالقياس
    شرط). بلاغ تحليل 20: مموّهٌ يعيد نصّاً لا يزيد التغطية => حارسُ العملية
    العقيمة يوقف الحلقة بعد إكمالٍ واحد (لا نداءٌ ثالثٌ مهدِر)، والجزءُ الأوفى
    **يُسلَّم** موسوماً (لا None). النداءان المنفَّذان مقيسان بالكامل."""
    import silk_ai_judge as aj
    import silk_context
    from silk_pricing import estimate_cost_usd

    def fake_post(url, **kw):  # اقتطاع دائم *مع* نص جزئي لا يزيد التغطية
        return _resp({"stop_reason": "max_tokens",
    ...
    with _env(ANTHROPIC_API_KEY="k", SILK_API_KEY="x"), \
        ...
        out = aj.deep_report(_mission_reports(), "محلل",
                             {"verdict": "WATCH"}, "تمور", "هولندا")
    # قرار المالك (تجاوز §5-الإتلاف): الجزء يُسلَّم موسوماً، لا None
    assert out and aj._writer_incomplete(out)
    # حارسُ العملية العقيمة: مسوّدة + إكمالٌ واحدٌ عقيمٌ ثم توقّف — نداءان مقيسان
    assert c["llm_calls"] == 2
    usage = c["llm_usage"]
    total_out = sum(v.get("output_tokens", 0) for v in usage.values())
    assert total_out == 100                        # 2×50 إخراج متراكم (لا ثالثٌ مهدِر)
