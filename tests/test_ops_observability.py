...
def test_export_failure_reason_sanitized_before_ops_log():
    ...
    with _env(SILK_API_KEY="x", SILK_RATE_LIMIT="0", SILK_OPS_LOG_DB=db):
        ...
        blob = {"product": "تمور", "hs_code": "080410",
               ...
               "deep_research": {
                   # الموجة B (البند T-01): البوّابةُ صارت تُفشِل تقريراً بلا
                   # **أيّ** دليلٍ مرصود — وهذا الاختبارُ يقصد مسارَ فشلٍ
                   # **لاحقاً** للبوّابة، فيلزمه دليلٌ واحدٌ ليعبرها كما ينصّ
                   # تعليقُه أصلاً («كي يمرّ ببوابة الجودة، لا 409 حجب»).
                   # القيمةُ مطابقةٌ لمتن المدوّنة عمداً: فحصُ تناقضِ
                   # الأدلة والمتن حاجبٌ، ورقمٌ مخالفٌ كان سيُفشِل التشغيلة
                   # لسببٍ لا علاقة له بسؤال هذا الاختبار.
                   "missions": {"trade_flow": {"findings": [
                       {"value": 42000000, "source": "UN Comtrade",
                        "confidence": 0.8, "note": "واردات مرصودة 2023"}]}},
                   "analyst": {}, "verdict": {},
