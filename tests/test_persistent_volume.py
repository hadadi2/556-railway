...
def test_defaults_unchanged_without_env(monkeypatch):
    ...
    for var in ("SILK_DATA_DIR", "SILK_DB", "SILK_STORE_DB",
        ...
        monkeypatch.delenv(var, raising=False)
    # Issue #240: `conftest._reapply_isolated_analyses_db` (autouse) يُطعِّم
    # `silk_storage._DEFAULT_PATH` بمسارٍ مؤقّتٍ معزول لكل اختبارٍ آخر — هذا
    # الاختبار تحديداً يفحص **الحرفيّة الأصلية** من المصدر، فيستعيدها هنا
    # صراحةً قبل التأكيد (لا يمسّ عزل بقية الحزمة، ولا يمسّ الوحدات الثلاث
    # الأخرى غير المعزولة أصلاً بهذا التثبيت).
    monkeypatch.setattr(silk_storage, "_DEFAULT_PATH", os.path.join("data", "silk.db"))
