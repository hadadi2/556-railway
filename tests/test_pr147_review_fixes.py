...
def test_export_rephrase_is_gated_and_cached_in_source():
    """الضماناتُ الثلاث لنداء الصياغة — حيث صارت تعيش، **ولدى السطحين**.

    الموجة B (البند G-05): السلوكُ نفسُه انتقل من مغلقةِ `api.py` إلى
    `silk_export_gate.prepare_fallback_prose` كي **يستهلكه سطحُ المصنع أيضاً**
    (كان يُولَّد على مسار المشغّل وحدَه، فيتسلّم المصنعُ «السرد غير متاح» حيث
    يتسلّم المشغّلُ نثراً مُعادَ صياغته). المرساةُ تتبع الشيفرة ولا تُضعَّف:
    الضماناتُ الثلاث تُفحَص في موضعها الجديد، **ويُضاف** قفلٌ على أنّ كلا
    سطحَي التصدير ينادي المُحضِّر الواحد.
    """
    gate = _src("silk_export_gate.py")
    prep = gate.split("def prepare_fallback_prose(")[1].split("\ndef ")[0]
    assert "ai_allowed" in prep                       # بوابة السقف/الحجب
    assert "save_analysis(" in prep                   # تخزين النثر على السجل
    assert "_client_missing_narrative_heads" in prep  # لا حجز بلا حاجة فعلية

    # والبوّابةُ نفسُها تُقرَّر مرّةً واحدة لا مرّتين.
    op = _src("api.py").split("def _block_client_export_if_gate_failed(")[1] \
        .split("\n    def ")[0]
    assert "_free_ai_extras_allowed()" in op
    assert "prepare_fallback_prose(" in op
    factory = _src("silk_platform/api.py").split(
        "def _client_view_gated(")[1].split("\n    @app")[0]
    assert "free_ai_extras_allowed()" in factory
    assert "prepare_fallback_prose(" in factory
...
def test_malformed_bracket_limit_env_does_not_crash(monkeypatch):
    ...
    # تدقيق 2026-08-27: البوّابةُ صارت تقيس الهندسة (مواضع محارف الأقواس على
    # المحور x) لا سلسلةَ المحارف المستخرَجة — الصفُّ أدناه عربيُّ الأغلبية
    # وترتيبُ أقواسه `()`، أي انقلابٌ حقيقيّ.
    class _FakePage:
        def get_text(self, kind=None):
            return {"blocks": [{"lines": [{"spans": [{
                "origin": (0.0, 100.0),
                "chars": [{"bbox": (x, 0, x + 5, 10), "c": c} for x, c in
                          ((10.0, "("), (20.0, "م"), (30.0, "ر"), (40.0, ")"))],
            }]}]}]}
    ...
    # لا ValueError خام — الافتراضي (صفر) يسري فيُرفَض المستند المقلوب بوضوح.
