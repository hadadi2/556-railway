...
def test_the_platform_prefix_leads_to_the_page_not_a_bare_404(monkeypatch):
    """`/platform` كان 404 خالصاً — والآن صفحة هبوط حقيقية بزرّ الدخول.

    تدرّج عقدٍ معلن (لا إسكات): البلاغ الأصلي أُغلق بتحويل 307 إلى البوابة؛
    ثم طلب المالك (2026-08-17) «لاند بيج احترافية وفيها زر الدخول إلى
    البورتال» — فصار العقد الأقوى: البادئة تخدم صفحة هبوط 200 يقود زرُّها إلى
    `/platform.html`. جوهر البلاغ محفوظ في الحالين: لا 404 عارٍ أبداً.
    """
    setup_env(monkeypatch)
    _clear_gate(monkeypatch)
    cl = client()
    ...
    assert 'href="/platform.html"' in r.text
