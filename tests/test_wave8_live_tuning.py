...
def test_research_docx_leads_with_exec_summary_and_synthesis_verdict(
    ...
    # النافذةُ محدودةٌ **بالعنوان التالي** لا برقمٍ ثابت. كانت 600 حرفٍ
    # مضبوطةً يدوياً، فكلُّ إضافةٍ مشروعةٍ إلى القسم تدفع سطرَ القراءة
    # الاستشارية خارجَها فيفشل الاختبارُ فشلاً كاذباً (حدث مرّتين: توسيعُ
    # تسمية الحكم في PR A، ثمّ قسمُ فجوات الاقتصاد في الموجة B — البند E-03).
    # الحدُّ البنيويّ يقيس ما يقصده الاختبار فعلاً: **قسم الخلاصة التنفيذية**.
    _next_head = heads[toc_idx + 2] if len(heads) > toc_idx + 2 else ""
    _end = text.find(_next_head, exec_idx + 1) if _next_head else -1
    exec_section = text[exec_idx:_end if _end > exec_idx else exec_idx + 600]
    # قسمُ الخلاصة يحمل **الحكم المعتمد** مُعرَّباً بالكامل.
    assert "الحكم:" in exec_section and "توصية أولية بالدخول" in exec_section

    # وسطرُ القراءة الاستشارية يعيش في «قسم البحث العميق» لا في الخلاصة —
    # كانت نافذةُ الـ600 حرفٍ تلتقطه **بالمصادفة** لأنها تتخطّى حدَّ القسم،
    # فارتبط الاختبارُ بطولِ نصٍّ لا ببنية. يُفحَص الآن حيث يعيش فعلاً، وشرطُ
    # «لا رمزَ آلةٍ خام على وجه العميل» يُفحَص على **المستند كلِّه** — وهو
    # أقوى: رمزٌ مسرَّبٌ في أيّ قسمٍ عيبٌ لا في الخلاصة وحدها.
