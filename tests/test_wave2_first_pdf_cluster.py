...
def test_visual_pdf_lock_no_reversed_glyph_tokens():
    ...
    **الحالة: قفلٌ حيّ مرّةً أخرى (تدقيق 2026-08-27، المرحلة الثانية).**

    مرّ هذا الاختبارُ بثلاث حالات، وكلُّ انتقالٍ بينها كان بقياسٍ لا برأي:

    1. `xfail` ⇐ رُفع 2026-08-25 حين ثبت XPASS (بيئةٌ بخطّ Amiri).
    2. أُعيد `xfail(strict=True)` حين بدّل §7 الخطَّ إلى IBM Plex Sans Arabic
       فعادت الرموزُ — بعقدٍ مكتوبٍ ينصّ على رفعه فورَ زوال السبب.
    3. **رُفع نهائيًّا** حين زال السببُ فعلًا: `silk_pdf_textlayer`.

    والسببُ لم يكن الخطَّ أصلًا. خرائطُ `ToUnicode` **سليمةٌ ومنطقية**
    (`ير` `شر` `لأ`)؛ المستخرِجُ هو من يعكس السلسلةَ كاملةً فيقلب داخلَ كلّ
    عنقودِ رباط. Plex يحمل رباطاتٍ أكثر فيكثر الأثرُ، لا أكثر — ورباطُ
    لام-ألف يقع في **كلّ** خطٍّ مقيس (Amiri وNoto وPlex). فتبديلُ الخطّ كان
    سيعالج بعضَ العائلة ويترك أكثرَها شيوعًا، ويغيّر قرارَ مالكٍ بلا داعٍ.

    العلاجُ يعكس العناقيدَ مسبقًا في الخريطة فيُلغي عكسَ المستخرِج. القياسُ
    على تقرير العميل: **١٥ رمزًا مقلوبًا ⇐ صفر** في المستخرِجَين معًا،
    و**٥/٥ صفحاتٍ متطابقةٌ بكسليًّا** — التصييرُ لم يُمَسّ، والخطُّ لم يُمَسّ.

    توكيدُ `has_arabic_font()` أدناه يُبقي فشلَ البيئة الفونتية فشلًا مسمّى
    قابلًا للفعل لا انقلابًا غامضًا."""
    import sys as _sys
...
# ── تدقيق 2026-08-27 (W-01/W-04): معايرةُ بوّابةِ اتجاه الأقواس ──────────────

def test_client_docx_source_keeps_arabic_character_order():
    """ما **نتحكّم** فيه فعلاً: المصدرُ (docx) قبل تسليمه لـLibreOffice.

    قفلٌ هرمتيّ (لا soffice): انقلابُ الترتيب في الـPDF أثرُ استخراجٍ لا عطلُ
    منتَج — لكنّ انقلاباً في **مصدرنا** عطلٌ حقيقيّ. لو تسرّبت يوماً معالجةٌ
    تقلب المحارف (تشكيلٌ يدويّ، عكسُ نصّ، «إصلاحُ» bidi في طبقتنا) لالتقطها
    هذا الحارس فوراً — وهو الموضعُ الصحيح للحكم."""
    import zipfile
    p = os.path.join(tempfile.mkdtemp(), "r.docx")
    silk_reports.render_client_docx(_view(), p)
    xml = "".join(
        zipfile.ZipFile(p).read(n).decode("utf-8")
        for n in zipfile.ZipFile(p).namelist()
        if n.startswith("word/") and n.endswith(".xml"))
    for token in ("غري", "مؤرش", "أكرب", "مرسد", "المبارش", "رشط"):
        assert token not in xml, f"محرفٌ مقلوبٌ في **مصدر** الوورد: «{token}»"
    assert "غير" in xml, "الكلمةُ الهدفُ غائبةٌ — تحقّق من محتوى العيّنة"


@pytest.mark.skipif(not _pdf_tools(),
                    reason="soffice/pdftotext غير متاح (يعمل في e2e-live-shape)")
def test_production_pdf_has_zero_geometrically_inverted_brackets():
    """العقدُ الإنتاجيّ: صفرُ زوجِ أقواسٍ مقلوبٍ هندسياً في تقرير العميل.

    هذا هو التوكيدُ الذي **كان مستحيلاً** على المقياس القديم: كان يعدّ ٧ على
    هذا المستند نفسِه (سليمِ التصيير) فيرفضه بعتبة ٣."""
    pdf = os.path.join(tempfile.mkdtemp(), "client.pdf")
    silk_reports.render_client_pdf(_view(), pdf)      # البوّابةُ تعمل داخله
    measured = silk_reports.bracket_orientation_counts(pdf)
    assert measured is not None, "لم يقع قياسٌ أصلاً (pymupdf غائبة) — لا يُقرأ نجاحاً"
    ok, inverted, _ = measured
    assert inverted == 0, f"{inverted} زوجَ أقواسٍ مقلوب في تقرير العميل"
    assert ok > 0, "لم يُقَس أيُّ زوجِ أقواسٍ — المقياسُ عمياء لا سليمة"


@pytest.mark.skipif(not _pdf_tools(),
                    reason="soffice/pdftotext غير متاح (يعمل في e2e-live-shape)")
def test_bracket_gate_still_catches_a_genuinely_inverted_build():
    """الوجهُ الآخرُ للمعايرة — بلا هذا التوكيد يمرّ حارسٌ يقول «سليم» دائماً.

    العيّنةُ نفسُها بلا `<w:bidi/>` (أساسُ الفقرة LTR) تُصيَّر أقواسُها `()`
    فعلاً؛ المقياسُ يلتقطها، والمقياسُ القديم كان **يقبلها** (٠ مشبوهة)."""
    import sys as _sys
    tools_dir = os.path.join(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))), "tools")
    if tools_dir not in _sys.path:
        _sys.path.insert(0, tools_dir)
    import rtl_calibration
    docx = rtl_calibration.build_bracket_fixture(isolate=False, bidi=False)
    pdf = os.path.join(os.path.dirname(docx), "inverted.pdf")
    # تجاوُزُ البوّابة عمداً: نريد قياسَ العيّنة لا رفعَ استثنائها.
    silk_reports.docx_to_pdf(docx, pdf, lang="en")
    measured = silk_reports.bracket_orientation_counts(pdf)
    assert measured is not None, "لم يقع قياسٌ أصلاً (pymupdf غائبة)"
    assert measured[1] > 0, "الحارسُ لا يلتقط بناءً مقلوباً فعلاً — معايرةٌ ميتة"


@pytest.mark.skipif(not _pdf_tools(),
                    reason="soffice/pdftotext غير متاح (يعمل في e2e-live-shape)")
def test_client_pdf_text_layer_is_readable_by_both_extractors():
    """القفلُ الحيّ لطبقة النصّ: ما ينسخه العميلُ ويبحث فيه (Ctrl+F).

    المسارُ إنتاجيّ (`render_client_pdf`)، والتوكيدُ على **المستخرِجَين
    معًا** — MuPDF وpoppler — لأنّ العطلَ ظهر فيهما معًا، فإصلاحٌ يُرضي
    أحدَهما ليس إصلاحًا. القياسُ قبل التطبيع كان: ١٥ رمزًا مقلوبًا وصفرَ
    سليم؛ بعده: صفرُ مقلوبٍ و١٥ سليمًا."""
    pdf = os.path.join(tempfile.mkdtemp(), "client.pdf")
    silk_reports.render_client_pdf(_view(), pdf)
    import fitz
    with fitz.open(pdf) as doc:
        by_fitz = "".join(p.get_text() for p in doc).replace("\n", "")
    by_poppler = subprocess.run(
        ["pdftotext", "-enc", "UTF-8", pdf, "-"],
        capture_output=True, timeout=60).stdout.decode(
            "utf-8", "replace").replace("\n", "")
    for name, txt in (("fitz", by_fitz), ("pdftotext", by_poppler)):
        for reversed_token in ("غري", "مؤرش", "أكرب", "مرسد", "المبارش",
                               "موىص", "رشط", "الرتكز"):
            assert reversed_token not in txt, (
                f"محرفٌ مقلوبٌ في طبقة نصّ الـPDF عبر {name}: «{reversed_token}»")
        assert "غير" in txt, f"الكلمةُ الهدفُ غائبةٌ عبر {name}"


@pytest.mark.skipif(not _pdf_tools(),
                    reason="soffice/pdftotext غير متاح (يعمل في e2e-live-shape)")
def test_text_layer_normalization_never_changes_the_rendered_page():
    """**التوكيدُ الحاسم:** التطبيعُ يمسّ خريطةَ النصّ لا التصيير.

    لولا هذا القفلُ لكان «إصلاحُ» الاستخراج قادرًا على إفساد الورقة نفسِها
    بصمت — وهي المُسلَّم الذي يراه العميل. مقيسٌ بكسليًّا: ٥/٥ صفحاتٍ
    متطابقة، صفرُ فرق."""
    import fitz
    import silk_pdf_textlayer
    docx = os.path.join(tempfile.mkdtemp(), "r.docx")
    silk_reports.render_client_docx(_view(), docx)
    raw = os.path.join(os.path.dirname(docx), "raw.pdf")
    # مسارُ التحويل نفسُه، لكن بلغةٍ إنجليزية كي لا يُطبَّع تلقائيًّا.
    silk_reports.docx_to_pdf(docx, raw, lang="en")
    normalized = raw.replace("raw.pdf", "norm.pdf")
    import shutil as _sh
    _sh.copyfile(raw, normalized)
    assert silk_pdf_textlayer.normalize_arabic_text_layer(normalized) is True

    with fitz.open(raw) as a, fitz.open(normalized) as b:
        assert a.page_count == b.page_count
        for i in range(a.page_count):
            pa = a[i].get_pixmap(dpi=150).samples
            pb = b[i].get_pixmap(dpi=150).samples
            assert pa == pb, f"التصييرُ تغيّر في الصفحة {i} — التطبيع يمسّ الورقة"
