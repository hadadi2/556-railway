...
def test_client_docx_replaces_jargon_with_its_meaning():
    """**تغيّر عقد مقصود (قرار المالك 2026-08-19):** كان العقد السابق يفرض
    ظهور المصطلح **مع شرحه بين قوسين** عند أوّل ورود؛ فأنتج ذلك جملاً
    مكسورة في تقرير المالك («(HHI (مؤشر يقيس تركّز السوق…)≈2350)»). العقد
    الجديد: على **سطح العميل** يُستبدَل المصطلح بمعناه ولا يُشرح بين
    قوسين. نسخة المدقّق (md) تحتفظ بالمصطلح وشرحه — الاختبار أعلاه يحرسها،
    فالسطحان متمايزان عمداً."""
    import silk_render
    ...
    for t in _TERMS:
        if t not in PLAIN_TERMS:
            continue      # أسماء أنظمة حقيقية تبقى (TRACES/CHED/EORI)
        assert t not in blob, f"{t} وصل مستند العميل عارياً — لغة التاجر"
