...
def test_growth_span_adversarial_variants():
    ...
    # تحديث قفل معلن (موجة #14 — الدرس 174): نطاق الكابح صار الفقرة لا
    # الجملة. سردُ سلسلةٍ يمتد جُملاً («ذروة 2019. واستقرار 2023.») مسارٌ
    # حي واحد — وسمُ 2019 «الأحدث المتاح» فيه دلالة مقلوبة، فيُكبَح.
    v3 = "بلغ الحجم ذروته في 2019. وتُظهر بيانات 2023 استقراراً."
    assert "الأحدث المتاح" not in _tag_stale_years(v3, {2019})
    # والحدّ الفقري (\n) يحفظ الوسم للحقيقة القديمة المستقلة في فقرتها.
    v4 = "بلغ الحجم ذروته في 2019.\nوتُظهر بيانات 2023 استقراراً."
    assert "الأحدث المتاح" in _tag_stale_years(v4, {2019})
