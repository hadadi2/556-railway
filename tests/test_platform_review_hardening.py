...
from platform_helpers import (client, hdr, login, make_factory,
                              make_product_study, mock_engine, seed, setup_env)
from silk_platform import db as pdb, jobs, passwords, quota, wallet
...
def test_launch_claim_is_atomic_under_threads(monkeypatch):
    ...
    seed(monkeypatch)
    f = make_factory("gold", "atomic-launch@f.local")
...
def test_quota_denied_launch_reverts_study_to_draft(monkeypatch):
    ...
    seed(monkeypatch)
    f = make_factory("basic", "revert@f.local")
    eb = mock_engine(monkeypatch)
...
def test_relaunch_of_launched_study_is_409_and_quota_counted_once(monkeypatch):
    seed(monkeypatch)
    f = make_factory("gold", "relaunch@f.local")
    eb = mock_engine(monkeypatch)
    sid = make_product_study(f["account_id"], f["user_id"])
    cl = client()
    tok = login(cl, f["email"], f["password"])
...
def test_input_validation_returns_422_not_500(monkeypatch):
    ...
    cl = client()
    tok = login(cl, f["email"], f["password"])
    # R6 (FE-8): حقولُ التنقيب الميتة (`target_count`/`title_en`) تُتجاهَل الآن لا تُرفَض —
    # مجسُّ «4xx لا 500» صار رمزَ HS غيرَ الصالح عند الإنشاء والتعديل.
    assert cl.post("/platform/studies", headers=hdr(tok),
                   json={"title_ar": "x", "hs_code": "12"}).status_code == 422
    study = cl.post("/platform/studies", headers=hdr(tok),
                    json={"title_ar": "ok"}).json()
    assert cl.patch(f"/platform/studies/{study['id']}", headers=hdr(tok),
                    json={"hs_code": "abc"}).status_code == 422
    # سوق مستهدف ليس رمز iso3 ورمز HS غير رقمي — حقول التحوّل الجديدة.
    assert cl.patch(f"/platform/studies/{study['id']}", headers=hdr(tok),
                    json={"market_pref": "دبي"}).status_code == 422
    assert cl.patch(f"/platform/studies/{study['id']}", headers=hdr(tok),
                    json={"hs_code": "xyz"}).status_code == 422
...
def test_prospecting_tables_not_exposed_through_generic_repository():
    """جداول التنقيب اليتيمة (قرار المالك 2026-08-17) بلا أي مسار كتابة CRUD."""
    from silk_platform import repository
    for table in ("comparison_funnels", "prospects", "drafts", "smtp_configs",
                  "email_queue"):
        assert table not in repository._WRITABLE
    for factory in ("funnels", "prospects", "drafts", "smtp_configs"):
        assert not hasattr(repository, factory)
