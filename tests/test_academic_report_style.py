...
def test_committed_sample_narrative_is_academic_register(tmp_path):
    """قفل مقلوب عمداً (البند 22 من أمر إصلاح المحرّك، 2026-08-24 — يعدّل
    قرار 2026-07-22/D-23 الأسلوبي): جملة الدلالة تقف بذاتها؛ الافتتاحية
    القالبية «دلالة هذه النتيجة:» صارت محظورة لا مطلوبة."""
    doc = _build_doc(tmp_path)
    text = "\n".join(p.text for p in doc.paragraphs)
    assert "تخلص الدراسة إلى" in text
    assert "دلالة هذه النتيجة:" not in text
