...
def _listings(*prices: float) -> list[DataPoint]:
    return [DataPoint({"title": f"item{i}", "price": p, "currency": "USD", "pack_size": "1 kg"},
                      "Local retail", 0.8, "listing", _today())
            for i, p in enumerate(prices)]


def test_band_derived_from_observed_quartiles():
    out = suggest_price(_listings(10, 12, 14, 16, 20), currency="USD", pack_size="1 kg")
    b = out["basis"]
...
def test_cost_floor_raises_band_and_reports_margin():
    ...
    out = suggest_price(_listings(10, 12, 14, 16, 20),
                        cost_per_unit=10, tariff_pct=20, shipping_per_unit=1, currency="USD", pack_size="1 kg")
...
def test_floor_above_all_observed_prices_declares_no_band():
    out = suggest_price(_listings(10, 12, 14), cost_per_unit=30, tariff_pct=0, shipping_per_unit=0, currency="USD", pack_size="1 kg")
