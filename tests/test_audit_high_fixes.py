...
def test_usd_ledger_includes_all_escalation_attempts():
    ...
    class _EscalatingProvider:
        ...
        def complete(self, system, user, max_tokens, model, timeout):
            ...
            # بلاغ تحليل 20: حارسُ العملية العقيمة يوقف أيّ إكمالٍ لا يزيد تغطية
            # الأقسام. فكل نداءٍ هنا **يزيد التغطية** كي تُنفَّذ المحاولات الثلاث
            # وتُقاس رموزُها جميعاً (قصدُ الاختبار)؛ نداءان مقتطعان (max_tokens)
            # ثم إكمالٌ يختم الأحد عشر — لا إعادةَ توليد، إكمالٌ من الذيل.
            from silk_ai_judge import report_sections
            secs = report_sections("ar")
            if self.calls == 1:                               # مسوّدة: الأقسام ١-٤
                silk_llm_provider._last_stop_reason.set("max_tokens")
                return "\n".join(f"## {i}. {secs[i-1]}\nفقرة." for i in range(1, 5))
            if self.calls == 2:                               # إكمال ١: يضيف ٥-٨
                silk_llm_provider._last_stop_reason.set("max_tokens")
                return "\n".join(f"## {i}. {secs[i-1]}\nفقرة." for i in range(5, 9))
            silk_llm_provider._last_stop_reason.set("end_turn")  # إكمال ٢: يضيف ٩-١١
            return "\n".join(f"## {i}. {secs[i-1]}\nفقرة." for i in range(9, 12))
    ...
    assert fake.calls == 3                                    # مسوّدة + إكمالان — ٣ نداءات مقيسة
