...
def test_mission_declared_gaps_are_a_fourth_input():
    ...
    # قفل محدَّث معلن (#13 الدرس 171-البند 4): التأطير سطرُ تمهيدٍ واحد في
    # القسم لا ذيلٌ مخبوز في كل بند — البنود تفاصيل صرفة.
    assert all("لا تمنع القرار الحالي" not in g for g in informational)
...
def test_positive_sentence_only_when_all_four_lists_empty(tmp_path):
    ...
    assert "يقيّد يقينه" in t   # D4: صيغة «معطى ناقص واحد … يقيّد يقينه»
    assert "الحوكمة" in t
