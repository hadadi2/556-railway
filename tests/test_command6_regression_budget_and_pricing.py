...
def test_writer_first_attempt_budget_exceeds_measured_full_narrative():
    ...
    # تحليل 20: كان الشرط ٢×الأساس، لكن رفعَ الأساسِ إلى 24000 يجعل ٢× = 48000؛
    # المنشور sonnet-5 يحتمله، لكنّ بديل Opus حدُّه 32000 فيردّ 400. المطلوب هامشٌ
    # حقيقيّ فوق الأساس لا ضِعفُه — والسقف 32000 يمنح المسوّدةَ 24000 تصعيداً كافياً.
    assert aj._MAX_TOKENS_CEILING > aj._WRITER_MAX_TOKENS
    assert aj._MAX_TOKENS_CEILING - aj._WRITER_MAX_TOKENS >= 4000  # هامش معنويّ
...
def test_writer_continuation_call_uses_the_ceiling_not_the_base_budget():
    ...
    def fake_post(url, **kw):
        ...
        # الموجة ٨ (كاش بادئة الكاتب): المحتوى قد يصل قائمة كتل — يُطبَّع
        # نصاً لنفس التمييز (تحديث قفل معلن؛ النية كما هي: الإكمال بالسقف).
...
def test_smart_model_default_is_sonnet_and_priced():
    """قرار مالك 2026-08-21: النموذج الذكي الافتراضي Sonnet بدل Opus.
    قفلان معاً: (١) الافتراضي في المصدر عائلة `claude-sonnet-5` (فحص نصّي —
    الوحدة قد تُستورَد وبيئة الاختبار تحمل SILK_AI_MODEL)، (٢) العائلة مُسعَّرة
    في الدفتر بسعر Sonnet القياسي — وإلّا انكسرت بوابة السقف اليومي (M-19)."""
    import os
    from silk_pricing import _pricing_for
    src_path = os.path.join(os.path.dirname(__file__), "..", "silk_ai_judge.py")
    with open(src_path, encoding="utf-8") as fh:
        src = fh.read()
    assert '_MODEL = os.environ.get("SILK_AI_MODEL", "claude-sonnet-5")' in src
    price = _pricing_for("claude-sonnet-5")
    assert price == {"input": 2.00, "output": 10.00}
    # المعرّفات المؤرَّخة تسقط على نفس أسرة السعر
    assert _pricing_for("claude-sonnet-5-20260401") == price
