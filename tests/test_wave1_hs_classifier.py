...
def test_research_hard_gate_422_on_empty_hs6_no_reservation():
    ...
    # الدرس ٢١٠: الرفضُ صار يسمّي سببَه بدقّة — والمحميُّ هو **صفرُ إنفاق**
    # على رمزٍ مجهول، لا تهجئةُ رمز الخطأ.
...
def test_research_flag_off_tolerates_empty_hs6_todays_way():
    ...
    كان `SILK_REQUIRE_HS6` مُطفأً افتراضاً، فيمرّ منتجٌ لم يُحلّ رمزُه إلى
    بوّابة الجهوزية بـ`hs_code=None`. أمرُ الموجة صريح: «لا تقبل تصنيفاً غير
    محسومٍ بصمت». صار الحجبُ افتراضاً، والصمّامُ لم يعد يفتح البابَ.
    """
    pytest.importorskip("fastapi")
