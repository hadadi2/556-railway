...
def test_uncited_finding_is_dropped_and_logged(caplog):
    ...
    # **الموجة B (البند T-02) — تشديدٌ لا إضعاف.** الشبكةُ محجوبةٌ في هذا
    # المقعد، فنقطةُ `dp1` المسجَّلة قيمتُها `None` (فشلُ Comtrade مرصودٌ
    # ومُعلَن بحكم العقد). الادّعاءُ المستشهِد بها **مبنيٌّ على استعلامٍ
    # فشل**، وكان يُشحَن بنداً «مستشهَداً» يرث اسمَ مصدرِها فيبدو مسنوداً.
    # الآن مصيرُه **فجوةٌ معلنة** لا بند — وهذا هو الصواب الذي كان الاختبارُ
    # القديم يقفل عكسَه.
    assert report.findings == []
    # الفجواتُ تصل عبر ملخّص التقرير (`AgentReport` بلا حقل gaps مستقلّ).
    assert "مستشهد بشكل صحيح" in report.summary, (
        "ادّعاءٌ استشهد بنقطةٍ بلا قيمة يجب أن يصير فجوةً معلنة لا إسقاطاً صامتاً")
    assert "أُسقطت" in report.summary
    dropped_warnings = [r for r in caplog.records if "dropped" in r.message]
    assert len(dropped_warnings) == 3  # dp999 مجهول + بلا استشهاد + فجوة مستشهَدة


def test_a_claim_citing_a_valued_datapoint_survives():
    """النصفُ الإيجابيّ للعقد: استشهادٌ بنقطةٍ **ذاتِ قيمة** يمرّ كما كان.

    يُفحَص على `_parse_output` مباشرةً بسجلٍّ مبذورٍ بقيمةٍ حقيقية، لأنّ مقعدَ
    الوكيل أعلاه يعمل بشبكةٍ محجوبة فكلُّ نقطةٍ فيه فجوةٌ معلنة بالضرورة.
    """
    import silk_llm_runtime as rt
    from silk_data_layer import DataPoint
    registry = {
        "dp1": DataPoint(1234.0, "UN Comtrade", 0.8, "واردات مرصودة",
                         "2026-07-23"),
        "gap1": DataPoint(None, "FAOSTAT", 0.0, "401 — فجوة معلنة",
                          "2026-07-23"),
    }
    payload = json.dumps({"findings": [
        {"claim": "مستشهد بقيمة", "datapoint_ids": ["dp1"], "confidence": 0.8},
        {"claim": "مستشهد بفجوة", "datapoint_ids": ["gap1"], "confidence": 0.9},
    ], "gaps": [], "summary": "س"}, ensure_ascii=False)
    out = rt._parse_output(payload, registry)
    assert [f["claim"] for f in out["findings"]] == ["مستشهد بقيمة"]
    assert any("مستشهد بفجوة" in g for g in out["gaps"])
    reasons = " ".join(d["reason"] for d in out["dropped"])
    assert "declared gaps" in reasons
