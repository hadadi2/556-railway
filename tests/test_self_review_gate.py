...
# ── R7 (تدقيق 2026-09-01، CI-7): القالبُ الفارغ لا يمرّ ─────────────────────────
def test_rejects_a_body_that_still_carries_the_template_placeholders():
    """متنٌ يحمل عنوانَ القسم وإعلانَ high لكنّه ما زال يحمل تعليقاتِ القالب
    `<!-- … -->` — قالبٌ لم يُملأ كان يمرّ (البوّابة تبحث عن الكلمات لا عن المحتوى)."""
    body = ("## المراجعة الذاتية · self-review (§58)\n\n"
            "<!-- اكتب هنا ملخّص المراجعة -->\n"
            "no high-severity findings\n")
    problems = gate.check(body)
    assert problems and any("template" in p.lower() for p in problems)


def test_accepts_a_filled_body_without_placeholders():
    body = ("## المراجعة الذاتية · self-review (§58)\n\n"
            "- شغّلت /code-review على الفرق: ملاحظتان مُصلَحتان، لا عيوب high.\n")
    assert gate.check(body) == []
