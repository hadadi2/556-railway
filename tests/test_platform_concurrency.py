...
from silk_platform import (billing, db as pdb, entitlements,
                           lifecycle, quota, wallet)
...
def test_concurrent_archives_only_one_wins(monkeypatch):
    """أرشفتان متزامنتان ⇒ واحدة تنجح والأخرى تُرفَض (القفل الفوري)."""
    seed(monkeypatch)
