...
_TEST_ANCHORS = [
    ...
    # ٢٨٤ — حارسُ الأقواس يقرن بترتيب القراءة، وكلُّ عطلِ PDF برمزٍ يسمّيه.
    (284, "silk_reports.py", ["def _bracket_segments", "_BRACKET_CARRY_ROWS",
                              "class PdfBracketGateError",
                              "def pdf_error_detail",
                              "def record_pdf_failure", "def _pair_segment"]),
    (284, "silk_platform/api.py", ["_sr.record_pdf_failure("]),
    (284, "tools/pdf_smoke.py", ["def main"]),
    (284, ".github/workflows/e2e-live-shape.yml",
     ["tools/pdf_smoke.py", "tests/e2e/platform_pdf_failure_codes.cjs"]),
    (284, "tests/e2e/platform_pdf_failure_codes.cjs", ["pdf_rejected"]),
    (284, "tests/test_pdf_bracket_pairing.py",
     ["def test_a_pair_split_across_table_cells_is_not_an_inverted_pair",
      "def test_orphan_halves_of_wrapped_lines_are_not_an_inverted_pair",
      "def test_a_genuinely_inverted_pair_is_still_caught",
      "def test_each_pdf_failure_has_its_own_code"]),
    (284, "tests/test_platform_pivot_bridge.py",
     ["def test_report_pdf_names_the_failure_and_leaves_a_trace"]),
