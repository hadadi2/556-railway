...
def test_cap_helper_wired_into_render_and_api_paths():
    ...
    # البند ٧ (تدقيق 2026-08-27): المسار الرئيسي انتقل حرفياً إلى
    # `silk_research_pipeline`، وregen بقي في `api` — والحقيقة المحروسة
    # هي «مُسقِّف واحد في الاثنين» لا «في ملفٍ واحد».
    from tests.api_source import api_layer
    assert api_layer().count("cap_confidence_for_flagged_hs") >= 2
