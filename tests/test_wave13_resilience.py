...
def test_mid_run_crash_then_resume_skips_completed_missions():
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test", "SILK_RATE_LIMIT": "0",
                                 "SILK_API_KEY": "secret"}), \
         ...
         patch("silk_missions._augment_competitors_structured"), \
         patch("silk_missions._augment_risk_news_fx"), \
...
def test_async_run_returns_immediately_then_status_reflects_progress():
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test", "SILK_RATE_LIMIT": "0",
        ...
        status = None
        # نافذة استطلاع سخيّة (كسرٌ مبكر عند الاكتمال): الخيط الخلفي يكمل بثوانٍ
        # حين تفشل نداءات البيانات فوراً (CI بلا proxy)، لكن بطء وسيطٍ يفشل ببطء
        # (403 من proxy الحاوية) كان يتجاوز نافذةً ضيّقة فيتقلّب الاختبار.
        for _ in range(2000):
