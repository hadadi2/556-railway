...
from silk_platform.engine_bridge import _HS_GATE_CODES
...
def test_w2_3_broaden_if_weak_reports_both_terms_framed():
    ...
    import pandas as pd
    with mock.patch.object(T, "_interest_with_series", return_value=pd.DataFrame({"زبدة الفول السوداني": [.4], "فوائد زبدة الفول السوداني": [100.]})), \
    ...
    assert "استعلام مشترك" in broadened.note and "ليس حجم طلب" in broadened.note


def test_w2_3_broaden_loop_capped_at_two_candidates(monkeypatch):
    ...
    from silk_data_layer import DataPoint
    calls = []
    monkeypatch.setenv("SILK_TRENDS_BROADEN_MAX", "2")

    def _fake_interest(kw, geo=None, tf="today 12-m", label=""):
        calls.append(kw)
        return pd.DataFrame({term: [.4] for term in kw})  # مقياس مشترك، كلا المصطلحين ضعيف
    ...
    with block_network(), mock.patch.object(T, "_interest_with_series", side_effect=_fake_interest), \
         mock.patch.object(T, "trends_context", side_effect=_fake_ctx):
    ...
    assert len(calls) == 2, calls            # يجب بلوغ الحد بمرشحين ضعيفين، لا نجاح بصفر نداءات
...
def test_w3_1_price_row_reason_classifier():
    """3.1 — مصنِّف سبب غياب السعر/كجم: الوزن غير متاح / وحدة غامضة / قابل
    للحساب (تحديث قفل معلَن — البند 16: مفردتا الغياب)."""
    import silk_render as R
...
def test_w6_2_writer_prompt_caps_exec_summary_and_requires_flip_and_risks():
    ...
    assert "تحت 150 كلمة" in u
    assert "رقمان أو ثلاثة يتبع كلَّ رقمٍ معناه" in u
...
def test_w1_2_research_gate_422_on_unconfirmed_hs_before_spend():
    ...
    **قبل** أيّ حجز، وتأكيد المستخدم (hs_confirmed) يتجاوزها."""
    ...
    # العائلةُ لا التهجئة (الدرس ٢١٠): الرفضُ يسمّي سببَه بدقّة، والسلوكُ
    # المحميّ هو الحجبُ قبل الإنفاق لا اسمُ رمز الخطأ.
    assert r.json()["detail"]["error"] in _HS_GATE_CODES
...
def test_w1_2_the_confirm_valve_no_longer_disables_classification():
    """**تغيّرُ سلوكٍ مقصود (موجة خطّ التصنيف، الدرس ٢١٠).**

    كان `SILK_HS_CONFIRM_GATE=0` مهرباً واعياً يُعطّل الحجب. بعد أن صار
    التحقّقُ الدلاليّ **جزءاً من التصنيف نفسه** لا بوّابةً فوقه، لم يعد
    لصمّامٍ بيئيّ أن يُلغيه: رمزٌ يخالف المنتجَ يبقى مخالفاً مهما ضُبِط في
    البيئة. أمرُ الموجة صريح: «لا تُعطّل بوّابة الثقة، ولا تتجاوز التأكيد
    الدلاليّ».

    والمهربُ لم يُحذَف بل **انتقل إلى صاحبه**: `hs_confirmed=true` — تأكيدُ
    إنسانٍ حاضر (LAW: المالك آخِر مؤكِّد، لا طريقٌ مسدود). الفرقُ أنّ المهربَ
    صار قراراً بشرياً مسجَّلاً (`classification_method="user_confirmed"`)
    لا متغيّرَ بيئةٍ صامتاً على مستوى الخادم كلّه.
    """
    pytest.importorskip("fastapi")
    pytest.importorskip("httpx")
    from fastapi.testclient import TestClient
    ...
    with mock.patch("requests.get",
        ...
        r = client.post("/research",
                        json={"product": "زبدة الفول السوداني",
                              "market": "Yemen", "hs_code": "040510",
                              "async_run": False, "persist": False})
    # الصمّامُ لم يعد يفتح البابَ: الرمزُ المخالف يُحجَب كما لو لم يُضبَط.
    assert r.status_code == 422
    assert r.json()["detail"]["error"] in _HS_GATE_CODES

    # والمهربُ الشرعيّ يعمل: تأكيدُ إنسانٍ حاضر يمرّ ويُسجَّل.
    with mock.patch("requests.get",
                    side_effect=OSError("net blocked for offline test")):
        r2 = client.post("/research",
                         json={"product": "زبدة الفول السوداني",
                               ...
                               "async_run": False, "persist": False})
    assert not (r2.status_code == 422
                and r2.json().get("detail", {}).get("error") in _HS_GATE_CODES)
...
def test_w2_hs_gate_blocks_on_both_analyze_and_research_by_default():
    ...
    for label, r in (("research", r_research), ("analyze", r_analyze)):
        ...
        assert r.json()["detail"]["error"] in _HS_GATE_CODES, label
