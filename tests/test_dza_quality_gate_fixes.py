...
def test_overall_verdict_moves_from_fail_to_pass_with_warnings():
    ...
    assert out["verdict"] != "FAIL"
    checks = {f["check"] for f in out["findings"]}
    # الموجة ٥ أضافت تحذير الجدول الزمني للنفاذ (§5.5) — WARN مقصود
    # بالتصميم على هذه المدوّنة (بعثة الاشتراطات نجحت بلا سطر المدة).
    # الموجة B (البند R-02): فحصٌ **تحذيريّ** جديد يرصد أنّ قسم الاشتراطات
    # لم يستند إلى المرجع التنظيميّ المُقنَّن. لا يمسّ الحكم (repairable)،
    # ومدوّنةُ DZA لا تحمل وسمَ المرجع فيُطلِق بحقّ.
    # الموجة C (E-01/E-09): فحصُ تطبيع الأسعار — قائمٌ منذ #209 — صار
    # **يُطلِق فعلاً** لأنّ فجوة مستوى السعر صارت تُعلَن بدل أن تُبتلَع
    # (كانت المرساةُ تتكوّن من أيّ رقمٍ فلا فجوةَ تُكتَب). تحذيريٌّ لا حاجب:
    # الحكمُ ما يزال ليس FAIL، والتقريرُ يُسلَّم بملاحظةٍ صحيحة.
    assert checks <= {"curated_reference_not_consulted",
                      # هدف الدراسة الاحترافية (البند ٤): فحصا الملخص
                      # التحذيريان — مدوّنة DZA سابقة للقالب فيُطلقان بحق.
                      "exec_summary_recommendation_first",
                      "exec_summary_constraints",
                      "markdown_artifacts", "style_repeated_key_figure",
                      "audit_coverage", "access_timeline_missing",
                      "price_comparability_gaps",
                      # الموجة C (C-07): فرقُ الثقة بين الحقل المُسجَّل ونصّ
                      # المدوّنة (0.66 مقابل «60%») — تحذيريّ، ويكشف تناقضاً
                      # حقيقياً في المدوّنة نفسها.
