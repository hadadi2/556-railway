...
def test_research_a2_advisory_422_until_explicit_consent():
    ...
    with _env(SILK_A2_PLAUSIBILITY="1", SILK_HS_CONFIRM_GATE="0",
        ...
        c = _client()
        # `hs_confirmed` صار **لازماً** لبلوغ هذه البوّابة بهذا الزوج: خطُّ
        # التصنيف الواحد (`silk_hs_pipeline`) يرصد أن 040510 (زبدة الألبان)
        # يخالف «زبدة الفول السوداني» ⇒ 200811، فيرتدّ `hs_catalog_conflict`
        # قبل أ٢. هذا **سلوكٌ مقصود** (حادثة التأسيس نفسها)، والتأكيدُ الصريح
        # هو المَخرج المشروع الذي يُبقي هذا الاختبارَ يقيس ما وُضِع ليقيسه.
        body = {"product": "زبدة الفول السوداني", "market": "UAE",
                "hs_code": "040510", "hs_confirmed": True}
