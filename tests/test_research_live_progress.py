...
def _full_ar() -> str:
    """مسوّدةٌ مكتملة (الأقسام الأحد عشر) تمرّ فحصَ `_writer_incomplete` — كي
    يُستدعى المراجع (الناقصةُ تتخطّاه عمداً، تحليل 20)."""
    from silk_ai_judge import report_sections
    return "\n\n".join(f"## {i}. {t}\nفقرة كاملة تنتهي بجملة سليمة."
                       for i, t in enumerate(report_sections("ar"), 1))
...
def test_write_reviewed_report_emits_writer_then_reviewer_in_order():
    ...
    # مسوّدةٌ **مكتملة** (الأقسام الأحد عشر) كي يمرّ فحصُ الاكتمال ويُستدعى
    # المراجع — الناقصةُ تتخطّى المراجع عمداً (تحليل 20)، فاختبارُ الترتيب
    # يحتاج مسوّدةً مكتملة.
    def fake_call(system, user, max_tokens=1600, model=None, timeout=None):
        return _full_ar()

    def fake_review(draft, mission_reports, trace_id=None, lang="ar"):
...
def test_write_reviewed_report_emits_reviewer_then_writer_on_revision_cycle():
    ...
    def fake_call(system, user, max_tokens=1600, model=None, timeout=None):
        calls["n"] += 1
        # مسوّدةٌ مكتملة في كل دورة (اختبارُ ترتيبِ التنقيح، لا الاكتمال)
        return _full_ar() + f"\n\nمسوّدة رقم {calls['n']}."
...
def test_status_endpoint_reflects_stage_elapsed_calls_and_cost_end_to_end():
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test",
        ...
        status = None
        # نافذة استطلاع سخيّة (كسرٌ مبكر) — تقلّبٌ توقيتيّ حين يفشل وسيطٌ ببطء؛
        # الاكتمال يكسر الحلقة فلا كلفة على المسار السريع (CI).
        for _ in range(3000):
