...
def _sectioned_report_text(extra: str = "") -> str:
    ...
    # الحشو متنوّع بعدد القسم عمداً — جملة واحدة مكرّرة حرفياً في كل قسم
    # صارت عيبَ توليدٍ يُفشِله `repeated_span` (البند 11، الدرس 131)،
    # وسؤال هذا الملف غيره.
    # الدرس ٢٣٨ (الصنف ٥): الحشوُ القالبيّ الواحد بتبديل الرقم والاسم صار
    # **إعادةَ صياغةٍ عبر الأقسام** يُطلِق عليها
    # `cross_section_near_duplicate` — نفسُ ما حدث مع `repeated_span` أعلاه
    # حرفياً. الحشوُ يتنوّع فعلاً في بنيته لا في كلمتين منه.
    # ١١ صيغةً **متمايزةً بنيوياً** لا خمساً تتناوب: ١١ قسماً و٥ صيغٍ
    # يعني قسمَين بالصيغة نفسها، وهو بعينه ما تُطلِق عليه القاعدة (مقيسٌ:
    # «ديناميكيات السوق» و«تقييم المخاطر» تشابهاً 67%).
    _SHAPES = (
        "لم تُبدّل حدودُ {s} اتجاهَ القرار في هذه المرحلة.",
        "يُقرأ {s} قبل أيّ التزامٍ تشغيليّ أو شحنيّ جديد.",
        "أثرُ {s} محدودٌ والمتابعةُ الربعيةُ تكفيه.",
        "أغلق {s} سؤالَه دون الحاجة إلى مصدرٍ إضافيّ.",
        "تنوّعت مؤشراتُ {s} بلا إشارةٍ حاسمةٍ إلى انقلاب.",
        "سجّل {s} استقراراً يُغني عن إجراءٍ عاجل.",
        "ظهر في {s} نقصٌ لا يمنع المضيّ في هذه المرحلة.",
        "قيسَ {s} على مصدرٍ واحدٍ منشورٍ يمكن فتحُه.",
        "يحتاج {s} خطوةً ميدانيةً واحدةً لاستكماله.",
        "انتهى {s} إلى ما يوافق بقيةَ التقرير بلا تعارض.",
        "بقي {s} خارج مسار المخاطر الحرجة هذا الربع.",
    )
    body = "\n\n".join(
        f"## {i}. {s}\n" + _SHAPES[(i - 1) % len(_SHAPES)].format(s=s)
...
def test_quality_gate_stays_warn_for_ordinary_repairable_findings():
    ...
    # الموجة B (البند T-01): بعثةٌ واحدةٌ بمؤشّرٍ مرصودٍ ومصدرٍ عموميّ — عيّنةٌ
    # بلا أيّ دليلٍ صارت تُفشِل البوّابة عمداً (تقريرٌ سرديٌّ بلا أساسٍ مُسنَد
    # لا يُسلَّم). البذرةُ تُبقي هذا الاختبارَ على سؤاله وحدَه: هل يُصعَّد
    # القابلُ للإصلاح إلى FAIL أم يبقى WARN.
    out = run_quality_gate({"deep_research": {
        "report": {"text": text}, "analyst": {},
        "missions": {"trade_flow": {"findings": [
            {"value": 8400000, "source": "UN Comtrade", "confidence": 0.8,
             "note": "واردات مرصودة 2023"}]}}}})
    checks = {f["check"] for f in out["findings"]}
    # Part B: العيّنة بلا قسم «أرقام القرار» — تحذيري متوقَّع بالتصميم
    # (تحديث قفل معلن)؛ سؤالُ الاختبار يبقى: القابل للإصلاح لا يُصعَّد FAIL.
    # + فحص افتتاح الملخص بالتوصية (هدف الدراسة الاحترافية، البند ٤) —
    # تحذيري بالتصميم على عيّنة قسمُها الأول عام (تحديث قفل معلن).
    assert checks == {"markdown_artifacts", "decision_numbers_present",
                      "exec_summary_recommendation_first"}
