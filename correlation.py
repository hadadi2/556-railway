...
def correlate(row: dict, product_card: dict, product_name: str = "") -> dict:
    ...
    for t in competitor_threads:
        ...
        try:
            ...
            continue
        gaps = [shipping_note]
        # المعادلة الواحدة القانونية (silk_economics.landed_cost — الموجة 2أ):
        # فُصلت إلى الوحدة الموحّدة وتُستورد هنا كي لا تتباعد نسختان.
        from silk_economics import landed_cost as _landed_cost
        if tariff_pct is None:
            gaps.append("الرسوم الجمركية غير مرصودة — الهامش أدناه محسوب "
                        "بلا احتساب جمارك")
        landed = _landed_cost(cost, shipping, tariff_pct)
        if obs.get("currency"):
