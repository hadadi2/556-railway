...
def economics_view(dr: dict, product_card: dict | None = None,
    ...
    if vat is None:
        ...
        try:
            ...
            vat = None
    if vat is None:
        # تقرير ٧ §4.3 (الدرس ٢٨٠): الضريبةُ غيرُ المرصودة فجوةٌ مسمّاة أيّاً
        # كانت التعرفة — والتعرفةُ الصفرية المرصودة لا تعني انعدامَ بقية الرسوم.
        gaps.append(("التعرفة الجمركية المرصودة 0%، وهذا لا يعني انعدام بقية "
                     "الرسوم: " if tariff is not None and float(tariff) == 0.0
                     else "")
                    + "ضريبة المبيعات أو الاستيراد لهذا الصنف في السوق غير "
                      "مرصودة — تحتاج تحققاً ولم تدخل الحساب")
