...
def _indicative(note: str, derivation: str) -> str:
    """ألصق وسمَ «تقدير استرشادي» باشتقاقه — البند T-14.

    `silk_source_coverage.tag_indicative_estimate` كان **معرَّفاً ولا مُنادىً**:
    المسارُ الشرعيّ الوحيد لعدّ رقمٍ مشتقٍّ بلا مصدرٍ مباشر «مسنوداً» ميّت. فإمّا
    يُحتسَب الرقمُ المنمذَج مسنوداً بلا حقّ، أو يُرفَض وهو مشروعٌ معلَنُ المعادلة.
    وسمُه هنا عند منتِجه يجعل قياسَ التغطية صادقاً في الاتجاهين.
    """
    try:
        from silk_source_coverage import tag_indicative_estimate
        return tag_indicative_estimate(note, derivation)
    except Exception:  # noqa: BLE001 — الوسمُ تحسينٌ لا شرطُ إنتاج
        return note
...
def _growth_window_pairs(hs: str, iso3: str, year: int,
                         tam: "float | None") -> "list[tuple[int, float | None]]":
    """أزواج (سنة، قيمة) لنافذة النمو الخمسية من المخزن — مع **القاعدة
    الدائمة** (#13، تصحيح المُشرِف): النمو المركّب يُحسب إلى أحدث سنة
    مرصودة. TAM سنة الدراسة مرصود حيّاً بينما المخزن قد لا يكون دُفئ بها
    بعد — فكانت النافذة تقف سنةً خلف أحدث رصدٍ في الدراسة نفسها
    (CAGR ‏2019–2023 وTAM ‏2024 على نفس الصفحة)؛ الرصد الحيّ يسدّها."""
    import silk_store
    ...
    for y in range(year - 4, year + 1):
        try:
            ...
            pairs.append((y, None))
    if tam is not None and pairs and pairs[-1][0] == year \
            and pairs[-1][1] is None:
        pairs[-1] = (year, float(tam))
    return pairs
...
class MarketSizeAgent(ResearchAgent):
    ...
    def _research(self, task):
        ...
        # النمو من مخزن الحقائق حصراً (لا توسيع نداءات حية هنا — الميزانية
        # للجامع)؛ وأحدث سنة مرصودة حيّاً تسدّ فجوة مخزنٍ بارد (القاعدة
        # الدائمة — انظر `_growth_window_pairs`).
        from silk_trend import cagr_pct, growth_pct
        pairs = _growth_window_pairs(hs, iso3, year, tam)
        ...
        if tam is not None and tier in _TIER_FACTORS:
            ...
            F.append(_f("sam_usd", round(tam * factor), [_src("UN Comtrade")],
                        ...
                        note=_indicative(
                            "مُقدَّر — نموذج بافتراضات معلنة، ليس رصداً",
                            f"SAM = TAM × {factor}")))
        else:
            gaps.append("sam_usd: يتطلب حجم سوق مرصوداً + تحديد شريحتك "
                        "السعرية (فاخرة/متوسطة/شعبية)")
        cap = card.get("monthly_capacity")
        # قيمة الحدود USD/kg؛ طاقة العبوات ليست كتلة ولا يجوز ضربهما.
        ...
        if capacity_kg and uv and tam is not None:
            ...
            F.append(_f("som_usd", som, [_src("UN Comtrade")], unit="USD",
                        ...
                        note=_indicative(
                            "مُقدَّر — سقف إيراد الطاقة الإنتاجية بأسعار "
                            "الحدود المرصودة",
                            "SOM = min(SAM أو TAM، الطاقة × 12 × قيمة الوحدة)")))
        else:
            gaps.append("som_usd: يتطلب طاقتك الإنتاجية الشهرية بوحدة كتلة معلومة + قيمة وحدة "
...
class CompetitorAgent(ResearchAgent):
    ...
    def _research(self, task):
        ...
        if comps:
            ...
            # الموجة 2ب: المقياس الموحّد 0–10000 (silk_economics.hhi) —
            # نهاية ازدواجية 0–1 مقابل عتبات العميل المعلنة (LESSONS 83).
            from silk_economics import hhi as _hhi_fn
            hhi = _hhi_fn(shares)
            ct = _src("UN Comtrade")
            F.append(_f("hhi", hhi, [ct],
                        note=f"مؤشر هيرفنداهل لتركّز المورّدين ({len(rows)} دولة) — "
                             ">2500 سوق مركّز (مقياس 0–10000)"))
...
class RiskAgent(ResearchAgent):
    ...
    def _research(self, task):
        ...
        if partners:
            ...
            # الموجة 2ب: نفس المقياس الموحّد 0–10000 عبر التحويل الصريح الوحيد.
            from silk_economics import hhi_from_fractions
            hhi = hhi_from_fractions(p["value_usd"] / grand for p in partners)
            F.append(_f("supplier_concentration_hhi", hhi, [_src("UN Comtrade")],
                        note="تركّز مصادر التوريد للسوق — >2500 اعتماد مركّز "
                             "(مقياس 0–10000؛ خطر انقطاع/حرب أسعار)"))
        else:
...
def muslim_share(iso3: str) -> dict | None:
    ...
    **المرجع الأساسي `demographics_l1.csv`** (٢٥٠ سوقاً، مبنيّ آلياً عبر
    `tools/fetch_demographics.py` من نفس أصل Pew) لا `muslim_share.csv` الأقدم
    (٤٤ صفاً ينقصه LBN/YEM/GHA من الأسواق المرتَّبة) — حقيقةٌ واحدة كانت تعيش
    في ملفين بقيم قابلة للانحراف، وهي عائلة القوائم المتشعّبة بعينها (الدرس
    ٦٣: عضوية EU المكرّرة أسقطت ١٢ دولة بصمت). الأقدم يبقى احتياطَ قراءةٍ
    عند غياب الأشمل فقط (فشل آمن مفتوح، لا انهيار).
    """
    global _muslim_cache
    if _muslim_cache is None:
        _muslim_cache = {}
        for fname, pct_col, year_col in (
                ("demographics_l1.csv", "muslim_pct", "muslim_pct_year"),
                ("muslim_share.csv", "muslim_share_pct", "ref_year")):
            # يُبنى في قاموس مؤقت ويُعتمَد **فقط عند اكتمال الملف كاملاً** —
            # فشلٌ منتصف التحليل كان يُبقي نصف الأشمل ثم يكدّس الأقدم فوقه
            # (هجين صامت من مصدرين — ملاحظة المراجعة الذاتية §58).
            parsed: dict = {}
            try:
                with open(os.path.join(_DATA_DIR, fname),
                          encoding="utf-8") as fh:
                    rows = [r for r in fh if not r.startswith("#")]
                for r in csv.DictReader(rows):
                    if not (r.get(pct_col) or "").strip():
                        continue   # صفّ بلا قيمة = فجوة تبقى فجوة
                    parsed[r["iso3"].strip().upper()] = {
                        "pct": float(r[pct_col]),
                        "ref_year": r[year_col], "note": r.get("note", "")}
            except Exception as e:  # noqa: BLE001 — مرجع غائب = فجوة، لا انهيار
                log.warning("muslim share reference %s unavailable: %s", fname, e)
                continue
            _muslim_cache = parsed
            break   # ملف واحد مكتمل — لا خلط قيم من ملفين أبداً
...
class LogisticsAgent(ResearchAgent):
    ...
    def _research(self, task):
        ...
        gaps.append("landed_cost_usd_kg: التكلفة حتى الوصول = سعر الحدود + الشحن؛ "
                    "تُحسب بعد رصد الشحن (الطبقة المدفوعة) وسعر المصنع المُدخَل")
        return F, gaps
...
class ResearchOrchestrator:
    ...
    def run_market(self, task: dict) -> dict:
        ...
        # R1 (تدقيق 2026-09-01، CONC-1): كل وكيلٍ في نسخةٍ مستقلة من سياق
        # المنادي — بوّابةُ منع نداء كلود (حجب الإضافات/لوحة الوكلاء) والعدّاد
        # تعمل داخل العامل لا في الخيط الأب وحده.
        import silk_context
        # EXT-5/EXT-6: انتظارٌ على جدارٍ مشترك (`cf_wait` FIRST_COMPLETED) — كان
        # `fut.result(timeout=)` بالتتابع يمنح كلَّ وكيلٍ ما تبقّى فيمتدّ السوقُ الواحد
        # حتى ثمانية أضعاف الجدار؛ والمجمّعُ لا ينتظر عمّالَه بعده.
        ex = _cf.ThreadPoolExecutor(max_workers=len(self.agents))
        try:
            futs = {silk_context.submit_with_context(ex, a.run, dict(task)): a
        ...
        outputs = {a.AGENT: outputs[a.AGENT] for a in self.agents if a.AGENT in outputs}
...
def _incumbent_is_self(supplier_rows) -> "bool | None":
    """موجة سدّ الفجوات (F1، البند 1): المهيمنُ سعوديّ ⇒ علمٌ مستقل على
    مسار /analyze أيضاً — كان المسارُ العميق وحده ينتجه فبقي None دوماً هنا.
    نفسُ عقد `silk_deep_pillars`: اسمٌ غير محلول («Unclassified…»/رقم) =
    هويةٌ غير مرصودة ⇒ None لا نفيٌ واثق؛ رمز كومتريد 682 حاسمٌ قبل الاسم."""
    rows = supplier_rows or []
    top = rows[0] if rows and isinstance(rows[0], dict) else {}
    p = str(top.get("partner") or "")
    if not p or p.lower().startswith("unclassified") or p.isdigit():
        return None
    from silk_deep_pillars import _INCUMBENT_SAUDI_TOKENS
    return (str(top.get("code")) == "682"
            or any(t in p.lower() for t in _INCUMBENT_SAUDI_TOKENS))
...
def _pillar_inputs(outputs: dict) -> dict:
    ...
    return {
        ...
        "competition_intensity": {
            ...
            "incumbent_is_self": _incumbent_is_self(
                mv("competitor", "supplier_countries"))},
        "regulatory_fit": {
