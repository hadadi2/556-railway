...
def _seed_codes() -> frozenset:
    # البذرة القديمة صراحةً لا افتراض المُحلِّل (صار `data/hscodes_full.csv`
    # المشتقّ من المرجع الرسمي نفسه): البنود الثلاثة عشر التي تعرفها البذرةُ
    # وحدها (عائلة اللائحة ٣٩ أدناه) تعيش هناك فقط.
    from silk_hs_resolver import load_hs_codes
    return frozenset(r.get("hs_code")
