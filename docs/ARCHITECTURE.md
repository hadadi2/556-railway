...
7. **الكاتب/المراجع**: `silk_ai_judge.write_reviewed_report()`
   ...
   (الكاتب، `_MODEL`/Sonnet — Opus سابقاً، قرار 2026-08-21، بنية ١١ قسماً إلزامية `_REPORT_SECTIONS`) و
...
Haiku $1/$5 — لاحقاً Sonnet $3/$15 مع قرار 2026-08-21، من توثيق Anthropic الرسمي)؛ `silk_context.record_llm_usage()`
...
### عقد التدقيق الجنائي 2026-09-08

`silk_request_identity` يعزل هوية الطلب: مفتاح POST /research محصور بهوية الخدمة وبصمة الحمولة؛ حجز SQLite يمنع السباق، وربط analysis_id في معاملة إنشاء التحليل نفسها. الطلب نفسه يعيد النتيجة أو معرّف الدراسة، وحمولة مختلفة 409. المفاتيح لا تنقضي تلقائياً: لا يُعاد إنفاق طلب قديم بصمت. طلب عالق قبل التخصيص يظل 409 حتى مراجعة المشغّل. `silk_evidence_contract` يحتفظ بلقطة المصدر داخل DataPoint.raw_evidence ويختبر الأرقام قبل قبول ادعاءات الأدوات. الوحدات/الفترات تبقى ضمن المصدر؛ هذا فحص رقمي وليس برهان استلزام دلالي شامل.

تغيير بعثة محفوظة يبطل opportunity_gaps ثم مراحل التحليل والتقرير التابعة، مع الحفاظ على البعثات السليمة. مقارنة السعر تتطلب تطابق العملة والتعبئة؛ تحويل FX أو العبوات غير الموثق يظل فجوة. SOM يتطلب طاقة بوحدة كتلة. محاولات HTTP لكلود تُحجز ذرّياً قبل الشبكة بما فيها retries والذيل، ويُعلن النفاد بدل صرف محاولة إضافية. الإلغاء المقبول في study_runs قبل معاملة الاكتمال يفوز؛ حدث خيط متأخر وحده لا يطمس نجاحاً محفوظاً.
