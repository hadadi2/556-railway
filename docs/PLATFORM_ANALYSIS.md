...
> (`requirements.txt`).
>
> **⚠ لقطة متقادمة — stale snapshot (وسم 2026-08-17):** هذا المرجع يصف الواقع
> عند PR #83 وقد لحقته **~115 PR** (بوابات الجودة #143–#185، طبقة HS بالسمات
> #177، طبقة التعميم G1/G2 #169–#170، ومبادرة منصّة المصانع `silk_platform/`
> كاملةً #186–#198 التي لا ذكر لها هنا إطلاقاً — مرجعها
> `docs/PLATFORM_ROADMAP.md`). المنهج (كل ادعاء بمرساة file:line) يبقى القدوة؛
> الأرقام والصفوف تُقرأ كتاريخ لا كحاضر — الصف ٦ أدناه مثلاً يصف «اللقطة
> السريعة» وقد **حُذفت نهائياً بقرار مالك** (`docs/DECISIONS.md` A1/#107).
...
| 6 | **Quick snapshot** — **DELETED after this snapshot** (owner decision A1, PR #107, 2026-07-16): `silk_snapshot.py` + `POST /products/snapshot` + the UI button were all removed; `product_snapshots` table kept dormant. Row retained for history only — do NOT rebuild. | ~~`POST /products/snapshot`~~ (gone) | **Deleted** | `docs/DECISIONS.md` A1; LESSONS row 9 (PART D). |
