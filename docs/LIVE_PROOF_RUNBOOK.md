...
يحتاج LibreOffice سليماً على الخادم (الكود يرفض التسليمَ بلا PDF بدل docx بديلٍ
صامت). **503 وحده لا يدلّ على السبب (البند ٢٨٤)** — اقرأ الرمز في الردّ وسطرَ
WARNING المقابل في سجلّ النشر (أو `/ops/last-errors`): `pdf_unavailable` = soffice
غائب (يُفحَص تثبيتُه)؛ `pdf_failed` = التحويلُ فشل (السجلّ يحمل rc وذيلَ stderr)؛
`pdf_rejected` = فحصٌ رفض الملفَّ (الأقواس بعد تحويلٍ ناجح، أو بوّابةُ محتوى) — عطلُ
شيفرةٍ أو محتوى لا بيئة، يتكرّر مع كلّ محاولة؛ يُعاد إنتاجه محلياً بـ`tools/pdf_smoke.py`.
