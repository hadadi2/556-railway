...
def _timeout_pair_for(host: str) -> tuple[float, float]:
    """(اتصال، قراءة) — EXT-11: مهلةُ الاتصال كانت تساوي مهلةَ القراءة (٤٥ ث على
    البنك الدولي) فيُنتظَر مضيفٌ ميت طويلاً؛ الاتصالُ ≤ ١٠ ث والقراءةُ كما كانت."""
    t = _timeout_for(host)
    return (min(10.0, t), t)
...
_KEY_AT_IMPORT = COMTRADE_KEY


def _comtrade_key() -> str:
    """مفتاحُ Comtrade **عند النداء** — R7 (SEC-4/EXT-18): القراءةُ وقتَ الاستيراد
    كانت تجعل مفتاحاً يُضبَط لاحقاً (لوحة الإعدادات، إعادة تشغيل بلا إعادة استيراد)
    غيرَ مرئيّ. الثابتُ `COMTRADE_KEY` يبقى للاختبارات التي تُطعّمه صراحةً."""
    if COMTRADE_KEY != _KEY_AT_IMPORT:
        return COMTRADE_KEY
    return os.environ.get("COMTRADE_API_KEY", "").strip()


def comtrade_auth(params: dict) -> dict | None:
    """اعتمادُ Comtrade الواحد — R7 (SEC-4): ترويسةٌ `Ocp-Apim-Subscription-Key` عادةً؛
    `SILK_COMTRADE_KEY_IN_QUERY=1` يعيدها معاملَ استعلام (صمّامُ رجوع). يعيد الترويسات
    أو None؛ المسبارُ وجامعُ البيانات وخطُّ التحليل يمرّون كلُّهم من هنا."""
    key = _comtrade_key()
    if not key:
        return None
    if os.environ.get("SILK_COMTRADE_KEY_IN_QUERY", "").strip() == "1":
        params["subscription-key"] = key
        return None
    return {"Ocp-Apim-Subscription-Key": key}


def _redacted(exc) -> str:
    """نصُّ استثناءٍ منقّح للسجلّ — الرابطُ بمعامله (المفتاح) لا يبلغ السجلّ (API-10)."""
    try:
...
def _comtrade_url() -> str:
    ...
    return ENDPOINTS["comtrade_data"] if _comtrade_key() else ENDPOINTS["comtrade"]
...
class ThrottleBacklog(ConnectionError):
    """طابورُ المباعدة أطولُ من السقف — EXT-10/CONC-2: كان النداءُ ينام حصّته مهما طال
    الطابور (مئةُ نداءٍ متزامن = آخرُها ينام مئةَ ثانية داخل خيطِ طلب). يُرفَع بلا حجزٍ
    ولا نوم؛ يرث ConnectionError فيُعلَن «تعذّر الجلب» كأيّ عطلِ شبكة."""


def _max_throttle_wait_s() -> float:
    try:
        return max(0.0, float(os.environ.get("SILK_HTTP_MAX_THROTTLE_WAIT_S", "30") or "30"))
    except ValueError:
        return 30.0
...
def _throttle(host: str) -> None:
    """مباعدةٌ لكل مضيف — EXT-10/CONC-2: الانتظارُ المتوقَّع فوق `SILK_HTTP_MAX_THROTTLE_WAIT_S`
    (٣٠ ث) يُرفَض (`ThrottleBacklog`) بلا حجزِ خانةٍ ولا نوم؛ وإلا يُنام الانتظارُ كاملاً
    (لا اقتطاعَ عند ٥ ث كان يخرق المباعدة فعلياً)."""
    ...
    with _host_lock:
        ...
        if wait > _max_throttle_wait_s():
            raise ThrottleBacklog(
                f"{host}: projected throttle wait {wait:.1f}s exceeds "
                f"SILK_HTTP_MAX_THROTTLE_WAIT_S={_max_throttle_wait_s():.0f}s")
...
def _http_get(url: str, params: dict | None = None,
    ...
    timeout = _timeout_pair_for(host)          # EXT-11: اتصالٌ قصير، قراءةٌ كما كانت
    # WS4 + EXT-1/EXT-9: قاطعُ الدائرة لكل مضيف — **مفتوحٌ** ⇒ `CircuitOpen` بلا أيّ
    # نداء (كان «الفشلُ السريع» نداءً حيّاً واحداً لكلّ واحدٍ من ~١٥٠ نداء fan-out)؛
    # **نصفُ مفتوح** (انقضت التهدئة) ⇒ محاولةٌ استكشافية واحدة بلا حلقة إعادة؛
    # نجاحُها يغلق الدائرة (record_success). أعطالُ الاتصال (لا 5xx وحدها) تُحسَب
    # على القاطع وتترك حدثاً في التتبّع ثم تُعاد للمستدعي كما كانت.
    import silk_circuit
    breaker = silk_circuit.http_breaker
    if breaker.is_open(host):
        _record_fetch_failure_event(host, url, status="circuit_open")
        raise silk_circuit.CircuitOpen(f"{host}: circuit open — no call made")
    retries = 0 if breaker.half_open(host) else int(
    ...
    for attempt in range(retries + 1):
        ...
        except Exception as exc:  # noqa: BLE001 — عطلُ اتصال: يُحسَب ويُعلَن ثم يُعاد رفعه
            breaker.record_failure(host)
            _record_fetch_failure_event(host, url, error_repr=repr(exc),
                                        attempt=attempt + 1)
            raise
...
def throttled_request(method: str, url: str, *, params: dict | None = None,
    ...
    **لماذا (تدقيق 2026-08-27، البند ٨).** كل الحماية الحقيقية (مباعدة لكل
    مضيف، إعادة محاولة بتراجع أسّي على 429/5xx مع احترام `Retry-After`، قاطع
    دائرة لكل مضيف، حدث فشلٍ بنيويّ في التتبّع) كانت محبوسةً داخل `_http_get`
    الخاص بهذه الوحدة — بينما معظم الوكلاء ينادون `requests.get` عارياً. أي
    ارتجاجة 429 عابرة من مزوّد كانت تتحوّل مباشرةً إلى **فجوة معلنة في تقرير
    مدفوع** بينما القاطع والإعادة كانا سيمتصّانها.

    **الفرق الوحيد عن `_http_get`:** ينادي `requests.get/post` على مستوى
    الوحدة لا الجلسة المجمّعة — مقصود: كل الاختبارات الهرمتية للوكلاء تعترض
    بـ`patch("requests.get", ...)`، فاستعمال الجلسة هنا كان سيجعلها تمرّ من
    فوق الاعتراض. (الجلسة المجمّعة تبقى لمسارات هذه الوحدة نفسها.)

    يرفع ما يرفعه `requests` (شبكة/مهلة) كما كان تماماً — المستدعي يمسك ويُعلن
    الفجوة كعادته؛ هذه الدالة لا تلفّق استجابة أبداً.

    Same throttle/retry/breaker as `_http_get`, but through module-level
    `requests` so every agent's hermetic patch keeps intercepting.
    """
    import requests as _rq  # lazy: الوحدة تبقى مستوردة بلا شبكة
    import silk_circuit
    host = url.split("/")[2] if "://" in url else url
    eff_timeout = timeout if timeout is not None else _timeout_pair_for(host)   # EXT-11
    breaker = silk_circuit.http_breaker
    if breaker.is_open(host):                    # EXT-1: لا نداءَ على قاطعٍ مفتوح
        _record_fetch_failure_event(host, url, status="circuit_open")
        raise silk_circuit.CircuitOpen(f"{host}: circuit open — no call made")
    retries = 0 if breaker.half_open(host) else int(
        os.environ.get("SILK_HTTP_RETRIES", "3"))
    fn = _rq.post if str(method).upper() == "POST" else _rq.get
    resp = None
    for attempt in range(retries + 1):
        _throttle(host)
        # **كل محاولة نداءٌ فعليّ** (مراجعة §58): على مزوّدٍ يُحاسِب بالنداء
        # (Serper) كانت إعادة المحاولة إنفاقاً **خفيّاً** — المستدعي يعدّ
        # واحداً بينما تخرج حتى أربعة طلبات مدفوعة. العدّ هنا يجعل المُضاعِف
        # مرئياً في `data_economics` كما تفرض قاعدة «لا إنفاق خفيّ».
        # `count_only` (الافتراضي True) يسمح لمستدعٍ يعدّ بنفسه بإطفائه.
        if count_fetches:
            try:
                import silk_context
        ...
        if attempt >= retries:
            breaker.record_failure(host)
            _record_fetch_failure_event(host, url, status=status,
                                        attempt=attempt + 1)
            return resp
        delay = _backoff_delay(
            attempt, (getattr(resp, "headers", None) or {}).get("Retry-After") or "")
        log.warning("HTTP %s from %s — retry %d/%d in %.1fs",
                    status, host, attempt + 1, retries, delay)
        _time.sleep(delay)
    return resp
...
def throttled_get(url: str, params: dict | None = None,
                  ...
                  count_fetches: bool = True):
    """GET عبر المسار المقوّى — see `throttled_request`."""
    return throttled_request("GET", url, params=params, headers=headers,
                             timeout=timeout, count_fetches=count_fetches)
...
class DataPoint:
    ...
    source_ids: tuple = ()
    # عقد المصدر (توجيه المنصّة، الموجة ١ — إضافي بحت، القيم الافتراضية تحفظ
    # كل مواقع الإنشاء القائمة): الوحدة، رابط المصدر العمومي (من
    # SOURCE_PUBLIC_URL — لا رابط استعلامٍ مختلَق)، الفترة المرجعية التي يصفها
    # الرقم (نص مثل "2023 CY" — أدقّ من data_year حين تلزم)، وطريقة الاسترجاع
    # (api | store | derived | llm_web | manual). نقصُ هذه الحقول لا يحجب
    # الرقم أبداً (تعديل مالك ٤) — فحص بوابة تحذيري فقط يقيس اكتمالها.
    unit: str = ""
    url: str = ""
    reference_period: str = ""
    retrieval_method: str = ""
    # الموجة C (البند T-04): معرّفاتُ الأدلة (`dp1`, `dp7`, …) التي استشهد بها
    # هذا البند داخل سجلّ بعثته. كانت تُنشأ وتُستعمَل للتحقّق **مرّةً** ثمّ
    # تُهمَل، فيستحيل ميكانيكياً اقتفاءُ جملةٍ في التقرير إلى دليلها — والادّعاءُ
    # المعلَن «كلُّ رقمٍ يقتفى إلى مصدره» يبقى شفهياً. **متمايزٌ عمداً عن
    # `source_ids`**: تلك أسماءُ مصادرَ عمومية (للمراجع والروابط)، وهذه معرّفاتٌ
    # داخل السجلّ (للاقتفاء). دمجُهما كان سيُفسِد الإسنادَ في المراجع.
    evidence_ids: tuple = ()
    # لقطة مصدر مستقلة عن ادعاء النموذج؛ تُحفظ مع نقاط التفتيش.
    raw_evidence: tuple = ()


# ── تصنيف الفجوات G1–G4 (توجيه المنصّة §2.3، الموجة ١) ─────────────────────
# G1 غير موجود (لا يجمعه أي مصدر معروف — يتطلب بحثاً أولياً)؛ G2 مقيَّد (خلف
# اشتراك/ترخيص — يُغلق بالشراء)؛ G3 فشل استرجاع (منشور وعام؛ عيب منصّة لا قيد
# معرفي)؛ G4 غير منطبق (لا ينطبق على هذا الزوج منتج/سوق — يُحذف لا يُعلن).
GAP_G1, GAP_G2, GAP_G3, GAP_G4 = "G1", "G2", "G3", "G4"
GAP_CLASS_LABEL_AR = {
    GAP_G1: "غير مجموع في أي مصدر معروف — يتطلب بحثاً أولياً",
    GAP_G2: "متاح خلف اشتراك/ترخيص — يُغلق بالشراء",
    GAP_G3: "منشور وعام؛ فشل الاسترجاع تقنياً — عيب منصّة لا قيد معرفي",
    GAP_G4: "غير منطبق على هذا المنتج/السوق",
}
_GAP_G2_NEEDLES = ("اشتراك", "مدفوع", "ترخيص", "الطبقة المدفوعة")
# النصّ الحقيقيّ الذي تُنتِجه مسارات الفشل هنا — **يُستورَد** حيثما احتيج
# (silk_gap_recovery)، فلا تُنسَخ إبرةٌ يدويّةٌ تتحجّر: إبرة «خطأ في واجهة»
# لم تُطابق يوماً نصّاً إنتاجياً وأسكتت التصنيف صامتاً (حادثة 2026-08-19).
RETRIEVAL_FAILURE_NEEDLES = (
    "فشل", "تعذّر", "حد المعدل", "حدّ المعدل", "غير مضبوط", "غير مُهيَّأ",
    "fetch_failed", "مهلة", "أعاد خطأ API", "خطأ API", "ليست قائمة",
    "بلا صفحة سجلات", "شكل غير متوقع", "خطأ في واجهة",
)
_GAP_G3_NEEDLES = RETRIEVAL_FAILURE_NEEDLES
# «429» رمزُ حالةٍ قائم بذاته — لا جزءاً من رقم/سنة (4290، 1429هـ، $429M):
# مطابقة حدّية لا احتوائية (مراجعة §58).
_GAP_429_RE = re.compile(r"(?<![\d$])429(?![\dهM])")
_GAP_G4_NEEDLES = ("لا ينطبق", "غير منطبق")


def classify_gap_class(text: str, status: str = "") -> str:
    """صنّف فجوة معلنة إلى G1–G4 من نصّها/حالتها — G1 الافتراضي الصادق
    (فجوة حقيقية) حين لا دليل على غير ذلك."""
    blob = f"{text or ''} {status or ''}"
    if any(n in blob for n in _GAP_G4_NEEDLES):
        return GAP_G4
    if any(n in blob for n in _GAP_G2_NEEDLES):
        return GAP_G2
    if (any(n in blob for n in _GAP_G3_NEEDLES)
            or _GAP_429_RE.search(blob) or status == "fetch_failed"):
        return GAP_G3
    return GAP_G1


def fact_to_datapoint(fact: dict) -> DataPoint:
    """محوّل باتجاه واحد: قاموس نتيجة الوكلاء الحتميين (`silk_research._f`)
    → DataPoint موحَّد (الموجة ١ — لا توحيد قسري للشكلين؛ قرار الخطة ١).

    يحمل unit/formula/modeled من شكل `_f`، وأول مصدر (`_src`) اسماً ورابطاً
    وثقةً وتاريخاً. الفشل = None/0.0 كالعادة — لا اختلاق."""
    srcs = fact.get("sources") or []
    first = srcs[0] if srcs and isinstance(srcs[0], dict) else {}
    note = fact.get("note", "") or ""
    if fact.get("modeled") and fact.get("formula"):
        note = (note + f" · مستنتَج: {fact['formula']}").strip(" ·")
    return DataPoint(
        ...
        # «من المخزن» لا يُقدَّم كجلبٍ حيّ (القانون A8) — الطريقة store لا api.
        retrieval_method=("derived" if fact.get("modeled")
                          else "store" if "من المخزن" in note
                          or "من المخزن" in str(first.get("source", ""))
                          else "api"),
    )


def datapoint_provenance(dp: object) -> dict:
    """بطاقة نسب موحَّدة لأي نقطة بيانات (كائن أو dict) — يقرؤها العرض
    والبوابة بشكل واحد. الرابط يُملأ من السجلّ العمومي عند غيابه (لا رابط
    استعلامٍ مختلَق — رابط مجموعة البيانات الرسمي فقط)."""
    def g(key, default=""):
        if isinstance(dp, dict):
            return dp.get(key, default)
        return getattr(dp, key, default)
    url = g("url") or public_source_url(g("source"))
    return {
        "value": g("value", None), "source": g("source"),
        "confidence": g("confidence", 0.0), "note": g("note"),
        "retrieved_at": g("retrieved_at"), "status": g("status"),
        "data_year": g("data_year", None), "unit": g("unit"),
        "url": url, "reference_period": g("reference_period"),
        "retrieval_method": g("retrieval_method"),
    }
...
def atomic_source_ids(source: object, source_ids: object = None) -> list:
    ...
    raw: list = []
    # **الشكلُ غيرُ المتوقَّع يُطبَّع هنا — عند القارئ القانونيّ لا عند كلّ
    # مستهلك.** الحقلُ يصل أحياناً من JSON نموذجٍ بشكلٍ غيرِ قائمة، وثلاثةُ
    # أشكالٍ منها كانت تُنتِج ضرراً مقيساً (مراجعةٌ ذاتية §٥٨):
    #   • سلسلةٌ مفردة  `"GAFTA"`      ⇒ `['G','A','F','T']` — أسماءُ مصادرَ
    #     **مختلَقة** تُطبَع في المصنوع، في نظامٍ عقدُه أن لا شيء يُختلَق؛
    #   • قاموس        `{"A":1,"B":2}` ⇒ `['A','B']` — مفاتيحُ قاموسٍ تُقدَّم
    #     أسماءَ مصادر (أسوأُ من الانهيار: اختلاقٌ صامت)؛
    #   • عددٌ/قيمةٌ مفردة `5`          ⇒ `TypeError` خامٌّ يخرج من `build_view`
    #     فيصير `500` على تصديرِ تقريرٍ بدل فجوةٍ معلَنة.
    # الثلاثةُ تُقرأ الآن على أنّها «لا معرّفاتٍ صريحة» أو «معرّفٌ واحد» —
    # فيرتدّ القارئ إلى `source` بدل أن يخترع أو ينهار.
    if isinstance(source_ids, str):
        source_ids = [source_ids]
    elif isinstance(source_ids, dict):
        source_ids = None
    elif source_ids is not None and not isinstance(source_ids,
...
def _cached_get(url: str, params: dict, ttl_seconds: int = 86400,
                headers: dict | None = None, cacheable=None, short_lived=None) -> object:
    ...
    try:
        ...
        _hk = {"headers": headers} if headers else {}
        if cacheable is not None:
            _hk["cacheable"] = cacheable
        if short_lived is not None:
            _hk["short_lived"] = short_lived
        # EXT-2: فشلُ الجلب نفسِه يعود علامةً (`_FETCH_FAILED`) لا `None` — `None`
        # يبقى «طبقةُ الكاش نفسها تعطّلت» فيسقط المستدعي إلى جلبٍ مباشر؛ أمّا الفشلُ
        # المعلَن فلا يُعاد حيّاً مرّةً ثانية (كان يُنفَق مرّتين على كلّ 429).
        return cached_get(url, params, ttl_seconds=ttl_seconds,
                          fetcher=_http_get, fail_sentinel=silk_cache._FETCH_FAILED,
                          **_hk)  # pooled fetch (P2)
...
def _is_fetch_failed(payload: object) -> bool:
    """هل العائدُ علامةُ «فشل الجلب» من طبقة الكاش (EXT-2)؟"""
    try:
        import silk_cache
        return payload is silk_cache._FETCH_FAILED
    except Exception:  # noqa: BLE001
        return False


def _count_live_fetch() -> None:
    """عُدّ نداءً حياً — سقوطُ الكاش إلى GET مباشر نداءٌ ثانٍ كان لا يُحسب
    في عدّاد اقتصاد البيانات (قاعدة «لا إنفاق خفي» — صيد الفجوات ٣)."""
    try:
        import silk_context
        silk_context.count_data("live_fetches")
    except Exception:  # noqa: BLE001 — العدّ قناة جانبية لا شرط
        pass
...
def comtrade_trade(
    ...
    try:
        # بلا مفتاحٍ يبقى شكلُ النداء القديم (مفتاحُ الكاش والاختباراتُ التي تُطعّم `cached_get`).
        _hk = {"headers": headers} if headers else {}
        # EXT-3: ردٌّ ناجح بلا سجلات يعيش عشرَ دقائق لا يوماً (سقفُ المعاينة يعطي
        # فراغاً عابراً)، والأغلفةُ غيرُ القاموسية لا تُخزَّن.
        payload = _cached_get(url, params, ttl_seconds=ttl,
                              cacheable=lambda p: isinstance(p, dict),
                              short_lived=lambda p: not p.get("data"), **_hk)
        if _is_fetch_failed(payload):   # EXT-2: فشلٌ معلَن — لا جلبَ حيّاً ثانياً
            raise RuntimeError("fetch failed (declared by the cache layer)")
        if payload is None:  # cache layer itself unavailable -> direct GET as before
            _count_live_fetch()  # نداء حي ثانٍ — لا إنفاق خفي (صيد ٣)
            r = _http_get(url, params, **_hk)
            r.raise_for_status()
            payload = r.json()
...
def _world_bank_for_year(iso3: str, indicator: str,
    ...
    try:
        # EXT-3: أغلفةُ خطأ البنك الدولي لا تُخزَّن أسبوعاً، والصفحةُ الفارغة تعيش عشرَ دقائق.
        payload = _cached_get(url, params, ttl_seconds=7 * 86400,  # M2: WB 7d
                              cacheable=lambda p: _wb_shape_error(p) is None,
                              short_lived=lambda p: not (p[1] if len(p) > 1 else None))
        if _is_fetch_failed(payload):   # EXT-2: فشلٌ معلَن — لا جلبَ حيّاً ثانياً
            return DataPoint(None, "World Bank", 0.0,
                             f"{indicator} ({iso3}): تعذّر الجلب (حد معدل/شبكة) — أعد المحاولة",
