...
def _tariff_keywords() -> tuple:
    """مفرداتُ التعرفة من مصدرها الواحد (`silk_economics`) — لا قائمةَ ثانية
    تتباعد عند أوّل إصلاح (الدرس ٢٦٢). استيرادٌ كسولٌ داخل دالّة كي تبقى هذه
    الوحدة خفيفةً بلا تبعيةٍ دائرية عند التحميل."""
    try:
        from silk_economics import tariff_words
        return tuple(dict.fromkeys(tariff_words() + ("MFN",)))
    except Exception:  # noqa: BLE001 — القائمةُ القائمة احتياطاً
        return ("التعرفة", "الرسوم الجمركية", "tariff", "MFN")
...
_KW = {
    ...
    # الدرس ٢٦٢: مفرداتُ التعرفة **واحدةٌ للريبو كلّه** — كانت هنا «التعرفة»
    # وحدَها فتفوت «التعريفة المطبَّقة» (صيغة مزوّد WTO/WITS الحرفية التي
    # تكتبها البعثة فعلاً)، فيخرج عمودُ الملاءمة التنظيمية بلا تعرفةٍ بينما
    # القسمُ التنظيميّ يطبعها — نفسُ تناقض التقرير 6 في قارئٍ ثالث.
    "tariff_applied_pct": _tariff_keywords(),
