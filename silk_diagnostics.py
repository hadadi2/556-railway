...
def _redact(text: str) -> str:
    ...
    """
    out = str(text or "")
    # صيد الفجوات ٣: مسبار Serper يقرأ SERPER_API_KEY احتياطاً
    # (silk_websearch_agent.search_key) وكان خارج قائمة التنقيح.
    # تدقيق 2026-08-27 (البند ١٥): كانت القائمة تنقص مفاتيح WTO ومزوّدي الدفع،
    # فأوّل مسبار يُضاف لهما كان سيرث قيمةً غير منقّحة. القائمة تُقاس بـ
    # `.env.example` لا بالذاكرة — راجع `tests/test_audit_2026_08_27_fixes.py`.
    for env in ("COMTRADE_API_KEY", "GOOGLE_MAPS_API_KEY", "SEARCH_API_KEY",
                "SERPER_API_KEY",
                "LOCALPRICE_API_KEY", "VOLZA_API_KEY", "EXPLEE_API_KEY",
                "ANTHROPIC_API_KEY",
                "WTO_TTD_API_KEY", "WTO_API_KEY",
                "MOYASAR_SECRET_KEY", "TAP_SECRET_KEY", "STRIPE_SECRET_KEY",
                "SILK_API_KEY", "SILK_OWNER_KEY", "SILK_PLATFORM_SECRET"):
...
def _probe_comtrade(year: int) -> dict:
    ...
    key = bool(dl._comtrade_key())

    def go():
        params = {"reporterCode": "784", "period": str(year), "cmdCode": "080410",
                  "flowCode": "M", "partnerCode": "0"}
        # مراجعة R7: المسبارُ يسلك مسارَ الإنتاج نفسه (ترويسةٌ أو صمّامُ الاستعلام) —
        # كان يختبر الاستعلامَ القديم فيُخضّر بينما التحليلُ الحيّ يفشل.
        headers = dl.comtrade_auth(params)
        r = dl._http_get(dl._comtrade_url(), params, **({"headers": headers} if headers else {}))
        r.raise_for_status()
