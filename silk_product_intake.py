...
_INTAKE_MODEL = os.environ.get(
    "SILK_INTAKE_MODEL",
    os.environ.get("SILK_AI_FAST_MODEL", "claude-haiku-4-5"))
...
def _extract_json(text: str | None) -> dict | None:
    ...
    المنطق في `silk_json.extract` (تدقيق البند ٥)؛ `dict_only=True` يحفظ سلوك
    هذه النسخة الأصلي: مصفوفة عليا ليست نتيجةً مقبولة هنا."""
    import silk_json
    return silk_json.extract(text, dict_only=True)
...
def _vision_extract(image_b64: str, media_type: str, kind: str) -> str | None:
    ...
    from silk_vision import call_vision          # EXT-13: مقعدُ الرؤية الواحد (لا حكمَ هنا — الدرس ٢١)
    steer = _isolate(_PROMPT.get(kind, _PROMPT["product"]))
    out = call_vision(_SYSTEM, steer, image_b64, media_type,
