...
def _run_start(source: str, requested: int) -> int:
    with silk_store._open() as conn:   # R5 (DB-8): يُغلَق لا يُلتزَم فحسب
        cur = conn.cursor()
...
def comtrade_budget_left() -> int:
    ...
    """
    from silk_data_layer import _comtrade_key   # مراجعة R7: المفتاحُ عند النداء لا الاستيراد
    default = 450 if _comtrade_key() else 4
...
def collect_worldbank(indicators: list[str] | None = None,
    ...
    for ind in indicators:
        ...
        try:
            ...
            r = throttled_get(url, params=params, timeout=60, count_fetches=False)
            r.raise_for_status()
            payload = r.json()
            recs = payload[1] if isinstance(payload, list) and len(payload) > 1 else []
            # دفعة واحدة لا صفّاً صفّاً (تدقيق البند ٦): `per_page=20000` يعني
            # حتى عشرين ألف معاملة/التزام منفصل على قرص النشر للمؤشر الواحد،
            # وكلٌّ منها قفلٌ على المخزن الذي تقرأ منه الطلبات الحيّة.
            batch = [
                {"iso3": iso3, "indicator": ind, "year": int(yr),
                 "value": float(val), "source": "World Bank",
                 "confidence": _WB_CONF, "note": f"{ind} bulk collect"}
                for rec in (recs or [])
                for iso3 in [(rec.get("countryiso3code") or "").strip()]
                for val, yr in [(rec.get("value"), rec.get("date"))]
                if len(iso3) == 3 and val is not None and yr
            ]
            n = silk_store.upsert_indicators(batch) if batch else 0
            fetched += 1
...
def _seed_fallback(indicator: str) -> int:
    ...
    if getter is None:
        return 0
    # دفعة واحدة كنظيرتها في `refresh_worldbank` (تدقيق البند ٦): القاعدة
    # واحدة في كل مسارات الكتابة — لا صفّاً صفّاً على قرص النشر.
    batch = []
    for iso3 in ISO3_TO_M49:  # الأسواق المعروفة للمنصّة تكفي كتغطية بذور
        got = getter(iso3)
        if got:
            batch.append({"iso3": iso3, "indicator": indicator,
                          "year": int(got[1]), "value": float(got[0]),
                          "source": seed._SOURCE, "confidence": 0.85,
                          "note": "seed fallback"})
    n += silk_store.upsert_indicators(batch) if batch else 0
    return n
...
def check_post_entry(path: str | None = None) -> list[dict]:
    ...
    for item in _entered_deep_research_analyses(path):
        ...
        if cur is None or prev is None:
            # 1b: تعذّر الجلب ≠ صفر — جمعُ None صفراً كان يولّد إنذار «نمو
            # سالب» كاذباً لسوقٍ مدخول من فشل جلبٍ عابر (صيد الفجوات ٣).
            # القياس المتعذّر يُتخطى معلَناً في السجل، لا يُختلَق اتجاه.
            log.warning("post-entry monitoring: fetch unavailable for "
                        "analysis %s (%s year) — قياس متعذّر، لا إنذار",
                        aid, "current" if cur is None else "previous")
            continue
        cur_v = sum(v for v in (primary_value(r) for r in cur) if v)
        prev_v = sum(v for v in (primary_value(r) for r in prev) if v)
...
def start_scheduler():
    ...
    def _loop() -> None:
        ...
        while True:
            ...
            # كنّاس القرص الدائم (تدقيق البند ١١) — مطفأ ما لم تُضبَط نافذة
            # احتفاظ؛ مستقل عن refresh كالحصاد أعلاه: فشلُه لا يمنع التحديث.
            try:
                import silk_janitor
                silk_janitor.sweep()
            except Exception as e:  # noqa: BLE001 — الكنس يفشل، الخيط يبقى
                log.warning("scheduled janitor sweep failed: %s", e)
            try:
                got = refresh()
