...
class CircuitOpen(ConnectionError):
    """القاطعُ مفتوح — EXT-1 (تدقيق 2026-09-01): يُرفَع **بلا أيّ نداء شبكة** بدل
    «فشلٍ سريع» كان لا يزال نداءً حيّاً لكلّ واحدٍ من ~١٥٠ نداء fan-out على مضيفٍ ميت.
    يرث ConnectionError فيمسكه كلُّ مستدعٍ يعامل أعطالَ الشبكة أصلاً."""
...
class CircuitBreaker:
    ...
    def half_open(self, key: str) -> bool:
        """نصفُ مفتوح: بلغ الفشلُ العتبةَ وانقضت التهدئة — محاولةٌ استكشافية واحدة (EXT-9)."""
        with self._lock:
            st = self._state.get(key)
            if not st or st[0] < self.threshold:
                return False
            return (time.monotonic() - st[1]) >= self.cooldown_s
