...
- Same-origin is the ONLY frontend mode: `api.py` mounts `web/` on `/`. The
  decoupled `netlify.toml` alternative was deleted in the 2026-09-01 audit
  (DEBT-6) — it was never deployed and drifted. `CORS_ORIGINS` stays for
  scripted API callers, not for a second frontend host.
...
- **Daemon threads die.** An `async_run` /research mid-flight leaves DB status
  ...
  not self-healing.
- **Platform studies (R2, 2026-09-02) are stamped, not lost.** Every launch is a
  durable `study_runs` row (`silk_platform/study_runtime.py`, migration 018).
  On SIGTERM the shutdown handler marks this process's running rows
  `interrupted` (study → draft + quota refund + resume pointer + notification);
  on boot `study_runtime.recover()` interrupts any row whose heartbeat is older
  than `SILK_PLATFORM_RUN_STALE_S` (300 s) and starts queued rows. Consequence:
  **every deploy interrupts live factory studies and notifies them** —
  recovery is explicit by owner rule (the factory relaunches; checkpoints make
  it cents, never dollars). `--timeout-graceful-shutdown 15` (Dockerfile +
  railway.json) is what lets the stamp run before Railway's kill; without it
  the heartbeat path still recovers, just up to 300 s later. Root `/research`
  async threads are NOT covered — the manual `resume` path above still applies.
