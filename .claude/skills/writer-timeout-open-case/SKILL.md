...
4. **Candidate fixes already considered and deliberately NOT chosen without
   ...
   - ~~Streaming responses from the Anthropic API.~~ **ADOPTED 2026-08-22
     (wave p6, owner-confirmed live branch b: analyst `ReadTimeout` swallowed →
     writer run on the error string → writer `ReadTimeout` → `report=None`).**
     Both heavy calls now stream (`silk_llm_provider.complete/complete_tools
     (stream=True)`): the read timeout is an idle gap between SSE events
     (`SILK_AI_STREAM_IDLE_S`=120) under a total ceiling
     (`SILK_AI_STREAM_TOTAL_S`=900); an abort returns the partial text with
     `stop_reason="aborted_timeout"` and the writer continues from it. New
     decision-table rows: `aborted_timeout` with partial text → continuation
     path (`draft_continue*` events); `StreamTotalTimeout` → raise
     `SILK_AI_STREAM_TOTAL_S`; `empty_stream` → a gateway returned no SSE and
     no JSON body; `path=json_fallback` in the log → a proxy is stripping the
     stream flag (still works, but the 300s wall is back — fix the proxy).
   ...
   **Decision rule: pick exactly the fix the captured evidence points at.**
   ...
   Cheap reproduction after wave p6: `POST /research {"resume": <id>}` on a
   completed-but-reportless run re-uses the saved missions **and the saved
   analyst stage** and reruns only the writer; `GET /analyses/{id}` on a failed
   run returns the saved missions/stages (`partial: true`).
