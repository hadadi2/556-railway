...
| 2 | Analyst builds exactly 5 intersections | `silk_market_analyst.py` — `REQUIRED_CATEGORIES` at lines 41-42 (drifted anchor fixed 2026-08-17): `demand, entry_cost, price_competitiveness, entry_door, swot`; category matching at lines 133-140 | The analytical spine. Matching is NORMALIZED (lowercase + strip) since the wave-9 live bug: Claude returning `"Demand"` instead of `"demand"` silently emptied all 5 intersections (comment at `silk_market_analyst.py:128-132`). Timeout is `_LONG_TIMEOUT` (`SILK_AI_LONG_TIMEOUT_S`, default 300s — `silk_ai_judge.py:25`). |
...
| 6 | Quality gate | `silk_quality_gate.run_quality_gate()` at `silk_quality_gate.py:229` — **40 deterministic check functions** as of PR #185 (grew from 10 via live incidents #143/#183/#184/#185; 20 are FAIL-triggering, 17 are regression guards — count them with `grep -c 'def _check_' silk_quality_gate.py`, don't trust this line), zero Claude, zero network | The gate verdict never blocks operator surfaces, but client export IS blocked on FAIL (409 unless `?override=1` with `X-Owner-Key` — LESSONS 46/53), and `run_client_artifact_text_gate` refuses to build a leaking client docx. Verdict attached — verdict `PASS` / `PASS-WITH-WARNINGS` / `FAIL` attached via `_attach_quality_gate()` (`api.py:759`). Non-repairable findings become `methodology_notes` injected into the docx under "حدود المنهجية وجودة البيانات" inside section 2 (`silk_reports.py:1058-1068`). |
...
6. Only after the cheap loop passes, measure a FULL run against the golden case:
   `python3 -m silk_evals --case nigeria_peanut_butter` — score drop > 10 vs `evals/scores.json`
