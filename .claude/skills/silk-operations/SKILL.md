...
| **Durable study runs (R2, 2026-09-02)** | `silk_platform/study_runtime.py` + `study_runs` (migration 018): `create_run` inside the launch transaction, `dispatch()` claims under `BEGIN IMMEDIATE` up to `SILK_PLATFORM_MAX_CONCURRENT_RUNS` (3), one `silk-platform-run-supervisor` thread heartbeats (`SILK_PLATFORM_HEARTBEAT_S` 30) / interrupts stale rows (`SILK_PLATFORM_RUN_STALE_S` 300, any boot) / times out at the grace window, `shutdown()` (registered via `add_event_handler` in `mount()`) stamps `interrupted`, `request_cancel()` is cooperative through `silk_context.check_cancelled` at pipeline stage boundaries. No auto-resume by owner rule. | Not `engine_bridge._ACTIVE` (now a mirror kept for test seams and the legacy sweep of rows without a run record), not `sweep_orphans` (legacy rows only — its "readable analysis_id ⇒ completed" branch is RC-2, deliberately untouched), not the root `/research` async threads |
