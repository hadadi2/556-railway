...
def _report_fields(rep: object) -> dict:
    ...
    if isinstance(rep, dict):
        return {"findings": rep.get("findings") or [], "failed": bool(rep.get("failed"))}
    return {"findings": getattr(rep, "findings", None) or [], "failed": bool(getattr(rep, "failed", False))}
...
def _known_numbers(mission_reports: dict) -> set[float]:
    ...
    for rep in (mission_reports or {}).values():
        for f in _report_fields(rep)["findings"]:
            raw = f.get("raw_evidence") if isinstance(f, dict) else getattr(f, "raw_evidence", ())
            if raw:
                from silk_evidence_contract import source_numbers
                known.update(source_numbers(raw))
            else:
...
def _judge_prompt(result: dict) -> str:
    from silk_ai_judge import _isolate, report_sections
    dr = result.get("deep_research") or {}
    report_text = (dr.get("report") or {}).get("report") or ""
    missions = dr.get("missions") or {}
    # نافذة محدودة موزعة على التقرير، فلا تختفي الأقسام الأخيرة خلف المقدمة.
    if len(report_text) > 32000:
        stride = max(1, (len(report_text) - 1000) // 31)
        report_text = "\n[مقتطفات موزعة؛ التقييم البنيوي الكامل مطلوب منفصلاً]\n" + "\n".join(
            report_text[i * stride:i * stride + 1000] for i in range(32))
    mission_summaries = "\n".join(
    ...
    return (
        ...
        f"1. section_completeness: هل الأقسام {len(report_sections())} المطلوبة حاضرة "
