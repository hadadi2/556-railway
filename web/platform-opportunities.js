...
window.SilkOpportunities = (() => {
  ...
  const call=async(path='',options)=>{try{return await api('/export-opportunities'+path,options);}catch(e){
    const messages={hs_not_found:text('الرمز غير موجود في جدول مطابقة ITC.','HS code not found in ITC correspondences.'),itc_product_excluded:text('الرمز صحيح، لكن ITC يستبعده من حساب إمكانات التصدير.','Valid HS code, but excluded from ITC export potential.')};
    if(messages[e.detail?.code])e.message=messages[e.detail.code];throw e;
  }};
  ...
  async function explore(){
    ...
    try{
      ...
      '<p><a class="btn gh" href="/export-potential?axis=markets&exporter=682&product='+encodeURIComponent(d.product_mapping?.itc_code||d.product.hs_code)+'&market=w&to=j&what=k" target="_blank" rel="noopener">'+text('عرض الرسوم وجميع الأسواق ↗','View charts and all markets ↗')+'</a></p>';
      if(d.product_mapping?.grouped){const note=document.createElement('p');note.className='epnotice';note.textContent=text('يعرض ITC هذه القيم لمجموعة المنتجات: ','ITC reports these values for the product group: ')+d.product_mapping.itc_code+' — '+d.product_mapping.itc_name;node.prepend(note);}
