...
window.SilkOpportunities = (() => {
  ...
  const number=v=>v==null?'—':new Intl.NumberFormat(LANG==='ar'?'ar-SA':'en-US',{maximumFractionDigits:0}).format(v);
  ...
  function renderSaved(){
    ...
    node.innerHTML=saved.length?'<div class="eptable"><table><thead><tr><th>'+text('المنتج / السوق','Product / market')+'</th><th>'+text('لقطة إمكانات ITC','ITC potential snapshot')+'</th><th>'+text('الحالة','Status')+'</th><th>'+text('الخطوة التالية','Next step')+'</th></tr></thead><tbody>'+saved.map(o=>'<tr><td>'+esc(o.snapshot.product_name)+'<br><small>'+esc(marketName(o))+' · '+esc(o.hs_code)+'</small></td><td>'+number(o.snapshot.row.potential)+'<br><small>'+esc(date(o.snapshot.provenance.retrieved_at))+'</small></td><td>'+esc(o.link_status==='changed'?text('تغير نطاق الدراسة','Study scope changed'):state(o.study_state))+'</td><td><button class="btn gh sm" data-study="'+o.id+'">'+(o.study_state?text('الدراسة #','Study #')+o.study_id:text('قيّم السوق لمصنعي','Review market fit'))+'</button> <button class="btn gh sm" data-archive="'+o.id+'">'+(archived?text('استعادة','Restore'):text('أرشفة','Archive'))+'</button></td></tr>').join('')+'</tbody></table></div>':'<p class="epempty">'+text('ابدأ باستكشاف أحد منتجاتك، ثم احفظ السوق الذي يهمك.','Explore a product, then save a market that interests you.')+'</p>';
