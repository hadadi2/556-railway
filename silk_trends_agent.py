...
def reset_series_memo() -> None:
    """لا ذاكرةَ عمليةٍ عامّة (مراجعة EXT): السلسلةُ تُمرَّر صراحةً داخل التشغيلة الواحدة.
    تبقى الدالّةُ عقداً للاختبارات القائمة — لا شيءَ لتمسحه."""
    return None


def _interest_with_series(kw: str, geo: str | None, timeframe: str, label: str):
    """جلبُ سلسلة الاهتمام مرّةً واحدة تحت `_retry_429` — EXT-12: كانت التهيئةُ والجلبُ
    مكرّرَين في ثلاثة مواضع (اهتمام/سلسلة/موسمية) على حصّة pytrends المحدودة أصلاً.

    مراجعة EXT (§58): توحيدُ المسار وحدَه لم يقلّل **عدد** النداءات — وكيلُ الاتجاهات
    يطلب الاهتمامَ ثم الموسميةَ لنفس (كلمة، جغرافيا، مدى). الحلُّ تمريرُ الإطار المجلوب
    صراحةً (`series=`) داخل التشغيلة الواحدة، لا ذاكرةُ عمليةٍ عامّة (تلك تُخفي فشلاً
    لاحقاً بنجاحٍ سابق). يعيد DataFrame أو None."""
    from pytrends.request import TrendReq  # lazy: optional dep

    def _fetch():
        py = TrendReq(timeout=(10, 30))
        py.build_payload(kw if isinstance(kw, list) else [kw], timeframe=timeframe, geo=(geo or ""))
        return py.interest_over_time()
    return _retry_429(_fetch, label)
...
def trends_interest(
    ...
    try:
        ...
        df = series if series is not None else _interest_with_series(
            kw, geo, timeframe, f"interest '{kw}'")
...
def trends_interest_resilient(keyword: str, geo: str | None = None,
                              timeframe: str = "today 12-m", series=None) -> DataPoint:
    ...
    """
    live = trends_interest(keyword, geo, timeframe, series=series)
...
def trends_series(keyword: str, geo: str | None = None,
    ...
    try:
        df = _interest_with_series(kw, geo, timeframe, f"series '{kw}'")
        if df is None or df.empty or kw not in df.columns:
            return {"mean": None, "growth_pct": None, "n": 0, "confidence": 0.0,
...
def _seasonality(keyword: str, geo: str | None, timeframe: str, series=None) -> DataPoint:
    ...
    try:
        ...
        df = series if series is not None else _interest_with_series(
            kw, geo, timeframe, f"seasonality '{kw}'")
        if df is None or df.empty or kw not in df.columns:
            return DataPoint(None, "Google Trends", 0.0,
...
def broaden_if_weak(keyword: str, geo: str | None, timeframe: str,
    ...
    for cand in cands:
        ...
        if not broader or broader == keyword:
            continue
        # استعلام مشترك يضمن مقياس تطبيع واحد للكلمتين.
        try:
            pair = _interest_with_series([keyword, broader], geo, timeframe, "broaden_pair")
            narrow_mean, broad_mean = float(pair[keyword].mean()), float(pair[broader].mean())
        except Exception:  # noqa: BLE001 — لا مقارنة بين مقياسين مستقلين
            continue
        if broad_mean > narrow_mean:
            return DataPoint(
                broad_mean, "Google Trends", 0.6,
                f"في استعلام مشترك: '{broader}'={broad_mean}/100 و'{keyword}'={narrow_mean}/100؛ "
                "اهتمام بحث نسبي، وليس حجم طلب أو مبيعات",
                _today())
    return None


# رسالة إغلاق فجوة الموسمية (Wave 2.2) — خطوة عملية مقترحة في «ما لم يكتمل».
SEASONALITY_GAP_CLOSURE = ("الموسمية (رمضان/الأعياد) غير متاحة من مؤشرات "
...
class TrendsAgent(BaseAgent):
    ...
    def _execute(self, task: dict) -> AgentReport:
        ...
        # مراجعة EXT (§58): جلبٌ واحد لكلّ تشغيلة — الاهتمامُ والموسميةُ من الإطار نفسِه
        # (كانا نداءَي pytrends على نفس الثلاثية، وحصّتُها محدودة).
        _series = None
        try:
            _series = _interest_with_series(
                (keyword or "").strip(), geo, timeframe, f"interest '{keyword}'")
        except Exception as _e:  # noqa: BLE001 — الجلبُ المشترك أفضلُ جهد
            log.debug("shared trends series unavailable: %s", _e)
        interest = trends_interest_resilient(keyword, geo, timeframe, series=_series)
        ...
        if broadened is not None:
            ...
            summary += ("؛ الطلب على الفئة الأعمّ موجود — الصفة الدقيقة "
                        "غير مبحوثة")
        season = _seasonality(keyword, geo, timeframe, series=_series)
