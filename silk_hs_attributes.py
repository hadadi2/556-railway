...
def resolve_by_attribute(product: str, candidates: list,
                         ...
                         allow_web: bool = True,
                         gl: str | None = None,
                         band_context: list | None = None) -> dict:
    ...
    **قناتا المحور** (متابعة المهمة ٥ — عمارةُ المُشرِف 2026-08-31):
    `band_context` — إن مرّره المستدعي — هو أشقّاءُ المحور كاملين (بصفوف
    الإكمال) فمنه تُبنى حدودُ المُميِّز؛ أمّا `candidates` فقناةُ **الأهلية**
    والنتيجةُ لا تكون إلا منها. قيمةٌ تصيب نطاقَ صفِّ إكمالٍ حصراً لا تُحسَم
    — يُعرَض الحوار بسببٍ يسمّي ذلك النطاق. بلا `band_context` يبقى السلوك
    القديم حرفياً."""
    if not enabled():
        return _report(None, missing_ar="حسمُ السمة الرقمية مُعطَّل "
                                        "(SILK_HS_ATTRIBUTE_RESOLVE=0)")
    disc = discriminator(band_context if band_context else candidates)
    if disc is None:
        return _report(None, missing_ar="لا تتمايز المرشّحات بعتبةٍ رقمية "
                                        "واحدة — لا حسمَ آليّ ممكن")
    # قناةُ الأهلية: النتيجةُ من هؤلاء وحدهم مهما اتّسع سياقُ النطاقات.
    eligible = {code for code, _ in _candidate_codes(candidates)}
    ...
    for r in sorted(readings, key=lambda x: -_SOURCE_RANK.get(x["source"], 0)):
        ...
        if hs6 is not None and hs6 not in eligible:
            # القيمةُ أصابت نطاقَ صفٍّ أُدرج لإكمال عرض المحور لا مرشّحاً
            # مُقيَّماً — النطاقُ يُعلِم الحدودَ ولا يصلح نتيجةً أبداً (نهيُ
            # المُشرِف عن توسيع التغطية): يبقى الحوارُ بسببٍ يسمّيه.
            searched.append(
                f"القيمة {_fmt(r['value'])}{r['unit']} تقع في نطاق البند "
                f"{hs6} المعروضِ لإكمال المحور لا في نطاق مرشّحٍ مُقيَّم — "
                "لا يُحسَم به")
            break
        if not hs6:
