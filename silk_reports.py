...
class ClientArtifactGateError(RuntimeError):
    ...
    نوعٌ مستقلٌّ كي يميّزه سطحُ المصنع عن «python-docx غائبة»: الأخيرُ 501/503
    `*_unavailable`، وهذا رفضُ تسليمٍ يُوجَّه إلى 409 `quality_gate_fail` عبر
    `silk_export_gate.factory_detail` بلا تسريبِ لغةِ البوّابة الداخلية إلى
    العميل (G-04). يحمل `findings` (أسماءُ الفحوص) لبناءِ حمولةِ الـ409.
    """
...
def _degraded_banner_text(view: dict, lang: str = "ar") -> str | None:
    ...
    if not view.get("degraded"):
        return None
    # الموجة ٠: سببُ التدهور نصٌّ داخليّ قد يكون بلغة الأدلة — يُسقَط إن كان
    # بلغةٍ أخرى ويحلّ محلّه السببُ العامّ، فاللافتة تبقى معلَنةً ومقروءة.
    reason = _lang_safe(view.get("degraded_reason"), lang) \
        or _T("degraded_reason_default", lang)
    return _T("degraded_banner", lang, reason=reason)


def _stamp_degraded_banner(doc, view: dict, lang: str = "ar") -> None:
    """اطبع لافتة تدهور حمراء بارزة — تُستدعى عند الغلاف وأعلى كل قسم رئيسي
    من قسم البحث العميق. لا أثر إن لم يكن التشغيل متدهوراً.

    الموجة ٠: تشغيلةٌ متدهورة بالإنجليزية كانت تُنتِج **لا تقرير** بدل تقريرٍ
    موسوم — اللافتة العربية تُفشِل البوابة الحاجزة. التحذيرُ أهمّ من أن يضيع."""
    from docx.shared import RGBColor
    # بلاغ تحليل 20 (تجاوز §5-الإتلاف): لافتة «تقرير غير مكتمل» حمراء بارزة —
    # نفسُ نقطة اللافتة المشتركة (تُستدعى من كل بنّاء docx) فلا يُفلِت سطح. تقرأ
    # علَم العرض الواحد؛ الأقسام الغائبة تُعلَن فجواتٍ لا نتائج. مستقلّة عن لافتة
    # التدهور (قد يكون التقرير ناقصاً بلا تدهورٍ عامّ).
    _rep = (view.get("deep_research") or {}).get("report") or {}
    if _rep.get("incomplete"):
        _miss = "، ".join(m for m in (_rep.get("missing_sections") or []) if m)
        _miss_en = ", ".join(m for m in (_rep.get("missing_sections") or []) if m)
        if lang == "en":
            _txt = (("⚠ INCOMPLETE REPORT — the following sections were not "
                     f"written and are declared gaps, not findings: {_miss_en}. "
                     "The decision and the written sections stand.") if _miss_en
                    else ("⚠ INCOMPLETE REPORT — the report was truncated before "
                          "completion (its final section is cut off). The "
                          "decision and the written sections stand."))
        else:
            # مراجعة §58 #2: اقتطاعٌ منتصفَ الجملة بلا أقسامٍ غائبة لا يُصرَّح
            # بقائمة «—» مضلِّلة — يُقال «اقتُطِع قبل اكتماله».
            _txt = (("⚠ تقرير غير مكتمل — لم تُكتَب الأقسام التالية وتُعَدّ فجواتٍ "
                     f"معلنة لا نتائج: {_miss}. القرار والأقسام المكتوبة سليمة.")
                    if _miss else
                    ("⚠ تقرير غير مكتمل — اقتُطِع قبل اكتماله (قسمه الأخير مبتور). "
                     "القرار والأقسام المكتوبة سليمة."))
        _p = doc.add_paragraph()
        _r = _p.add_run(_txt)
        _r.bold = True
        _r.font.color.rgb = RGBColor(0xCC, 0x00, 0x00)
    text = _degraded_banner_text(view, lang)
    if not text:
        return
    p = doc.add_paragraph()
    run = p.add_run(text)
...
def _truncate_at_word(text: str, max_len: int) -> str:
    ...
    لا «…» — الذيل القديم كان يُطلِق بوابة نصّ المُنتَج على docx العميل."""
    if len(text) <= max_len:
        return text
    cut = text[:max_len]
    sp = cut.rfind(" ")
    if sp > max_len * 0.5:  # لا تراجُع مفرط إن كانت الكلمة الأخيرة طويلة جداً
        cut = cut[:sp]
    return _avoid_mid_number(cut.rstrip(), text).rstrip("،؛,:") + " (مختصر)"
...
from silk_render import (_VERDICT_LABELS_AR, _verdict_label,  # noqa: E402
                         _verdict_tone)

import silk_i18n  # noqa: E402


# ── الموجة ٠: لغة التقرير في طبقة التصدير · report language in the exporters ─
#
# اللغة تصل المصدِّرات عبر **العرض نفسه** (`view["report_language"]` الذي يرفعه
# `build_view`) لا عبر وسيطٍ إضافيّ في كل توقيع — فمستهلكٌ ينسى تمريره يقع على
# العربية (الافتراض الآمن) لا على لغةٍ عشوائية.

# العلامة التجارية نفسها — تُكتب بحروف لغة التقرير، لا تُترجَم (اسمُ علَم).
_SILK_WORDMARK = {"ar": "سِلك", "en": "Silk"}


def _view_lang(view: object) -> str:
    """لغة تقرير هذا العرض — عربيّ ما لم يُصرَّح بغيره."""
    if isinstance(view, dict):
        return silk_i18n.normalize(view.get("report_language"))
    return silk_i18n.DEFAULT_LANG


def _T(key: str, lang: str = "ar", **fmt: object) -> str:
    """اختصارٌ محلّي لقاموس المصطلحات القانوني — كل نصٍّ قالبيّ يمرّ منه."""
    return silk_i18n.t(key, lang, **fmt)


def _lang_safe(text: object, lang: str) -> str:
    """أعِد النصّ إن كان بلغة التقرير، وإلا سلسلةً فارغة.

    **الفصل الصلب — الطبقة ٢ (بنيويّة).** بعضُ القوالب تُقحِم نصَّ دليلٍ خاماً
    داخل جملةٍ مصاغة (اسمُ قناة دخول، ملاحظةُ مصدر…). الدليلُ مكتوبٌ بلغة طبقة
    الأدلة الداخلية (عربية)، فإقحامُه في تقريرٍ إنجليزيّ يُنتج جملةً مختلطة.

    الحلّ **ليس** ترجمةً هنا (ذاك مسارُ الترجمة الممنوع، §5) بل **الإسقاط**:
    الجملةُ المصاغة تبقى كاملةَ المعنى بلا المقطع المُقحَم، والفجوةُ تبقى
    معلَنة. أسماءُ العلَم القصيرة (شركة/علامة) تمرّ لأنها ليست نثراً — وذاك
    ما يميّزه `foreign_prose_spans` بنيوياً لا بقائمةٍ مخمَّنة.
    """
    body = str(text or "").strip()
    if not body:
        return ""
    return "" if silk_i18n.foreign_prose_spans(body, lang) else body


def _lang_safe_name(name: object, lang: str) -> str:
    """اسمُ مصدرٍ بلغة التقرير — **بلا فقدان المصدر أبداً**.

    أسماءُ المصادر تصل أحياناً بوصفٍ فرعيّ بلغة طبقة الأدلة الداخلية
    («Eurostat (مسح ميزانية الأسرة)») — فيخرج سطرُ مرجعٍ إنجليزيّ بمقطعٍ
    عربيّ. إسقاطُ الاسم كلّه ممنوع (النسبُ مقدَّس، والمصدر يجب أن يُسمّى)،
    فيُسقَط **الوصفُ المُقوَّس وحده** ويبقى اسمُ العلَم ورابطُه. وإن لم يبقَ
    شيء، يُعاد الاسمُ كما هو — مصدرٌ باسمٍ مختلطٍ خيرٌ من مرجعٍ بلا اسم.
    """
    raw = str(name or "").strip()
    if not raw:
        return ""
    if not silk_i18n.foreign_prose_spans(raw, lang):
        return raw
    stripped = re.sub(r"\s*[(（][^)）]*[)）]", "", raw).strip(" —-–،,")
    if stripped and not silk_i18n.foreign_prose_spans(stripped, lang):
        return stripped
    return raw


def _apply_direction(doc, lang: str) -> None:
    """اضبط اتجاه المستند بلغته — RTL للعربية، وLTR افتراضيّ Word للإنجليزية.

    الإنجليزية **لا تمرّ** بطبقة RTL إطلاقاً: حقنُ `<w:bidi/>` ومحاذاة
    `start` وخطّ `cs` العربي على مستندٍ إنجليزيّ يقلب محاذاته ويستبدل خطّه بلا
    سبب. (مزلق `_set_rtl_paragraph` الموثَّق — «`start` لا `right`» — يخصّ
    الفقرات ثنائية الاتجاه وحدها ويبقى كما هو للعربية.)
    """
    if silk_i18n.is_rtl(lang):
        _apply_rtl(doc)


def _finalize_direction(doc, lang: str) -> None:
    """التمريرة الختامية للاتجاه — للعربية وحدها (انظر `_apply_direction`)."""
    if silk_i18n.is_rtl(lang):
        _finalize_rtl(doc)
...
# WP-5 §2 — قياسُ اتجاه الأقواس في الـPDF النهائي.
#
# **المقياسُ الأوّل كان مُعاكِسَ الإشارة (تدقيق 2026-08-27، البندان W-01/W-04).**
# كان يعدّ «قوساً افتتاحياً يتبعه فراغ/نهايةُ سطر» في النصّ **المستخرَج**
# مؤشِّرَ انعكاس. لكنّ استخراجَ نصٍّ عربيٍّ من PDF أنتجَته LibreOffice **فاقدٌ
# بطبيعته**: المستخرِج يعكس سلسلةَ المحارف **كاملةً** لقلب اتجاه السطر،
# فيقلب معها ترتيبَ المحارف **داخل** كلّ عنقودِ رباط، ويُزيح المحارفَ
# المحايدةَ (الأقواس) إلى رأس السطر. لذلك يعدّ المقياسُ القديم ظاهرةَ
# استخراجٍ لا عطلَ تصيير.
#
# **تصحيحٌ لصياغةٍ سابقةٍ هنا (2026-08-27، المرحلة الثانية):** كُتِب أوّلًا أنّ
# `ToUnicode` تحمل العناقيدَ بترتيبٍ **بصريّ**. فحصُ الخرائط نقض ذلك —
# هي **منطقيةٌ وسليمة**، والقلبُ كلُّه من المستخرِج. لا يغيّر ذلك حكمَ هذه
# البوّابة، لكنّ تفسيرًا خاطئًا في الشيفرة يُضلِّل قارئَها التالي.
# (العلاجُ الكامل لطبقة النصّ في `silk_pdf_textlayer`.)
#
# **مصفوفةُ المعايرة (نفسُ المحتوى، متغيّرٌ واحد، قياسٌ مباشر):**
#
# | البناء                          | المقياسُ القديم | المقياسُ الجديد |
# |---------------------------------|-----------------|------------------|
# | A — الوصفة (bidi+start+RLM)     | 4 ⇒ **يفشل** ✗  | مقلوب 0 ⇒ يمرّ ✓ |
# | B — بلا RLM                     | 4 ⇒ يفشل ✗      | مقلوب 0 ⇒ يمرّ ✓ |
# | C — jc=right                    | 4 ⇒ يفشل ✗      | مقلوب 0 ⇒ يمرّ ✓ |
# | D — **بلا bidi (المقلوب فعلاً)**| 1 ⇒ **يمرّ** ✗  | مقلوب 3 ⇒ **يفشل** ✓ |
#
# المقياسُ القديم خاطئٌ في الخلايا الأربع: يرفض المستندَ السليم ويقبل المقلوب.
# الجديدُ صحيحٌ في الأربع لأنه يقيس **الهندسة** لا سلسلةَ المحارف: في فقرةٍ
# أساسُها RTL يُصيَّر القوسُ الافتتاحيُّ المنطقيّ `(` معكوسَ الشكل على **يمين**
# مقطعه، فترتيبُ محارف الأقواس على المحور x (يسار⇐يمين) يكون `)(`؛ وحين ينقلب
# المقطعُ إلى LTR يصير `()`. المرجعُ البصريّ لهذا الحكم مُثبَّتٌ بلقطاتٍ في
# سجلّ القرار (`docs/DEEP_RESEARCH_DECISIONS.md`).
#
# Old metric measured a lossy-extraction artifact and was anti-correlated:
# it failed the correct build and passed the genuinely inverted one. The new
# one measures glyph geometry instead — right in all four calibration cells.

_SUSPICIOUS_OPEN_BRACKET_RE = re.compile(r"\((?=\s|$)", re.M)

# نطاقاتُ الحروف العربية — تُستعمَل لتمييز الصفوف عربيةِ الأغلبية (سطرٌ
# لاتينيُّ الأغلبية أساسُه LTR، فترتيبُ `()` فيه هو **الصحيح**).
_AR_LETTER_RANGES = ((0x0600, 0x06FF), (0x0750, 0x077F), (0x08A0, 0x08FF),
                     (0xFB50, 0xFDFF), (0xFE70, 0xFEFF))

# أزواجُ الأقواس المقيسة: (افتتاحيّ منطقيّ، ختاميّ منطقيّ).
_BRACKET_PAIRS = (("(", ")"), ("[", "]"))


def is_arabic_majority(text: str) -> bool:
    """سطرٌ عربيُّ الأغلبية إن كانت حروفُه العربية > اللاتينية و> صفر.

    الأرقامُ والترقيمُ لا تُحسَب — فسطرُ `080410` أو `UN Comtrade` ليس عربيًّا،
    وأساسُه LTR فلا يُقاس عليه اتجاهُ الأقواس."""
    ar = sum(1 for c in text
             if any(lo <= ord(c) <= hi for lo, hi in _AR_LETTER_RANGES))
    la = sum(1 for c in text if c.isascii() and c.isalpha())
    return ar > 0 and ar > la


def count_suspicious_brackets(text: str) -> int:
    """عددُ الأقواس الافتتاحية المعلّقة (يتبعها فراغ/نهايةُ سطر) في نصٍّ
    مستخرَج من PDF.

    ⚠ **تشخيصيّ فقط — ليس حَكَماً على التصيير.** أُثبِت بالقياس أنّ هذا العدد
    يرتفع على مستنداتٍ **سليمةِ التصيير** (تقطيعُ المقاطع مختلطةِ الاتجاه في
    الاستخراج، وكلُّ رابطٍ في ملحق الأثر يزيده واحداً) وينخفض على مستندٍ
    **مقلوبٍ فعلاً**. البوّابةُ تستعمل `bracket_orientation_counts` بدلَه.
    Diagnostic of the text layer only — never a rendering verdict."""
    return len(_SUSPICIOUS_OPEN_BRACKET_RE.findall(text or ""))


def classify_bracket_sequence(seq: str, opener: str, closer: str) -> tuple:
    """صنِّف تسلسلَ محارف الأقواس المرتَّبَ على المحور x لصفٍّ عربيّ واحد.

    يعيد `(سليم، مقلوب)`. في سياق RTL يظهر الزوجُ السليم `)(` (لأنّ القوس
    الافتتاحيّ المنطقيّ يُصيَّر على يمين مقطعه)، والمقلوبُ `()`.
    ...
    **الصفُّ غيرُ المتوازن يُتجاهَل كاملاً** (زوجٌ تشظّى عند لفّ السطر يترك
    ...
    for i in range(0, len(seq) - 1, 2):
        pair = seq[i:i + 2]
        if pair == closer + opener:
            ok += 1
        elif pair == opener + closer:
            inverted += 1
    return ok, inverted
...
def _pdf_bracket_check(pdf_path: str) -> None:
    """WP-5: افحص اتجاهَ الأقواس هندسياً في الـPDF النهائي — أيُّ زوجٍ مقلوبٍ
    فوق `SILK_PDF_BRACKET_FAIL_MAX` (افتراضياً **صفر**) يُفشِل التصديرَ بصوتٍ
    عالٍ بدل تسليم مستندٍ معكوس الأقواس.

    العتبةُ صارت صفراً (كانت ٣ على المقياس القديم): المقياسُ الجديد يعدّ
    **انقلاباً مُثبَتاً هندسياً**، وواحدٌ منه عطلٌ حقيقيّ — لا هامشَ ضجيجٍ
    يُتسامَح معه. بلا pymupdf يُتخطّى الفحصُ بسطرِ تشخيص (لا ادعاءَ فحصٍ لم
    يحدث)، وهي منذ تدقيق 2026-08-27 تبعيةُ إنتاجٍ في `requirements.txt`."""
    import os
    measured = bracket_orientation_counts(pdf_path)
    if measured is None:
        log.info("pdf bracket check skipped: لم يقع قياسٌ (لا pymupdf/تعذّر "
                 "الاستخراج) — لا حكمَ على هذا المستند")
        return
    ok, inverted, skipped = measured
    # مراجعة شيفرة PR #147: قيمة بيئة مشوَّهة كانت تُفجِّر ValueError خاماً
    # فتقتل كل تصدير PDF — تراجُع آمن للافتراضي بدل الانهيار.
    try:
        limit = int(os.environ.get("SILK_PDF_BRACKET_FAIL_MAX", "0"))
...
def _stamp_report_metadata(doc, view: dict, lang: str) -> None:
    ...
    الغرض التشغيلي: يُعرَف لاحقاً بأيّ لغةٍ وُلِّد هذا التقرير ولماذا، وبأيّ
    عقدِ مخطّطٍ ومحرّك — فمراجعةُ تقريرٍ قديمٍ تقرأ سياقَه لا سياقَ اليوم.

    خصائص الملف يحفظها تحويلُ LibreOffice إلى PDF، فتصل المصنوعَ النهائي.
    والسطرُ المعروض في المتن يجعلها مقروءةً لمن لا يفتح خصائص الملف.
    كلّ قيمةٍ **مقيسة** — لا حقلَ مختلَقاً: ما لا يُعرَف يُترَك فارغاً.
    """
    meta = view.get("report_meta") or {}
    # #13 ص18: رمز اللغة الخام («ar») على سطح عميل — القيمة المعروضة
    # باسم اللغة المقروء؛ رمزا نسختي المخطط والمحرك يبقيان (عقد تتبّع).
    _lang_names = {"ar": "العربية", "en": "English"}
    fields = [
        ("meta_report_language", _lang_names.get(lang, lang)),
        ("meta_factory_language",
         _lang_names.get(meta.get("factory_language") or lang,
                         meta.get("factory_language") or lang)),
        ("meta_study_id", meta.get("study_id")),
        ("meta_factory_id", meta.get("factory_id")),
    ...
    if not shown:
        return
    doc.add_heading(_T("meta_heading", lang), level=2)
    _add_table(doc, [_T("item", lang), _T("value", lang)],
               [[_T(k, lang), str(v)] for k, v in shown])


def _add_cover_wordmark(doc, branding: dict, lang: str = "ar") -> None:
    ...
    p = doc.add_paragraph()
    # صيد الفجوات ٣: الغلاف الإنجليزي كان يحمل «سِلك» دائماً بينما تعليق
    # `_SILK_WORDMARK` نفسه يقول «الشعار بلغة التقرير» (الترويسة كانت ملتزمة).
    _wm = _SILK_WORDMARK.get(lang, _SILK_WORDMARK["ar"])
    run = p.add_run(_shape_safe_ar(_wm) if lang == "ar" else _wm)
    run.bold = True
    run.font.size = Pt(28)
...
def _add_page_header_footer(doc, title: str, lang: str = "ar") -> None:
    ...
    **الموجة ٠ (§7).** التذييل كان يحقن هوية `config/branding.yaml` العربية
    في **كل صفحة** من كل تقرير — فتقريرٌ إنجليزيّ يخرج بتذييلٍ عربيّ على كلّ
    صفحة. لم يلتقطه فحصُ متن الـdocx (يقرأ الفقرات والجداول لا الترويسات)،
    والتقطه استخراجُ نصّ الـPDF: دليلُ أنّ الفحص يجب أن يقع على **المصنوع
    النهائي** لا على الكائن الوسيط. العربية تبقى من `branding` حرفياً؛
    الإنجليزية من قاموس المصطلحات."""
    branding = _load_branding()
    section = doc.sections[0]
    hp = section.header.paragraphs[0] if section.header.paragraphs \
        else section.header.add_paragraph()
    hp.text = _shape_safe_ar(title) if silk_i18n.is_rtl(lang) else title
    fp = section.footer.paragraphs[0] if section.footer.paragraphs \
        else section.footer.add_paragraph()
    if silk_i18n.is_rtl(lang):
        identity = _shape_safe_ar(branding["contact_footer"])
    else:
        identity = _T("footer_identity", "en")
    fp.add_run(identity + _T("footer_page", lang))
...
# ── الموجة Z · البند Z-03 — كلُّ تصنيفٍ له لون، وإلا سقط التسليم ─────────
# **حادثةٌ مُعادُ إنتاجها على الشيفرة المدموجة:** `JuryCommittee` تُصدِر
# «PRELIMINARY / INCONCLUSIVE» كلّما أعلنت بعثةٌ واحدة فجوةَ بيانات — وهي
# الحالةُ الجزئية العادية لا حالةً نادرة — فيُصنَّف الحكمُ `inconclusive`،
# ولا مدخلَ له هنا، فيرفع `_add_verdict_badge` **KeyError** ويسقط تصديرُ
# تقرير العميل (docx وPDF معاً) قبل أن يُكتَب حرفٌ واحد. أي: دراسةٌ بفجوةٍ
# مُعلَنة = لا تقرير إطلاقاً. لم تلتقطه الحزمةُ الهرمتية لأنّ مدوّناتِها
# كاملةُ التغطية دائماً، فلا تبلغ هذا الفرع.
#
# اللونُ زينةٌ لا حكم: نقصُه لا يجوز أن يمنع تسليمَ تقرير. لكلّ تصنيفٍ في
# `_VERDICT_LABELS_AR` مدخلٌ صريحٌ هنا، والقراءةُ بافتراضٍ محايد فوق ذلك،
# وقفلٌ في `tests/test_wave_z_one_verdict.py` يمنع تباعدَ الخريطتين ثانيةً.
_VERDICT_TEXT_COLORS = {"go": (0x1E, 0x7D, 0x32), "conditional": (0x00, 0x69, 0x5C),
                        ...
                        # حكمٌ حتميٌّ صادرٌ بتغطيةٍ ناقصة — كهرمانيٌّ باهت.
                        "inconclusive": (0x8D, 0x6E, 0x21),
                        # نبراتُ **حالةِ الأدلة** (الموجة B): ليست أحكاماً
                        # تجارية، فلونُها محايدٌ رماديٌّ مزرقّ لا أخضر/أحمر.
                        "data_complete": (0x37, 0x4A, 0x5A),
                        "data_partial": (0x5A, 0x5A, 0x64),
                        "data_absent": (0x70, 0x70, 0x70)}
_VERDICT_HIGHLIGHTS = {"go": "BRIGHT_GREEN", "conditional": "TEAL",
                       "preliminary": "BRIGHT_GREEN",
                       "watch": "YELLOW", "nogo": "RED"}


def _add_verdict_badge(doc, vtxt: str, lang: str = "ar") -> None:
    ...
    الموجة ٠: `tone` (ومنه اللون) يُشتقّ من الحكم الحتمي وحده مهما كانت اللغة
    — التسمية المكتوبة على الشارة هي ما يتبع لغة التقرير."""
    from docx.shared import Pt, RGBColor
    tone = _verdict_tone(vtxt)
    p = doc.add_paragraph()
    run = p.add_run(f"  {_verdict_label(tone, lang)}  ")
    run.bold = True
    run.font.size = Pt(16)
    # لا `KeyError` عند حدّ التسليم: تصنيفٌ بلا لونٍ يأخذ المحايد (Z-03).
    rgb = _VERDICT_TEXT_COLORS.get(tone) or _VERDICT_TEXT_COLORS["unknown"]
...
# الموجة ٠: مرايا إنجليزية لبوابة اتساق الحكم — **نفس البوابة بنفس الصرامة**
# في اللغتين. صياغتا «Recommendation:» و«Decision:» هما ما يكتبه المُصدِّر
# الإنجليزي فعلاً (`recommendation_line`)، وعمودا الجدول مثلهما.
_VERDICT_DECL_RE_EN = re.compile(r"^(?:Decision|Recommendation)\s*:\s*(.+)$",
                                 re.I)
_VERDICT_COL_HEADS = {
    "ar": ("الحكم", "التوصية"),
    "en": ("Decision", "Recommendation"),
}


def _verdict_labels_for(lang: str = "ar") -> dict:
    """خريطة tone→تسمية بلغة التقرير — مصدرٌ واحد يستهلكه فاحص الاتساق."""
    if silk_i18n.normalize(lang) == "ar":
        return dict(_VERDICT_LABELS_AR)
    return {tone: _verdict_label(tone, "en") for tone in _VERDICT_LABELS_AR}


def _verdict_decl_re(lang: str = "ar"):
    """نمط سطر التصريح بالحكم بلغة التقرير."""
    return (_VERDICT_DECL_RE_EN if silk_i18n.normalize(lang) == "en"
            else _VERDICT_DECL_RE)


def _match_known_verdict_label(s: str, lang: str = "ar") -> "str | None":
    """صنِّف نصّاً كتسمية حكمٍ معروفة إن كان **يبدأ** بإحدى تسميات الحكم
    (مطابقة بادئة لا مساواة سطر كامل) — سطر «مختصر» قد يُلحِق سياقاً بعد
    التسمية («مراقبة السوق — سوق الكويت (بحث عميق شامل)») بلا أن يكون هذا
    تناقضاً؛ نصٌّ لا يبدأ بأيّ تسمية معروفة يُهمَل (ليس تصريحاً بحكمٍ أصلاً)
    بدل تصنيفه تعارضاً زائفاً."""
    s = s.strip()
    for lbl in _verdict_labels_for(lang).values():
...
def _resolve_vtxt(dr: dict) -> str:
    ...
    # ── الموجة Z · البند Z-02 — التصنيفُ المُصالَح يسبق الرمزَ الخام ──────
    # الموجة B أدخلت `_coverage_basis_tone` في **موضعٍ واحد** فقط
    # (`silk_render`)، فصار العرضُ الواحد يحمل شارةً صادقة على الويب
    # («اكتمل البحث — لم يصدر تقييمٌ تجاريّ بعد») وغلافَ docx/PDF يقول
    # «توصية أولية بالدخول» — لأنّ هذه الدالةَ تُعيد الاشتقاق من الرمز الخام
    # فتتخطّاها. مُثبَتٌ بإعادة إنتاج على تقرير العميل المُسلَّم.
    #
    # `verdict_tone` في العرض هو الحقلُ الواحد المُصالَح (حكمُ المحرّك حين
    # وُجد، وحالةُ التغطية حين لم يوجد)؛ و`_verdict_tone` تُعيد مفتاحَ
    # التصنيف كما هو، فيصل كلَّ مواضع العرض الستّة بلا تعديلِ أيٍّ منها.
    tone = str((dr or {}).get("verdict_tone") or "")
    if tone and tone in _VERDICT_LABELS_AR:
        return tone
...
def _declared_verdict_labels(doc, lang: str = "ar") -> list[str]:
    """كل ذكرٍ تصريحيّ لتسمية الحكم في المواضع الثلاثة القابلة للفحص داخل
    كائن docx: شارة الغلاف (فقرة نصّها بالضبط إحدى تسميات الحكم)، سطر
    «الحكم:»/«التوصية:» في متن الفقرات، وصفّ جدول أول عموده «الحكم» أو
    «التوصية».

    الموجة ٠: مجموعةُ التسميات ومفاتيحُ الجدول تتبعان لغة التقرير — البوابة
    نفسها بنفس صرامتها في اللغتين، لا تعمى على تقريرٍ إنجليزيّ."""
    out: list[str] = []
    labels = set(_verdict_labels_for(lang).values())
    for p in doc.paragraphs:
        t = p.text.strip()
        if not t:
            continue
        if t in labels:  # شارة الغلاف: تطابق تامّ (فقرة قصيرة مستقلة) لا بادئة
            out.append(t)
            continue
        m = _verdict_decl_re(lang).match(t)
        if m:
            lbl = _match_known_verdict_label(m.group(1), lang)
            if lbl:
                out.append(lbl)
    for table in doc.tables:
        for row in table.rows:
            cells = [c.text.strip() for c in row.cells]
            if len(cells) >= 2 and cells[0] in _VERDICT_COL_HEADS[
                    silk_i18n.normalize(lang)]:
                lbl = _match_known_verdict_label(cells[1], lang)
                if lbl:
                    out.append(lbl)
    return out


def _assert_verdict_consistency_doc(doc, vtxt: str, where: str,
                                    lang: str = "ar") -> None:
    ...
    canonical_label = _verdict_label(_verdict_tone(vtxt), lang)
...
# ── الموجة Z · البند Z-04 (تكملةٌ من المراجعة الذاتية §٥٨) ───────────────
# كانت هنا خريطةُ أسماءِ أعمدةٍ **ثانية** تخالف `silk_decision._AR` في ثلاثةٍ
# من خمسة («المنافسة» مقابل «شدة المنافسة»، «التنظيم» مقابل «الملاءمة
# التنظيمية»، «الربحية» مقابل «هامش الربحية»)، فيحمل تقريرا الدراسة الواحدة
# اسمين للعمود نفسه — وهو بعينه تباعدُ الخريطتين الذي أضافت له هذه الموجة
# قفلاً في موضعٍ آخر. المصدرُ الآن واحد: `silk_decision.pillar_label`.
def _pillar_ar(key: str) -> str:
    from silk_decision import pillar_label
    return pillar_label(key, "ar")


def _missing_ar(keys) -> str:
    """مكوّناتٌ غائبة بلغةِ قارئ — نفسُ مُعرِّب `silk_decision` لا نسخةٌ ثانية."""
    from silk_decision import part_labels
    return part_labels(keys, "ar") or "—"
...
def _f_src_bare(f: dict | None) -> str:
    ...
    for s in (f or {}).get("sources") or []:
        seg = str(s.get("source") or "غير متاح")
...
def render_brief(view: dict, dashboard_url: str = "/") -> str:
    ...
    # بلاغ تحليل 20: المختصر يحمل سطر «غير مكتمل» أيضاً — لا يُفلِت أيّ مشتقّ.
    # بلغة التقرير (مراجعة §58 #5)، ويتكيّف مع الاقتطاع منتصفَ الجملة (#2).
    _rep_brief = (dr or {}).get("report") or {}
    if _rep_brief.get("incomplete"):
        _blang = _view_lang(view)
        _miss_b = ((", " if _blang == "en" else "، ").join(
            m for m in (_rep_brief.get("missing_sections") or []) if m))
        if _blang == "en":
            L.append((f"⚠ INCOMPLETE REPORT — missing sections (declared gaps, "
                      f"not findings): {_miss_b}") if _miss_b else
                     "⚠ INCOMPLETE REPORT — truncated before completion")
        else:
            L.append((f"⚠ تقرير غير مكتمل — أقسام غائبة (غيابها معلَن، لا "
                      f"نتائج فيها): {_miss_b}") if _miss_b else
                     "⚠ تقرير غير مكتمل — اقتُطِع قبل اكتماله")
...
def _docx_entry_strategy(doc, m: dict) -> None:
    ...
    # الموجة 2ب: المقياس الموحّد 0–10000 — العتبة من silk_economics حصراً.
    from silk_economics import hhi_is_high
    concentrated = hhi_is_high(hhi)
...
def _docx_entry_decision(doc, m: dict) -> None:
    ...
    # الموجة ٥ (الجزء ٦-١): القاعدة قبل الحكم.
    if ed.get("decision_rule"):
        doc.add_paragraph(str(ed["decision_rule"]))
    ...
    _add_table(
        ...
        [[_pillar_ar(key), _fmt(p.get("value")), p.get("basis"),
          _missing_ar(p.get("missing"))]
    ...
    # الموجة ٥ (الجزء ٦-٥): الحجة المضادة + سبب ردّها.
    cc = ed.get("counter_case") or {}
    if cc.get("case"):
        doc.add_paragraph(f"الحجة المضادة: {cc['case']}")
        if cc.get("rebuttal"):
            doc.add_paragraph(str(cc["rebuttal"]))
    if ed.get("note"):
...
def _docx_pricing_layers(doc, m: dict) -> None:
    ...
    if refs:
        ...
        doc.add_paragraph("مراجع الأسعار: غير متاحة")
...
# عنوانُ قسم المنهجية في اللغتين — المطابقةُ الحرفية العربية كانت تُسقِط
# قناةَ ملاحظات البوّابة كلَّها على تقريرٍ إنجليزيّ (مراجعة ذاتية §58).
_MMETHODOLOGY_HEAD_RE = re.compile(r"منهجية|methodolog", re.I)
...
def _docx_deep_research(doc, view: dict) -> None:
    ...
    # البند 18 (موجة سدّ الفجوات F5): صندوق التحذير الواحد أعلى القسم —
    # نفس صندوق build_view، مرة واحدة لا مكرراً في كل قسم.
    _cbox_deep = str(dr.get("caveat_box") or "")
    if _cbox_deep:
        doc.add_paragraph(f"⚠ {_cbox_deep}", style="Intense Quote")
    ...
    if dr.get("report", {}).get("text"):
        ...
        while i < n:
            ...
            if line.startswith("## "):
                ...
                # **مراجعة ذاتية §58.** المطابقةُ كانت على «منهجية» حرفياً،
                # وعنوانُ القسم في تقريرٍ إنجليزيّ «Research Methodology and
                # Scope» — فقناةُ `methodology_notes` كلُّها (ومستهلكُها
                # الوحيد هو هذا الموضع) تسقط صامتةً على كلّ تشغيلةٍ
                # إنجليزية. المطابقةُ باللغتين، والعنوانُ الفرعيّ من
                # `silk_i18n` لا سلسلةً عربيةً مثبَّتة.
                if _MMETHODOLOGY_HEAD_RE.search(heading_text):
                    notes = ((dr.get("quality_gate") or {})
                            .get("methodology_notes") or [])
                    if notes:
                        # العنوانُ يبقى عربياً كبقيّة عناوين هذا القسم:
                        # مصنوعُ المشغّل عربيٌّ بالكامل، وعنوانٌ إنجليزيٌّ
                        # وحيدٌ بين عربيّاتٍ قسمٌ مهجَّن (مراجعة ذاتية §58،
                        # الجولة ٢). العطلُ المُصلَح هو **سقوطُ القناة**
                        # لا لغةُ عنوانها.
    ...
    for cat, ar_label in _CATEGORY_AR.items():
        ...
        items = by_cat.get(cat) or []
        if not items:
            doc.add_paragraph("دليل هذا التقاطع غير متاح في هذا التشغيل — "
                              "لم يُبنَ استنتاج")
            continue
        for f in items:
...
ECONOMICS_HEADING = "النموذج الاقتصادي — التكلفة الواصلة وأقصى سعر مصنع منافس"


def _economics_md_lines(dr: dict) -> list[str]:
    """قسم الاقتصاد (الموجة ٣ §5.4) بصيغة Markdown — من `dr["economics"]`
    حصراً (نموذج العرض القانوني)؛ غيابه = لا قسم (لا هيكل فارغ)."""
    eco = dr.get("economics") or {}
    rs = eco.get("reverse_solve") or {}
    _gaps = [g for g in (eco.get("gaps") or []) if str(g).strip()]
    # **الموجة B (البند E-03).** كان غيابُ الحلّ العكسيّ يحجب **القسمَ
    # كلَّه ومعه الفجواتِ المسمّاة** التي كتبها `silk_economics.economics_view`
    # بعناية («لا سعر رف منافس مرصود…»، «تكلفة المصنع (EXW) غير مدخلة…»).
    # فيصير الغيابُ **صامتاً** بعد أن كان معلَناً — خرقٌ مباشر لعقد
    # «فجوةٌ معلنة لا اختلاق». الآن: بلا نموذجٍ **تُعرَض الفجواتُ وحدَها**.
    # البند ٧: جدول أرقام القرار يُبنى مرة واحدة ويظهر في **المسارين** —
    # غياب الحل العكسي (لا سعر رف مرصود) هو تحديداً حال الفجوات الثلاثية
    # التي يحملها الجدول؛ حجبُه معها يعيد عيب E-03 نفسه.
    dn_lines: list[str] = []
    ...
    if not rs:
        if not _gaps and not dn_lines:
            return []
        return ([f"## {ECONOMICS_HEADING}", ""]
                + [f"- {g}" for g in _gaps] + dn_lines + [""])
    L = [f"## {ECONOMICS_HEADING}", ""]
    anchor = eco.get("anchor_price") or {}
    if anchor:
        cur = anchor.get("currency") or "بالعملة المحلية المرصودة"
        L.append(f"- مرساة السعر: أدنى سعر رف منافس مرصود "
                 f"{anchor.get('per_unit')} {cur} "
                 f"({anchor.get('source') or '—'}، أساس التقييم: تجزئة).")
        for g in anchor.get("gaps") or []:
            L.append(f"  - {g}")
    # البند 5: الرقم بعملته وأساسه («دينار/كجم») — رقم عارٍ غير قابل للتدقيق؛
    # عروضٌ مخزَّنة قبل الإصلاح بلا الحقلين، فالسطر يتدهور بسلام بلا أساس.
    _basis = "/".join(x for x in (str(rs.get("currency") or "").strip(),
                                  str(rs.get("unit") or "").strip()) if x)
    L.append(f"- **أقصى سعر مصنع قابل للمنافسة: {rs.get('max_exw')}"
             + (f" {_basis}" if _basis else "") + "** "
             f"(سيناريو {rs.get('headline_scenario')}). "
             f"المعادلة: {rs.get('formula')}")
    if rs.get("scenarios"):
        L += ["", "| السيناريو | شحن % | هامش الموزّع % | هامش التجزئة % | "
              "أقصى EXW |", "| --- | --- | --- | --- | --- |"]
        for s in rs["scenarios"]:
            L.append(f"| {s['scenario']} | {s['freight_pct']} | "
                     f"{s['distributor_pct']} | {s['retailer_pct']} | "
                     f"{s['max_exw']} |")
    if rs.get("parameters"):
        L += ["", "المعالم المعلنة (بافتراضٍ قابل للتعديل): "
              + "؛ ".join(rs["parameters"]) + "."]
    # البند 6: تناقض تسعيري محسوب — التحذير إلزامي على كل سطح عرض.
    if (eco.get("pricing_contradiction") or {}).get("note"):
        L += ["", f"**{eco['pricing_contradiction']['note']}**"]
    wf = eco.get("waterfall") or []
    if wf:
        L += ["", "| مرحلة التكلفة | القيمة | ملاحظة |",
              "| --- | --- | --- |"]
        for r in wf:
            L.append(f"| {r['name']} | {r['value']} | {r['note']} |")
...
def _docx_economics_section(doc, dr: dict) -> None:
    """قسم الاقتصاد في تصدير Word — جداول عربية من نفس نموذج العرض."""
    eco = dr.get("economics") or {}
    rs = eco.get("reverse_solve") or {}
    _gaps = [g for g in (eco.get("gaps") or []) if str(g).strip()]
    # **الموجة B (البند E-03).** كان غيابُ الحلّ العكسيّ يحجب **القسمَ
    ...
    if not rs:
        if not _gaps:
            return
        doc.add_heading(ECONOMICS_HEADING, level=1)
        for g in _gaps:
    ...
    if anchor:
        doc.add_paragraph(
            f"مرساة السعر: أدنى سعر رف منافس مرصود {anchor.get('per_unit')} "
            f"{anchor.get('currency') or 'بالعملة المحلية المرصودة'} "
            f"({anchor.get('source') or '—'}، أساس التقييم: تجزئة).")
    _basis = "/".join(x for x in (str(rs.get("currency") or "").strip(),
                                  str(rs.get("unit") or "").strip()) if x)
    doc.add_paragraph(
        f"أقصى سعر مصنع قابل للمنافسة: {rs.get('max_exw')}"
        + (f" {_basis}" if _basis else "") + " "
        f"(سيناريو {rs.get('headline_scenario')}) — {rs.get('formula')}")
    if rs.get("scenarios"):
        _add_table(doc, ["السيناريو", "شحن %", "هامش الموزّع %",
                         "هامش التجزئة %", "أقصى EXW"],
                   [[s["scenario"], s["freight_pct"], s["distributor_pct"],
                     s["retailer_pct"], s["max_exw"]]
                    for s in rs["scenarios"]])
    if rs.get("parameters"):
        doc.add_paragraph("المعالم المعلنة (بافتراضٍ قابل للتعديل): "
                          + "؛ ".join(rs["parameters"]) + ".")
    # البند 6: تناقض تسعيري محسوب — التحذير إلزامي على كل سطح عرض.
    if (eco.get("pricing_contradiction") or {}).get("note"):
        doc.add_paragraph(str(eco["pricing_contradiction"]["note"]))
    wf = eco.get("waterfall") or []
    if wf:
        _add_table(doc, ["مرحلة التكلفة", "القيمة", "ملاحظة"],
                   [[r["name"], r["value"], r["note"]] for r in wf])
    for g in eco.get("gaps") or []:
...
def _render_research_docx(doc, view: dict) -> None:
    ...
    # النموذج الاقتصادي (الموجة ٣ §5.4) — قبل متن البحث العميق.
    _docx_economics_section(doc, dr)
...
_CLIENT_FORBIDDEN_PATTERNS = [
    ...
    # #13 ص13-14: لغة مدخلات النظام وسقالة الاستشهاد بلغتا PDF العميل —
    # «بطاقة المنتج/منتجك» (يُطلب المعطى بلغة الزائر)، وتوقيعة السقالة
    # «[وسم] مبني على:» (لا «مبني على» العارية — نثر مشروع)، والمرجع
    # الداخلي «مرجع locale».
    ("product_card", re.compile(r"بطاقة\s+(?:ال)?منتج")),
    ("citation_scaffold", re.compile(r"\[[^\]\n]{1,40}\]\s*مبني على:")),
    ("locale_ref", re.compile(r"مرجع\s+locale|\blocale\b", re.I)),
...
_CLIENT_SANITIZE = [
    # هدف الدراسة الاحترافية (البند ٣): مصطلحات القياس العربية الداخلية
    # («الدرجة الموزونة»…) — قد تصل من نص كاتبٍ أو مدونة مخزَّنة قديمة؛
    # الزوج القانوني الواحد في silk_style_contract.CLIENT_TERM_REPLACEMENTS
    # (يستهلكه باني لوحة الأساس أيضاً — لا نسختان تتباعدان).
    *[(re.compile(term), repl) for term, repl in
      __import__("silk_style_contract").CLIENT_TERM_REPLACEMENTS],
    # #13 (دورة C4): تغطية مُطهِّر للإبر الثلاث الجديدة — الحارس بلا
    # استبدال مقابل هو بالضبط ما أنتج 501 الحي الثالث (عقد قفل التغطية).
    (re.compile(r"\[[^\]\n]{1,40}\]\s*مبني على:\s*"), ""),
    (re.compile(r"بطاقة\s+(?:ال)?منتج(?:ك)?(?:\s*\(product_card\))?"),
     "سعر المصنع المُدخَل"),
    (re.compile(r"مرجع\s+locale|\blocale\b", re.I), "مرجع السوق"),
    # رموز حالة HTTP لا تصل العميل (بلاغ الحليب–الأردن 2026-08-19: «خطأ 429»
    # ظاهرة نصاً في وجه العميل). المشغّل يبقى يرى الرمز — `_http_status_ar`
    # تحتفظ به عمداً لأن 429/503/404 أفعالٌ تشغيلية مختلفة — والتعريب هنا عند
    # حدّ العميل وحده. **مقيَّدٌ بسياق الخطأ** (HTTP/خطأ/استجابة الخادم) لا
    # بالرقم العاري: «بلغت الواردات 500 ألف دولار» رقمُ سوقٍ لا رمزُ حالة
    # (مراجعة ذاتية §58 — نفس سبب lookarounds في `_GAP_429_RE`).
    (re.compile(r"خطأ استجابة الخادم\s*\(HTTP\s*\d{3}\)\s*:?\s*"),
     "تعذّر مؤقت لدى المصدر: "),
    (re.compile(r"(?:خطأ\s+)?HTTP\s*429\b(?![\d.,])"),
     "تجاوز مؤقت لحدّ الاستعلامات لدى المصدر"),
    (re.compile(r"خطأ\s*(?<![\d$])429(?![\d.,هM])"),
     "تجاوز مؤقت لحدّ الاستعلامات لدى المصدر"),
    (re.compile(r"(?:خطأ\s+)?HTTP\s*(?:500|502|503|504)\b(?![\d.,])"),
     "تعذّر مؤقت لدى المصدر"),
    (re.compile(r"خطأ\s*(?<![\d$])(?:500|502|503|504)(?![\d.,هM])"),
     "تعذّر مؤقت لدى المصدر"),
...
def _plain_language(text: str) -> str:
    """لغة التاجر على سطح العميل — يُستبدَل المصطلح بمعناه ويُحذف الشرح
    المقحوم بين قوسين (بلاغ المالك: «التقرير بلغة غير مفهومة»).

    **محافظٌ عمداً**: لا يمسّ اسم شركةٍ ولا رابطاً ولا معرّفَ مؤشرٍ منقّطاً —
    «SAM Food Trading BV» شركةٌ لا مصطلح، و«LP.LPI.OVRL.XQ» معرّفُ سلسلة.
    الاستبدال يقع فقط حين يكون المصطلح داخل نثرٍ عربيّ (مراجعة ذاتية §58).
    """
    # أسماءُ الأنظمة (TRACES/CHED/EORI) تبقى بلا وصفٍ مقحوم: التطهيرُ يجري
    # لكلّ فقرةٍ على حدة فلا سبيل لمعرفة «أوّل ذكر»، فكان الوصفُ يتكرّر في
    # كلّ فقرة ويعيد الكسرَ الذي وُجد التبسيطُ ليزيله (مراجعة ذاتية §58).
    # شرحُها يعيش في المسرد وحده.
    from silk_style_contract import GLOSSARY, PLAIN_TERMS
    s = str(text or "")
    if not s.strip() or not plain_language_enabled():
        return s
    # عالج كل سطر، واحمِ الرابط نفسه داخل السطر لا النثر المحيط به.
    # Preserve sources without exempting the prose that cites them.
    if "\n" in s:
        return "\n".join(_plain_language(line) for line in s.split("\n"))
    if "://" in s or "www." in s:
        # احمِ الرابط وحده: استثناء السطر كله كان يترك مصطلحاته التقنية خاماً.
        # Preserve URL bytes while simplifying the surrounding cited prose.
        parts = re.split(r"((?:[A-Za-z][A-Za-z0-9+.-]*://|www\.)[^\s<>]+)", s)
        if len(parts) > 1:
            return "".join(part if i % 2 else _plain_language(part)
                           for i, part in enumerate(parts))
        return s
    # (١) احذف الشرح المقحوم لأي مصطلح في المسرد — نصّه معروف حرفياً.
    for gloss in GLOSSARY.values():
        for variant in (f" ({gloss})", f"({gloss})", f" — {gloss}"):
            s = s.replace(variant, "")
    # (٢) استبدل المصطلح بمعناه ضمن نثرٍ عربيّ فقط.
    for term, plain in PLAIN_TERMS.items():
        ...
        while True:
            ...
            if not m:
                break
            # نافذةٌ أوسع للسياق العربي: ٣٠ حرفاً كانت تقطع أوّل كلمةٍ في
            # العبارة فيفوت التقاطُ أن الكاتب سمّى المصطلح (مراجعة ذاتية).
            before = s[max(0, m.start() - 60):m.start()]
            after = s[m.end():m.end() + 30]
            if not _is_arabic_prose_context(before, after):
                pos = m.end()          # اسمُ شركةٍ/معرّفٌ — اتركه كما هو
                continue
            if _already_named_in_arabic(before, plain):
                s = s[:m.start()] + s[m.end():]   # سمّاه الكاتب — احذف الاختصار
                pos = m.start()                   # لا إعادة مسحٍ من موضعٍ قديم
            else:
                s = s[:m.start()] + plain + s[m.end():]
                pos = m.start() + len(plain)
    # (٣) نظافة أقواس فرغت بعد الحذف ومسافات مكرّرة.
    s = re.sub(r"\(\s*\)", "", s)
    # فراغٌ متخلّف بعد حذف الاختصار: «… السوق ≈2350» / «… (‏ ≈2350)».
    s = re.sub(r"\(\s+", "(", s)
    s = re.sub(r"\s+([)≈%،.])", r"\1", s)
    return re.sub(r"[ \t]{2,}", " ", s)


# حروفٌ عربية — وجودُها حول المصطلح دليلُ نثرٍ لا اسمِ علَم.
_ARABIC_RE = re.compile(r"[\u0600-\u06FF]")
# كلماتٌ شائعة لا تصلح دليلاً على أن الكاتب سمّى المصطلح («الذي»، «يمكن»…).
_COMMON_WORDS = frozenset({
    "الذي", "التي", "يمكن", "من", "في", "على", "هذا", "هذه", "كله", "أن"})


def _is_arabic_prose_context(before: str, after: str) -> bool:
    """المصطلح داخل نثرٍ عربيّ؟ — لا اسمَ شركةٍ لاتينيّ ولا معرّفاً منقّطاً."""
    # معرّفٌ منقّط (LP.LPI.OVRL.XQ) أو جزءٌ من اسمٍ لاتينيّ مركّب.
    if before.endswith(".") or after.startswith("."):
        return False
    if re.match(r"\s+[A-Z][A-Za-z]", after):      # «SAM Food Trading»
        return False
    if re.search(r"[A-Za-z]\s*$", before):        # «Trading SAM»
        return False
    return bool(_ARABIC_RE.search(before) or _ARABIC_RE.search(after))


def _already_named_in_arabic(before: str, plain: str) -> bool:
    """هل سمّى الكاتبُ المصطلحَ بالعربية قبله مباشرةً؟ — كلماتٌ مميّزة فقط،
    فلا يُحذف الوسمُ بسبب «الذي»/«يمكن» الشائعتين (مراجعة ذاتية §58)."""
    # المطابقةُ على جذرٍ من ثلاثة أحرف: «خدمته» و«يخدمه» كلمةٌ واحدة عند
    # القارئ، والتصريفُ لا يغيّر أن الكاتب سمّى المصطلح فعلاً.
    stems = {w[:3] for w in plain.split()
             if len(w) > 3 and w not in _COMMON_WORDS}
    return len(stems) >= 2 and sum(1 for st in stems if st in before) >= 2


def plain_language_enabled() -> bool:
    """صمّام الطبقة (الدرس ٩١) — الافتراض ON؛ الإطفاء يعيد سلوك ما قبلها."""
    import os as _os
    raw = _os.environ.get("SILK_PLAIN_LANGUAGE_ENABLED", "").strip().lower()
    return raw not in ("0", "false", "no", "off")


def _client_sanitize(text: object, lang: str = "ar") -> str:
    ...
    **الموجة ٠.** كلٌّ من `_plain_language` و`_CLIENT_SANITIZE` يستبدل
    بالعربية (`confidence`→«التوثيق»، `verdict`→«التوصية»، HHI→«مؤشر تركّز
    السوق»). تشغيلُهما على نصٍّ إنجليزيّ يمسخ نثراً سليماً ويحقن العربية فيه.
    فالمسار الإنجليزيّ يستعمل مرآتَه (`_CLIENT_SANITIZE_EN`) التي تحرس نفس
    المعنى بأنماطٍ لا تصطدم بلغة الأعمال الشرعية."""
    s = str(text or "")
    if silk_i18n.normalize(lang) == "en":
        for pat, repl in _CLIENT_SANITIZE_EN:
            s = pat.sub(repl, s)
        return re.sub(r"[ \t]{2,}", " ", s).strip()
    # لغة التاجر (قرار المالك 2026-08-19) — **قبل** بقية التنقية: يُحذَف
    # الشرح المقحوم بين قوسين ويُستبدَل المصطلح بمعناه، فلا تنكسر الجملة.
    s = _plain_language(s)
...
# (`status`, `run`, `call`, `successful`, `missions`) لأنّ ظهورها في تقريرٍ
# عربيّ = تسريبُ سباكةٍ لا محالة. لكنّ التقرير الإنجليزيّ الشرعيّ يستعمل هذه
# الكلمات نفسها في سياقٍ تجاريٍّ سليم («regulatory status»، «run rate»،
# «a successful entry») — فتطبيقُ القائمة العربية عليه يحجب نثرَه هو.
#
# المرآة تحرس **نفس المعنى** لا نفس الحروف: تسريب السباكة، ولغة الخوارزمية،
# واسم المزوّد الداخلي، ودور الوكيل، ورموز الاستشهاد، وعلامة القسم الداخلية.
# الفرق أنّ الأنماط مقيَّدةٌ بسياقها فلا تصطدم بلغة الأعمال الطبيعية.
_CLIENT_FORBIDDEN_PATTERNS_EN = [
    ("mission", re.compile(r"\bmissions?\b", re.I)),
    # «status» وحدها مشروعة («regulatory status») — الممنوع سياقُ السباكة.
    ("status", re.compile(r"\b(?:mission|run|agent|job|task)\s+status\b|"
                          r"\bstatus\s*[:=]\s*(?:ok|failed|success)", re.I)),
    ("successful", re.compile(r"\b(?:mission|run|call|agent)\s+"
                              r"(?:was\s+)?successful\b", re.I)),
    # «run» وحدها مشروعة — الممنوع الإشارة إلى تشغيلة النظام.
    ("run", re.compile(r"\b(?:this|the|current)\s+run\b|"
                       r"\brun\s+(?:failed|succeeded|id)\b", re.I)),
    ("call", re.compile(r"\b(?:tool|api|llm|model)\s+calls?\b", re.I)),
    ("tool_name", re.compile(
        r"\b(?:" + "|".join(_CLIENT_TOOL_NAMES) + r")\b")),
    # اسم المزوّد الداخلي لاتينيّ أصلاً — نفس الحارس يعمل في اللغتين.
    ("vendor_name", _CLIENT_VENDOR_RE),
    ("agent_role", re.compile(
        r"\bthe\s+(?:master\s+)?analyst\s+agent\b|\breport\s+writer\b|"
        r"\bquality\s+gate\b|LLMMissionAgent|LLMAgent", re.I)),
    ("citation_plumbing", re.compile(
        r"\bdatapoints?\b|\bdp\d+\b|\bciting\s+dp\b|"
        r"\b(?:un)?cited\s+finding\b", re.I)),
    # لغة الخوارزمية: الرمز الخام والدرجة الرقمية — لا كلمة «confidence»
    # المفردة (تسميةُ نطاقٍ مشروعةٌ على سطح العميل الإنجليزي: «high confidence»).
    ("algorithm_language", re.compile(
        r"\bverdict\b|\bconfidence\s+score\b|\bnumeric\s+score\b|"
        r"\btotal[_\s]score\b|\bconfidence\s*[:=]\s*0?\.\d", re.I)),
    ("section_marker", re.compile(r"§(?:\s*[0-9][0-9A-Za-z.\-]*)?")),
]


# مرآةُ التنقية الإنجليزية — تحرس **نفس المعنى** (سباكة/خوارزمية/مزوّد) بلا
# مساسٍ بلغة الأعمال الشرعية. أقصرُ من نظيرتها العربية عمداً: أكثرُ بنود تلك
# القائمة تعريبُ مصطلحٍ إنجليزيّ داخل نصٍّ عربيّ — وهي بلا معنى هنا.
_CLIENT_SANITIZE_EN = [
    (re.compile(r"\bdp\d+\b|\[dp\d+\]", re.I), ""),
    (re.compile(r"\bLLM(?:Mission)?Agent:\s*[a-z_]+", re.I), "the research"),
    (re.compile(r"§\s*[0-9][0-9A-Za-z.\-]*"), ""),
    (re.compile(r"\bverdict\b", re.I), "recommendation"),
    (re.compile(r"\bconfidence\s+score\b", re.I), "confidence"),
    (re.compile(r"\b(?:total[_\s])?score\b", re.I), "rating"),
    (re.compile(r"\b(?:tool|api|llm|model)\s+calls?\b", re.I),
     "data collection"),
    (re.compile(r"\b(?:mission|run|agent|job|task)\s+status\b", re.I),
     "progress"),
    (re.compile(r"\bmissions?\b", re.I), "research"),
    (re.compile(r"\b(?:this|the|current)\s+run\b", re.I), "this study"),
]

# بدائلُ التنقية الإنجليزية — بدائلُ عربية داخل نصٍّ إنجليزيّ تسرّبٌ لا تنقية.
_CLIENT_REDACT_PLACEHOLDER_EN = {
    "mission": "the research",
    "status": "progress",
    "successful": "completed",
    "run": "this study",
    "call": "data collection",
    "tool_name": "a data source",
    "vendor_name": "a paid verification service",
    "agent_role": "the research",
    "citation_plumbing": "",
    "algorithm_language": "the assessment",
    "section_marker": "",
}
...
_CLIENT_REDACT_PLACEHOLDER = {
    ...
    # #13 (دورة C4): الإبر الثلاث الجديدة بلا صفوف كانت تسقط إلى «—»
    # فتُشوّه الجملة عند تصدير مدونة مخزنة قديمة.
    "product_card": "سعر المصنع المُدخَل",
    "citation_scaffold": "",
    "locale_ref": "مرجع السوق",
...
def _client_redact_text(text: str, lang: str = "ar") -> str:
    ...
    الموجة ٠: الأنماطُ **والبدائل** تتبعان اللغة — بدائلُ عربية داخل نصٍّ
...
def _redact_paragraph(par, lang: str = "ar") -> bool:
    ...
    if not _client_forbidden_hits(par.text, lang):
        return False
    redacted = _client_redact_text(par.text, lang)
    if par.runs:
...
def _client_redact_residual(doc, lang: str = "ar") -> bool:
    ...
    for par in doc.paragraphs:
        changed = _redact_paragraph(par, lang) or changed
    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
...
def _client_assert_clean(doc, lang: str = "ar") -> None:
    ...
    الموجة ٠: قائمة الأنماط تتبع لغة التقرير — الحارس يحرس نفس المعنى في
    اللغتين، ولا يحجب نثراً إنجليزياً شرعياً بأنماطٍ صيغت أصلاً لكشف تسرّبٍ
    إنجليزيٍّ داخل نصٍّ عربي."""
...
_CLIENT_SECTION_MAP = {
    # هدف الدراسة الاحترافية (البند ٤): الملخص قسم مستقل يفتتح المستند —
    # كان يُذاب داخل «القرار وأساسه» فيقرؤه العميل بعد الغلاف واللوحة.
    "الخلاصة التنفيذية": "الملخص التنفيذي",
...
# ── الموجة ٠: التمفصل اللغوي-المحايد · language-neutral section anchoring ───
#
# الخريطة أعلاه تُمفصِل على **العنوان العربي حرفياً**، وهو ارتباطٌ صلب بين
# موجّه الكاتب والمُصدِّر: كاتبٌ إنجليزيّ يُخرج «## 1. Executive Summary» فلا
# يطابق شيئاً، فينهار تقرير العميل الإنجليزي إلى أقسامٍ فارغة كلها.
#
# المرساة القانونية هي **الرقم الترتيبي** — موجودٌ أصلاً في بنية العنوان التي
# يفرضها الكاتب (`## N. العنوان`) ومحايدٌ لغوياً بطبعه. العنوان العربي يبقى
# مرساةً احتياطية كي تظلّ التقارير **المخزَّنة** (المولَّدة قبل هذه الموجة)
# قابلةً للتفكيك حرفياً كما كانت.
_CLIENT_SECTION_BY_ORDINAL = {
    1: "الملخص التنفيذي",            # الخلاصة التنفيذية (البند ٤: مستقل)
    2: None,                          # منهجية البحث ونطاقه — تُسقَط
    3: "السوق بالأرقام",              # نظرة عامة على السوق وحجمه
    4: "السوق بالأرقام",              # ديناميكيات السوق
    5: "السوق بالأرقام",              # تحليل المستهلك والطلب
    6: "المنافسة والتسعير والهامش",   # المشهد التنافسي
    7: "مسار الدخول والمتطلبات",      # التنظيم والوصول للسوق
    8: "مسار الدخول والمتطلبات",      # اللوجستيات وسلسلة الإمداد
    9: "المخاطر",                     # تقييم المخاطر
    10: "القرار وأساسه",              # التوصيات الاستراتيجية (تُقسَم عند الخارطة)
    11: None,                         # الملاحق — تُسقَط
}
# القسم العاشر (التوصيات) يُقسَم عند عنوانٍ فرعيّ — الرقم مرساته المحايدة.
_RECOMMENDATIONS_ORDINAL = 10

_WRITER_HEADING_RE = re.compile(r"^##\s+\d+\.\s*(.+?)\s*$")
# نفس النمط مع التقاط الرقم — المرساة المحايدة لغوياً.
_WRITER_HEADING_NUM_RE = re.compile(r"^##\s+(\d+)\.\s*(.+?)\s*$")
_ROADMAP_SUBHEAD_RE = re.compile(r"^###\s+خارطة طريق الدخول")
# المرساة الإنجليزية لخارطة الدخول — يفرضها عقد الكاتب الإنجليزي بنفس الصياغة.
_ROADMAP_SUBHEAD_EN_RE = re.compile(r"^###\s+Entry\s+roadmap", re.I)


# مفتاح كل عنوان عميلٍ في قاموس المصطلحات — العنوان العربي يبقى **المفتاح
# الداخلي** للجرار (`buckets`) كي لا يتغيّر أيّ منطقٍ قائم، والتسمية المعروضة
# تُشتقّ منه بلغة التقرير. مفتاحٌ داخليّ ≠ نصٌّ معروض.
_CLIENT_HEAD_TERM = {
    "الملخص التنفيذي": "sec_exec_summary",
    "القرار وأساسه": "sec_decision",
    "السوق بالأرقام": "sec_market_numbers",
    "المنافسة والتسعير والهامش": "sec_competition",
    "مسار الدخول والمتطلبات": "sec_entry_path",
    "المخاطر": "sec_risks",
}


def _client_head_label(head: str, lang: str = "ar") -> str:
    """التسمية المعروضة لقسم عميلٍ بلغة التقرير — العربية تبقى حرفياً كما هي."""
    key = _CLIENT_HEAD_TERM.get(head)
    return _T(key, lang) if key else head


def _client_section_for(ordinal: "int | None", title: str) -> "str | None":
    """قسمُ العميل الذي يستقبل قسمَ الكاتب — بالرقم أولاً ثم بالعنوان العربي.

    الرقم مرساةٌ محايدة لغوياً تعمل للعربية والإنجليزية معاً؛ العنوان العربي
    احتياطٌ للتقارير المخزَّنة قبل الموجة ٠ (وأيّ نصٍّ بلا ترقيمٍ صالح).
    """
    if ordinal in _CLIENT_SECTION_BY_ORDINAL:
        return _CLIENT_SECTION_BY_ORDINAL[ordinal]
    return _CLIENT_SECTION_MAP.get(title)
...
def _split_at_roadmap(body: list[str]) -> "tuple[list[str], list[str]]":
    ...
    الموجة ٠: يُقبَل العنوان الإنجليزي المكافئ («### Entry roadmap») الذي
    يفرضه عقد الكاتب الإنجليزي — نفس نقطة القسم الحتمية، بلغتين."""
    for idx, line in enumerate(body):
        stripped = line.strip()
        if (_ROADMAP_SUBHEAD_RE.match(stripped)
                or _ROADMAP_SUBHEAD_EN_RE.match(stripped)):
            return body[:idx], body[idx:]
    return body, []


def _parse_writer_sections(
        report_text: str) -> "list[tuple[str, list[str]]]":
    ...
    return [(title, body) for _n, title, body
            in _parse_writer_sections_numbered(report_text)]


def _parse_writer_sections_numbered(
        report_text: str) -> "list[tuple[int | None, str, list[str]]]":
    """نفس التقسيم مع **الرقم الترتيبي** — مرساة التمفصل المحايدة لغوياً.

    الموجة ٠: مستهلكو تقرير العميل يمفصلون على الرقم فيعمل التفكيك للعربية
    والإنجليزية معاً، بدل الارتباط الصلب بالعنوان العربي حرفياً.
    """
    sections: list[tuple[int | None, str, list[str]]] = []
    cur_num, cur_title, cur_body = None, None, []
    for line in str(report_text or "").splitlines():
        m = _WRITER_HEADING_NUM_RE.match(line.strip())
        if m:
            if cur_title is not None:
                sections.append((cur_num, cur_title, cur_body))
...
def _client_render_body_block(doc, lines: list[str],
                              lang: str = "ar") -> None:
    ...
    while i < n:
        ...
        if not raw:
            ...
            continue
        if raw.startswith("### "):
            doc.add_heading(_client_sanitize(raw[4:], lang), level=3)
            i += 1
            continue
        if raw.startswith("## "):  # عنوان قسم فرعي غير متوقّع — كفقرة مطهَّرة
            doc.add_paragraph(_client_sanitize(raw[3:], lang))
            i += 1
            continue
        if _is_markdown_table_row(raw):
            ...
            if _client_forbidden_hits(joined, lang):
                continue  # جدول درجات/تِلِمِتري — يُسقَط من متن العميل
            _render_markdown_table(doc, joined.splitlines())
            continue
        stripped = _strip_inline_markdown(_client_sanitize(raw, lang))
        if stripped.startswith("ماذا يعني هذا لقرارك:"):
            p = doc.add_paragraph()
            p.add_run(stripped).bold = True
        else:
...
def _hs_provenance_sentence(dr: dict, lang: str = "ar") -> str:
    ...
    if not isinstance(prov, dict) or not prov.get("hs6"):
        return ""
    # `note_ar` عربيٌّ بالبناء — يُسقَط في تقريرٍ إنجليزيّ بدل حقنه.
    note = _lang_safe(
        _client_sanitize(str(prov.get("note_ar") or ""), lang), lang).strip()
    return f" {note}" if note else ""


def _client_methodology_paragraph(dr: dict, lang: str = "ar") -> str:
    ...
    for m in missions.values():
        for f in (m.get("findings") or []) if isinstance(m, dict) else []:
            ...
            for raw_id in atomic_source_ids(f.get("source"), f.get("source_ids")):
                src = _client_sanitize(_clean_source_label(raw_id), lang)
                src = str(src or "").strip()
                if not src or src == "—" or _client_forbidden_hits(src, lang):
                    continue
    ...
    _sep = "، " if lang == "ar" else ", "
    src_list = _sep.join(sorted(
        display[n] for n, ok in contributed.items() if ok)[:6]) \
        or _T("methodology_sources_generic", lang)
    dates = sorted({str(f.get("retrieved_at"))
                    for m in missions.values()
                    if isinstance(m, dict)
                    for f in (m.get("findings") or [])
                    if f.get("retrieved_at")})
    date_txt = (_T("methodology_latest_date", lang, date=dates[-1]) if dates
                else _T("methodology_dates_in_log", lang))
    return (
        _T("methodology_paragraph", lang, sources=src_list, dates=date_txt)
        # كيف حُسِم رمزُ HS حين قِيس آلياً — يراه العميلُ لا المشغّلُ وحده.
        + _hs_provenance_sentence(dr, lang))
...
_CLIENT_GAP_WHAT = {
    ...
    "price_competitiveness": ("موقعك السعري الدقيق مقابل المنافسين",
                              "سعر المصنع بوحدة المنتج وأسعار منافسين موثقة "
                              "من متاجر السوق بنفس العملة ووحدة المقارنة"),
...
# الموجة ٠: مفاتيح الفجوات التي لها تسميتان في قاموس المصطلحات. القاموس
# العربي `_CLIENT_GAP_WHAT` أعلاه يبقى مرجعاً تاريخياً لاختباراتٍ قائمة.
_CLIENT_GAP_TERMS = frozenset(_CLIENT_GAP_WHAT)


def _client_confidence_section(doc, dr: dict,
                               lang: str = "ar") -> None:
    ...
    from silk_narrative import evidence_badge_for
    # #13 (تعميق المُشرِف): الجدول كان يعدّ شارات تحقق ويسمّيها تصنيفَ
    # مصادر («مصدر رسمي أوّلي» لعدّ ✓) — فظهر Comtrade/البنك الدولي «مصدر
    # عام» و«2 رسمي أولي» بجوار ثقة 80%. التسمية صارت بما يُعدّ فعلاً،
    # ورتبة ○ انقسمت: قيمة غائبة (فجوة معلنة) ≠ قيمة حاضرة لم يكتمل
    # التحقق منها — كلتاهما كانت «معطى ناقص» زوراً للثانية.
    counts = {"verified": 0, "secondary": 0, "unverified": 0, "missing": 0}

    def _tally(f) -> None:
        badge = evidence_badge_for(f)
        value = (f.get("value") if isinstance(f, dict)
    ...
    doc.add_heading(_T("coverage_heading", lang), level=1)
    doc.add_paragraph(_T("coverage_intro", lang, total=total,
                         pct=verified_pct))
    # سطر تغطية المصادر المسمّاة الحقيقي (compute_source_coverage) — البعد
    # الثاني الذي غيابه جعل «2 رسمي أولي» يُقرأ انهيارَ إسنادٍ كاملاً.
    try:
        from silk_source_coverage import compute_source_coverage
        cov = compute_source_coverage(dr)
        if cov["total"]:
            doc.add_paragraph(_T("coverage_named_sources", lang,
                                 pct=round(cov["pct"]),
                                 backed=cov["backed"], n=cov["total"]))
    except Exception:  # noqa: BLE001 — سطر إثرائي، غيابه لا يسقط القسم
        pass
    _add_table(doc, [_T("coverage_col_kind", lang),
                     _T("coverage_col_count", lang)], [
        [_T("coverage_primary", lang), str(counts["verified"])],
        [_T("coverage_public", lang), str(counts["secondary"])],
        [_T("coverage_unverified", lang), str(counts["unverified"])],
        [_T("coverage_gap", lang), str(counts["missing"])]])


def _client_gap_inputs(dr: dict,
                       lang: str = "ar") -> "tuple[list[str], list[str]]":
    ...
    critical: list[str] = []
    for cat in missing:
        # الموجة ٠: نصّ الفجوة يُولَّد من **مفتاحه القانوني** بلغة التقرير — لا
        # يُنسَخ من قالبٍ عربيّ ثابت، فلا يتسرّب عربيٌّ إلى تقريرٍ إنجليزيّ.
        if cat in _CLIENT_GAP_TERMS:
            what = _T(f"gap_what_{cat}", lang)
            how = _T(f"gap_how_{cat}", lang)
        else:
            what = _T("gap_what_generic", lang)
            how = _T("gap_how_generic", lang)
        critical.append(_T("client_gap_template", lang, what=what, how=how))
    if "entry_door" not in missing:
        ...
        if unverified_doors:
            names = ("، " if lang == "ar" else ", ").join(
                n for n in (_lang_safe(
                    _client_sanitize(_client_prose(f.get("value"), 80), lang),
                    lang)
                    for f in unverified_doors[:2]) if n)
            _named = f" ({names})" if names else ""
            critical.append(_T("gap_entry_door_unverified", lang,
                               named=_named))
    for c in (dr.get("flip_conditions") or []):
        if not c.get("met"):
            critical.append(_T("gap_flip_condition", lang,
                               condition=c.get("condition"),
                               closes_via=c.get("closes_via")))
    ...
    def _content_key(text: str) -> str:
        ...
        return _client_sanitize(body, lang).rstrip(".؛،").strip()
    ...
    for k, m in (dr.get("missions") or {}).items():
        ...
        for line in _mission_gap_lines(label, m.get("summary") or ""):
            ...
            g = _lang_safe(
                _client_sanitize(_trim_sentence(line, 400), lang),
                lang).rstrip(".؛،")
            if not g:
                continue
            key = _content_key(g)
            ...
            seen.add(key)
            per_mission += 1
            # #13 ص15: البند تفصيله وحده — تأطير «لا تمنع القرار» سطرُ
            # تمهيدٍ واحد للقائمة في القسم لا ذيلٌ مخبوز يتكرر مع كل بند.
            informational.append(f"{g}.")
    return critical, informational


# صيد الفجوات ٣: تسميات سيناريوهات الاقتصاد ووحدته تُخزَّن عربيةً في العرض
# (silk_economics) وكانت تُحقَن خاماً في قوالب docx الإنجليزية — خريطة عرضٍ
# صغيرة عند الحدود لا تغييرَ في شكل التخزين (العروض المخزّنة تبقى صالحة).
_ECO_TERM_EN = {"منخفض": "low", "متوسط": "mid", "مرتفع": "high",
                "كجم": "kg", "لتر": "liter", "طن": "ton"}


def _eco_term(val: object, lang: str) -> str:
    v = str(val or "").strip()
...
def _client_economics_section(doc, dr: dict, lang: str = "ar") -> None:
    ...
    # فيصير الغيابُ **صامتاً** بعد أن كان معلَناً — خرقٌ مباشر لعقد
    # «فجوةٌ معلنة لا اختلاق». الآن: بلا نموذجٍ **تُعرَض الفجواتُ وحدَها**.
    if rs.get("max_exw") is None:
        if not _gaps and not (eco.get("decision_numbers") or []):
            return
        doc.add_heading(_T("eco_heading", lang), level=1)
        for g in _gaps:
            _txt = _lang_safe(_client_sanitize(str(g), lang), lang)
            if _txt:
                doc.add_paragraph(_txt, style="List Bullet")
    ...
    doc.add_heading(_T("eco_heading", lang), level=1)
    anchor = eco.get("anchor_price") or {}
    if anchor.get("per_unit") is not None:
        # مصدرُ المرساة **لا** يُلحق هنا (دورة C4 لصيد ٣): الحقلُ ملاحظةُ
        # بعثةٍ خامٌ بوسم تقاطع وشارة دليل، والموقفُ الموثَّق قائم — «قسم
        # الاقتصاد لا يُصيَّر على سطح العميل بمصدره الخام أصلاً» (اختبار
        # wave3). الإسنادُ الكامل في قسم المراجع/التقرير التشغيلي.
        doc.add_paragraph(_T(
            "eco_anchor_line", lang, price=anchor["per_unit"],
            currency=(anchor.get("currency")
                      or _T("eco_local_currency", lang))))
    # البند 5: الرقم بعملته وأساسه («3.2 دينار/كجم») — عروضٌ مخزَّنة قبل
    # الإصلاح بلا الحقلين فيتدهور السطر بسلام إلى الرقم وحده.
    # صيد الفجوات ٣: تسميات السيناريو والوحدة كانت تصل docx الإنجليزي
    # عربيةً خام («under the متوسط scenario», «€/كجم») — تُترجَم عند العرض.
    _basis = "/".join(x for x in (str(rs.get("currency") or "").strip(),
                                  _eco_term(rs.get("unit"), lang)) if x)
    # البند 19: منزلتان في نثر العميل — الدقة الكاملة في جدول السيناريوهات.
    try:
        _exw2 = round(float(rs["max_exw"]), 2)
    except (TypeError, ValueError):
        _exw2 = rs["max_exw"]
    doc.add_paragraph(_T(
        "eco_max_exw_line", lang,
        max_exw=(f"{_exw2} {_basis}" if _basis else _exw2),
        scenario=_eco_term(rs.get("headline_scenario", ""), lang)))
    if rs.get("scenarios"):
        # هدف الدراسة الاحترافية (البند ٣ — العملة): جدول السيناريوهات كان
        # يعرض أرقام EXW عاريةً بلا عملة ولا وحدة — رأس العمود يحملهما
        # («دينار/لتر») من الحل العكسي نفسه؛ غيابهما يُبقي الرأس كما كان.
        _exw_head = _T("col_max_exw", lang) + (f" ({_basis})" if _basis else "")
        _add_table(doc, [_T("col_scenario", lang), _T("col_freight_pct", lang),
                         _T("col_distributor_pct", lang),
                         _T("col_retailer_pct", lang),
                         _exw_head],
                   [[_eco_term(s["scenario"], lang), s["freight_pct"],
                     s["distributor_pct"],
                     s["retailer_pct"], s["max_exw"]]
                    for s in rs["scenarios"]])
        doc.add_paragraph(
    ...
    if pc.get("shortfall_pct") is not None:
        doc.add_paragraph(_T(
            "eco_pricing_warning", lang, exw=pc.get("max_exw_usd"),
            pct=pc.get("shortfall_pct"),
            ref=pc.get("reference_import_price_usd_kg")))
...
def _client_decision_numbers_table(doc, eco: dict, lang: str) -> None:
    """جدول «أرقام القرار» في تقرير العميل (البند ٧) — يُستدعى من مسارَي
    قسم الاقتصاد (بحل عكسي وبدونه). عربية القالب (نصوصها من المحرك) —
    تُعرض على تقرير عربي حصراً كالفرضيات (سياسة الفصل الصلب لا الترجمة)."""
    dn = eco.get("decision_numbers") or []
...
# صيد الفجوات ٣ (عائلة الدرس 104): ثلاثة حقول كان يبنيها العرض ولا يعرضها
# أي سطح — صفوف الأسعار المرصودة بسبب كل صف + سطر الفتح الوحيد (Wave 3.1)،
# شارة «التركّز سياق فقط» (Wave 3.2)، وسجل اشتقاق الرمز بجملة إفصاح
...
def _client_price_observations(doc, dr: dict, lang: str = "ar") -> None:
    ...
    for r in rows:
        ...
        raw = _CAT_TAG_RE.sub("", str(r.get("note") or r.get("value") or ""))
        raw = _BADGE_MARKS_RE.sub("", raw)
        body = _lang_safe(_client_sanitize(
            _client_prose(raw, 160), lang), lang)
        if not body:
            continue
    ...
    if not lines:
        return
    doc.add_heading(_T("price_obs_heading", lang), level=2)
    for b in lines[:8]:
        doc.add_paragraph(b, style="List Bullet")
    if len(lines) > 8:   # دورة C4: لا سقف صامتاً — القطعُ يُعلَن (قاعدة البيت)
        doc.add_paragraph(
            f"({len(lines) - 8} رصداً إضافياً في سجل أدلة التقرير الكامل)"
            if lang != "en" else
            f"({len(lines) - 8} more observations in the full evidence log)")
    doc.add_paragraph(_T("price_unlock", lang))
    if dr.get("concentration_context_only"):
        doc.add_paragraph(_T("concentration_context_line", lang))


def _client_hypotheses_section(doc, dr: dict, lang: str = "ar") -> None:
    """قراءات تلتقي عندها الأدلة (البند ٦) — فرضيات الاستدلال العابر
    للمهمات بحقولها الأربعة كما بُنيت حتمياً (لا صياغة نموذج): الوقائع،
    التفسيران المرشحان، الأرجح ولماذا، والفحص الحاسم. عربية القالب —
    تُعرض على تقرير عربي حصراً (سياسة الفصل الصلب؛ قيد معلَن)."""
    if lang == "en":
        return
    hyps = dr.get("hypotheses") or []
    if not hyps:
        return
    doc.add_heading("قراءات تلتقي عندها الأدلة — فرضيات للتحقق", level=2)
    doc.add_paragraph(
        "كل بند أدناه فرضية جمعتها وقائع هذه الدراسة نفسها — ليست نتيجة، "
        "ومع كل فرضية الفحص الواحد الذي يحسمها.")
    for h in hyps[:5]:
        ...
        if len(exps) >= 2:
            doc.add_paragraph(f"تفسيران مرشحان: (١) {exps[0]}؛ "
                              f"(٢) {exps[1]}.")
        if h.get("likelier"):
            doc.add_paragraph(f"الأرجح: {h['likelier']} — "
                              f"{h.get('why') or ''}")
        if h.get("settling_check"):
            doc.add_paragraph(f"ما يحسم: {h['settling_check']}")


def _client_hs_derivation_section(doc, dr: dict, lang: str = "ar") -> None:
    deriv = dr.get("hs_derivation") or {}
    if not isinstance(deriv, dict) or not deriv.get("hs6"):
        return
    doc.add_heading(_T("hs_derivation_heading", lang), level=2)
    chain = f"{deriv.get('chapter')} → {deriv.get('heading')} → " \
            f"{deriv.get('subheading')}"
    desc = _lang_safe(_client_sanitize(
        str(deriv.get("official_description") or ""), lang), lang)
    doc.add_paragraph(chain + (f" — {desc}" if desc else ""))
    basis = _lang_safe(_client_sanitize(
        str(deriv.get("basis") or ""), lang), lang)
    if basis:
        doc.add_paragraph(basis)
    # دورة C4: الإفصاح بلغة التقرير — المرآة الإنجليزية من السجل نفسه، وإلا
    # كان `_lang_safe` يُسقط الجملة الإلزامية صامتاً عن docx الإنجليزي.
    sub_raw = (deriv.get("substitution_note_en") if lang == "en"
               else deriv.get("substitution_note")) or \
        deriv.get("substitution_note") or ""
    sub = _lang_safe(_client_sanitize(str(sub_raw), lang), lang)
    if sub:   # جملة إفصاح الاستبدال الإلزامية (§3.2) تصل القارئ فعلاً
...
def _client_gaps_section(doc, dr: dict, lang: str = "ar",
    ...
    doc.add_heading(_T("gaps_heading", lang), level=1)
    gap_lines, mission_gap_lines = _client_gap_inputs(dr, lang)
    ...
    if gap_lines:
        ...
        # واحد** بعمودِ حالةٍ يفرّق «لم يُعرَف بعد» عن «مرصود من مصادر غير رسمية»، بدل
        ...
        if mission_gap_lines:
            ...
            doc.add_paragraph(_T(
                "gaps_nonblocking_one" if n == 1 else "gaps_nonblocking_many",
                lang, n=n))
        ...
        doc.add_paragraph(_T("gaps_none", lang))
    # HF3: تحفّظاتُ المعقولية تُصيَّر دوماً حين وُجدت (بعد الفجوات أو وحدها).
    for _cv in _caveats:
        _cv_txt = _lang_safe(_client_sanitize(_cv, lang), lang)
        if _cv_txt:
            doc.add_paragraph(_cv_txt, style="List Bullet")
    # §F-5 (حزمة الفكس v2.1): دعوة التعميق المدفوع («next_step») أُزيلت من
...
def _client_readable_fact(value: object, note: object) -> "str | None":
    ...
    if isinstance(value, dict):
        ...
        if value.get("hhi") is not None:
            # البند ٣ (لغة الزائر): «مؤشر تركّز HHI=7118» يصير جملة معنى
            # تحفظ الرقم — العتبات من `silk_economics` (المقياس الموحد
            # 0–10000، فوق 2500 تركّز مرتفع) لا نسخة محلية.
            try:
                from silk_economics import hhi_is_high
                _h = float(value["hhi"])
                # §58 (find all gaps): القيمة الخام قد تصل كسراً 0–1 (نفس ما
                # يطبّعه silk_deep_pillars._scaled_hhi) — بلا تحجيم كان سوقٌ
                # مركّز (0.71 = 7100) يُعرض للعميل «موزّعاً على منافسين كثر».
                if 0.0 < _h <= 1.0:
                    _h *= 10_000
                if _h > 6000:
                    _desc = "مورّد واحد يسيطر على معظم السوق"
                elif hhi_is_high(_h):
                    _desc = "مورّد أو اثنان يسيطران على السوق"
                else:
                    _desc = "التوريد موزّع على منافسين كثر"
                return f"تركّز المورّدين {int(round(_h))} من 10000 — {_desc}"
            except (TypeError, ValueError):
                return f"تركّز المورّدين {value['hhi']} من 10000"
...
def _client_references_section(doc, dr: dict, lang: str = "ar") -> None:
    ...
    doc.add_heading(_T("sources_heading", lang), level=1)
    doc.add_paragraph(_client_methodology_paragraph(dr, lang))
    from silk_data_layer import atomic_source_ids
    refs: dict[str, dict] = {}
    for m in (dr.get("missions") or {}).values():
        if not isinstance(m, dict):
    ...
    for key in sorted(refs, key=lambda k: refs[k]["name"]):
        ...
        # التقرير — بلا هذا كان سطرُ مرجعٍ إنجليزيّ يحمل «تاريخ الجمع:».
        name = _lang_safe_name(r["name"], lang)
        line = f"{name} — {r['url']}"
        if r["retrieved_at"]:
...
def _client_decision_basis(doc, view: dict, lang: str = "ar") -> None:
    ...
    البنيةُ جاهزةٌ من `silk_render.decision_basis` ومُلحَقةٌ بـ
    `view["decision"]["basis"]`، فيقرأها هذا المُصدِّرُ و`web/platform.html`
    من **مصدرٍ واحد**. بناؤها هنا كان سيُجبِر سطحَ المصنع على نسخةٍ ثانية
    بلغةٍ ثانية — نفسُ عطل «خريطتان تتباعدان».

    والقسمُ يُسقِط نفسَه حين لا أساسَ محسوب (لا هيكلَ فارغ)، ولا يُعرَض حين
    يخالف الحكمُ المعروضُ حكمَ المحرّك (حكمان في مصنوعٍ واحد).
    """
    ...
    dr = view.get("deep_research") or {}
    if dr and _verdict_tone(_resolve_vtxt(dr)) != _verdict_tone(ed.get("verdict")):
        return
    ...
    _add_table(doc, [basis["col_pillar"], basis["col_strength"],
                     basis["col_note"]],
               [[r["name"],
                 (f"{r['strength_pct']}%" if r["strength_pct"] is not None
                  else basis["not_computed"]),
                 r["note"]] for r in basis.get("pillars") or []])
...
def render_client_docx(view: dict, path: str) -> str:
    ...
    lang = _view_lang(view)
    # مُطهِّرٌ مربوطٌ باللغة يُمرَّر لكل مُصيّرٍ فرعيّ — تمريرُ `_client_sanitize`
    # عارياً كان يطبّق الاستبدالات **العربية** على نصٍّ إنجليزيّ فيحقن العربية
    # فيه («in this دورة تحليل»)، وهو ما رصدته المراجعة الذاتية.
    def _sanitize(text: object) -> str:
        return _client_sanitize(text, lang)
    doc = Document()
    # §4: العربية من اليمين لليسار؛ الإنجليزية LTR بلا أيّ حقنِ bidi.
    _apply_direction(doc, lang)
    h = view.get("header") or {}
    market = dr.get("market") or {}
    verdict = dr.get("verdict") or {}
    ai = verdict.get("ai") or {}
    ...
    _title = f"{_T('report_title', lang)}: {view.get('product')}"
    _add_page_header_footer(doc, f"{_SILK_WORDMARK[lang]} — {_title}", lang)
    _add_cover_wordmark(doc, branding, lang)
    doc.add_heading(_title, 0)
    doc.add_paragraph(_T("report_by", lang), style="Intense Quote")
    if view.get("test_run"):
        doc.add_paragraph(_T("test_run_banner", lang))
    _stamp_degraded_banner(doc, view, lang)
    # البند 18 (موجة سدّ الفجوات الثانية): صندوق التحذير الواحد يصل مُسلَّم
    # العميل أيضاً — كان غائباً كلياً عن هذا القالب (نمط حادثة #10: أرقام
    # كومتريد غير موصوفة تبلغ العميل)، بينما اللوحة وmd والمشغّل يعرضونه.
    if dr.get("caveat_box"):
        doc.add_paragraph(f"⚠ {dr['caveat_box']}", style="Intense Quote")
    _add_verdict_badge(doc, vtxt, lang)
    _add_table(doc, [_T("item", lang), _T("value", lang)], [
        [_T("product", lang), h.get("product") or view.get("product")],
        [_T("hs_code", lang), h.get("hs_code") or view.get("hs_code")],
        [_T("origin", lang), _T("origin_saudi", lang)],
        [_T("target_market", lang), h.get("target_market")
         or (market.get("name_ar") if lang == "ar" else None)
         or market.get("name_en") or market.get("name_ar")],
        [_T("report_date", lang), h.get("date")]])
    ...
    sections = _parse_writer_sections_numbered(_report_text)
    ...
    for num, title, body in sections:
        ...
        if num == _RECOMMENDATIONS_ORDINAL or title == "التوصيات الاستراتيجية":
            ...
            continue
        client_head = _client_section_for(num, title)
        if client_head:
            buckets[client_head].append(body)

    # ٠) الملخص التنفيذي — يفتتح المستند مستقلاً (هدف الدراسة الاحترافية،
    # البند ٤): القارئ يقرأ الأسطر الستة الأولى ويقرر. يُسقَط بلا عنوان
    # يتيم حين لا سرد كاتب (سطر «تعذّر السرد» في قسم القرار يغطي الغياب).
    if any(any(str(ln).strip() for ln in b)
           for b in buckets["الملخص التنفيذي"]):
        doc.add_heading(_T("sec_exec_summary", lang), level=1)
        _client_body_or_fallback(doc, buckets["الملخص التنفيذي"], dr,
                                 "الملخص التنفيذي", lang)

    # ١) القرار وأساسه — يبدأ دوماً بالحكم وتعليله (المصدر الوحيد للحكم)،
    # ثم سرد الكاتب (التوصيات) إن توفّر.
    doc.add_heading(_T("sec_decision", lang), level=1)
    doc.add_paragraph(_T("recommendation_line", lang,
                         label=_verdict_label(_verdict_tone(vtxt), lang)))
    ...
    # ── الفصل الصلب (الموجة ٠) — الطبقة ٢: بنيويّة لا توجيهيّة ──────────────
    # `ai.reasoning` و`verdict.note` نصّان يكتبهما التوليفُ بلغة **طبقة الأدلة
    # الداخلية** (عربية). نسخُهما حرفياً إلى تقريرٍ إنجليزيّ تسرّبُ لغةٍ — لا
    # يُصلَح بترجمةٍ هنا (ذاك مسارُ الترجمة الممنوع، §5)، بل يُسقَط ويحلّ محلّه
    # سطرُ الأساس الحتميّ المحسوب من نفس الحقول. لا معلومةَ تُفقَد: الحكمُ
    ...
    if not reasoning and verdict.get("confidence") is not None \
        ...
        from silk_narrative import confidence_phrase
        reasoning = _T("confidence_basis_line", lang,
                       phrase=confidence_phrase(verdict.get("confidence"),
                                                lang))
    if reasoning:
        ...
        # صيد الفجوات ٣: كان النداء عارياً من اللغة فتُطبَّق الاستبدالات
        # العربية على نثرٍ إنجليزيّ («The overall التقييم is moderate») —
        # المُطهِّر المربوط `_sanitize` موجود لهذا الغرض تحديداً.
        _r = _sanitize(_client_prose(reasoning, 2000))
        if _r:
    ...
    if not (dr.get("report") or {}).get("text"):
        doc.add_paragraph(_T("narrative_unavailable", lang))
    # الموجة Z (Z-06): أساسُ الحكم قبل سردِ الكاتب — القارئُ يعرف على أيّ
    # شيءٍ بُني الحكم قبل أن يقرأ شرحَه.
    _client_decision_basis(doc, view, lang)
    ...
    # ٥ب) النموذج الاقتصادي للعميل (الموجة ٣ §5.4 — سدّ خياطة): أهم رقم
    # قرار في الدراسة يصل مُسلَّم العميل نفسه، بلغة تجارية بلا أي مصطلح
    # تشغيلي (يمرّ بحارس المفردات كسائر الأقسام).
    _client_economics_section(doc, dr, lang)
    # البند ٦ (هدف الدراسة الاحترافية): فرضيات الاستدلال العابر للمهمات.
    _client_hypotheses_section(doc, dr, lang)
    # صيد الفجوات ٣: صفوف الأسعار المرصودة + سطر الفتح + شارة «التركّز سياق»
    # وسجل اشتقاق الرمز — كانت تُبنى في العرض بلا أي سطح يعرضها (درس 104).
    _client_price_observations(doc, dr, lang)
    _client_hs_derivation_section(doc, dr, lang)
    # **الموجة B (البند T-11).** كان هذا القسمُ معرَّفاً بلا مُنادٍ إطلاقاً —
    ...
    _docx_glossary(doc, dr, sanitize=_sanitize, lang=lang)
    ...
    if _client_redact_residual(doc, lang) and view.get("internal"):
        doc.add_paragraph(
            "ملاحظة: نُقّيت بعض المصطلحات التقنية الداخلية من هذا التقرير "
            "تلقائياً لتقديمها بلغة تجارية؛ الأرقام ومصادرها لم تُمَسّ.",
            style="Intense Quote")
    _client_assert_clean(doc, lang)  # شبكة أمان أخيرة — لِما يستحيل تنقيته فقط
    _assert_verdict_consistency_doc(doc, vtxt, "تقرير العميل",
                                    lang)  # Master Prompt Part 2 §B
    ...
    _artifact_findings = _qg.run_client_artifact_text_gate(
        _artifact_text, lang=lang, view=view)
    if _artifact_findings:
        raise ClientArtifactGateError(
            "رفضت بوابة نصّ المُنتَج النهائي تسليم تقرير العميل: "
            + "؛ ".join(f["note"] for f in _artifact_findings[:5]),
            findings=_artifact_findings)
    # §4: اتجاه RTL صريح على كل فقرة/run قبل الحفظ — للعربية وحدها.
    _finalize_direction(doc, lang)
    doc.save(path)
    return path
...
def _client_missing_narrative_heads(dr: dict) -> "dict[str, list[str]]":
    ...
    sections = _parse_writer_sections_numbered(
    ...
    for num, title, body in sections:
        if num == _RECOMMENDATIONS_ORDINAL or title == "التوصيات الاستراتيجية":
            ...
            if roadmap_part:
                buckets["مسار الدخول والمتطلبات"].append(roadmap_part)
            continue
        head = _client_section_for(num, title)
        if head:
            buckets[head].append(body)
    out: dict[str, list[str]] = {}
    # «الملخص التنفيذي» خارج آلية النص البديل عمداً (البند ٤): غيابه غياب
    # سردٍ كامل (يغطيه سطر «تعذّر السرد») والقسم يُسقِط نفسه بلا عنوان يتيم
    # — ملخصٌ بديل من قائمة مصادر ليس ملخصاً تنفيذياً.
    for head in _CLIENT_SECTION_ORDER:
        if head == "الملخص التنفيذي":
            continue
...
def _client_body_or_fallback(doc, bodies: list[list[str]], dr: dict,
                             client_head: str, lang: str = "ar") -> None:
    ...
    if bodies:
        for body in bodies:
            _client_render_body_block(doc, body, lang)
        return
    prose = str(((dr.get("client_fallback_prose") or {}).get(client_head))
                or "").strip()
    if prose:
        for para in prose.splitlines():
            line = _lang_safe(_client_sanitize(_client_prose(para, 600),
                                              lang), lang)
            if line:
                doc.add_paragraph(line)
        return
    doc.add_paragraph(_T("section_narrative_absent", lang))
...
_PDF_BUSY = "محرّك تحويل PDF مشغول الآن — أعد المحاولة بعد قليل"
PDF_RETRY_AFTER_S = 15


def pdf_busy_detail() -> dict:
    """جسمُ 503 `pdf_busy` الواحد للمسارين (مراجعة R9) — الصفحةُ تترجم الرمز."""
    return {"error": "pdf_busy", "retry_after_s": PDF_RETRY_AFTER_S, "message": _PDF_BUSY}


class PdfBusy(RuntimeError):
    """R9 (تدقيق 2026-09-01، API-11/EXT-14): فتحاتُ تحويل PDF كلُّها مشغولة — تُترجَم
...
def _pdf_max_concurrent() -> int:
    """سقفُ تحويلات LibreOffice المتزامنة على الحاوية — `SILK_PDF_MAX_CONCURRENT` (٢)."""
    try:
        return max(1, int(os.environ.get("SILK_PDF_MAX_CONCURRENT", "2") or "2"))
    except ValueError:
        return 2


def _pdf_wait_s() -> float:
    """مهلةُ انتظار فتحةٍ قبل `PdfBusy` — `SILK_PDF_WAIT_S` (٠ = رفضٌ فوريّ)."""
    try:
        return max(0.0, float(os.environ.get("SILK_PDF_WAIT_S", "0") or "0"))
    except ValueError:
        return 0.0


def _pdf_slots() -> "threading.BoundedSemaphore":
    """سيمافورُ الفتحات — يُعاد بناؤه حين يتغيّر السقف بيئياً (نمطُ `silk_research_runtime._slots`)."""
    global _PDF_SLOTS, _PDF_SLOTS_CAP
    cap = _pdf_max_concurrent()
    with _PDF_SLOTS_LOCK:
        if _PDF_SLOTS is None or cap != _PDF_SLOTS_CAP:
            _PDF_SLOTS = threading.BoundedSemaphore(cap)
            _PDF_SLOTS_CAP = cap
        return _PDF_SLOTS
...
def _pdf_slot():
    """احجز فتحةَ تحويلٍ أو ارفع `PdfBusy` فوراً/بعد `SILK_PDF_WAIT_S` — تُحرَّر دائماً."""
    slots = _pdf_slots()
    if not slots.acquire(timeout=_pdf_wait_s()):
        raise PdfBusy(_PDF_BUSY)
    try:
        yield
    finally:
        slots.release()
...
def _kill_soffice_group(proc) -> None:
    """اقتل مجموعةَ عملية LibreOffice كاملةً (setsid ⇒ pgid = pid) ثم اجنِها."""
    import signal
    if os.name != "nt":
        try:
            os.killpg(proc.pid, signal.SIGKILL)
        except Exception:  # noqa: BLE001 — أفضل جهد ثم العمليةُ نفسها
            proc.kill()
    else:
        proc.kill()
    try:
...
def _run_soffice(cmd: list, *, timeout: int, env: dict) -> "subprocess.CompletedProcess":
    """شغّل LibreOffice في **مجموعة عملية جديدة** واقتلها كاملةً عند المهلة — R9
    (EXT-14): `subprocess.run(timeout=)` كان يقتل soffice وحده ويترك أبناءه
    (oosplash/…) أحياءً. مراجعة R9: أيُّ فشلٍ آخر أثناء القراءة يقتل المجموعةَ أيضاً
    (`subprocess.run` كان يضمن ذلك)، و`with Popen` يغلق الأنابيب ويجني العملية."""
    with subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                          env=env, start_new_session=True) as proc:
        try:
            out, err = proc.communicate(timeout=timeout)
        except BaseException:
            _kill_soffice_group(proc)
            raise
...
def _find_soffice() -> "str | None":
    """مسار ثنائي LibreOffice (soffice/libreoffice) إن وُجد على الخادم — يُحسَب مرّةً
    (مراجعة R9: `shutil.which` مرّتين لكلّ تحويل داخل فتحةٍ محدودة)."""
    import shutil
...
_SOFFICE_ENV_KEEP = ("PATH", "LANG", "LC_ALL", "TMPDIR", "TEMP", "TMP", "SYSTEMROOT",
                     "USERPROFILE", "APPDATA", "LOCALAPPDATA", "FONTCONFIG_PATH",
                     "XDG_RUNTIME_DIR", "DISPLAY")


def _soffice_env(profile: str) -> dict:
    """بيئةُ عملية LibreOffice — R7 (SEC-4): كانت نسخةَ بيئة الخادم كاملةً (مفاتيحُ
    المزوّدين وسرُّ المنصّة) لعمليةٍ تحلّل مستنداً؛ الآن قائمةٌ بيضاء + `HOME` مؤقّت."""
    env = {k: os.environ[k] for k in _SOFFICE_ENV_KEEP if os.environ.get(k)}
    env["HOME"] = profile
...
def docx_to_pdf(docx_path: str, pdf_path: "str | None" = None,
                timeout: int = 180, lang: str = "ar") -> str:
    ...
    # R9 (API-11/EXT-14): فتحةٌ محدودة قبل أيّ شيء — التحويلُ عمليةُ LibreOffice كاملة
    # على حاويةٍ واحدة؛ الزائدُ يُرفَض مسمّىً (`PdfBusy`) لا يصطفّ بلا سقف.
    import os
    import tempfile
    ...
    # الموجة ٠: التجريدُ علاجٌ لمشكلةِ تشكيلٍ **عربية** بحتة (شقُّ الكلمة في
    # استخراج نصّ الـPDF)؛ لا حركاتِ في مستندٍ إنجليزيّ فيُحوَّل الأصل مباشرة.
    if silk_i18n.is_rtl(lang):
        try:
            convert_src = _pdf_diacritic_free_copy(docx_path)
        except Exception:  # noqa: BLE001 — تعذّر التجريد => حوّل الأصل
            convert_src = docx_path
    else:
        convert_src = docx_path
    # §58 M5: بروفايل LibreOffice ومجلدُ النسخة المجرّدة (نسخة docx كاملة من
    # تقرير العميل) كانا يُتركان في /tmp حتى إعادة التشغيل — يُنظَّفان الآن
    # بـfinally مهما كانت نتيجة التحويل.
    try:
        try:
            # R9 (API-11) + مراجعة: الفتحةُ حول عملية LibreOffice **وحدها** — لا حول النسخة
            # بلا حركات ولا تطبيع طبقة النصّ ولا فحص الأقواس (معالجةٌ بايثونية خالصة).
            with _pdf_slot():
                proc = _run_soffice(
                    [soffice, "--headless", "--norestore",
                     f"-env:UserInstallation=file://{profile}",
                     "--convert-to", "pdf", "--outdir", out_dir, convert_src],
                    timeout=timeout, env=_soffice_env(profile))
        except PdfBusy:
        ...
        if convert_src != docx_path:
            shutil.rmtree(os.path.dirname(convert_src), ignore_errors=True)
    # تدقيق 2026-08-27: طبقةُ نصّ الـPDF (النسخ/البحث/قارئ الشاشة) كانت
    # مبعثرةً بينما الورقةُ سليمة — المستخرِجُ يعكس السلسلةَ كاملةً فيقلب
    # داخلَ كلّ عنقودِ رباط. التطبيعُ يعكسها مسبقًا في `ToUnicode` فتُقرأ
    # صحيحةً؛ التصييرُ لا يُمَسّ (مُثبَتٌ: ٥/٥ صفحاتٍ متطابقةٌ بكسليًّا).
    # يقع **قبل** فحص الأقواس كي يفحصَ الحارسُ المستندَ المُسلَّم فعلًا؛
    # وفشلُه لا يُسقِط التصدير (يعيد False ويترك الأصل كما هو).
    if silk_i18n.is_rtl(lang):
        import silk_pdf_textlayer
        silk_pdf_textlayer.normalize_arabic_text_layer(produced)
    # WP-5 §2: الفحص الآلي لاتجاه الأقواس على الـPDF النهائي — فوق العتبة
    # يفشل التصدير بصوت عالٍ بدل تسليم مستند معكوس الأقواس.
    # الموجة ٠: انعكاسُ الأقواس عَرَضٌ ثنائيُّ الاتجاه (bidi) — لا يقع في
    # مستندٍ إنجليزيٍّ خالص، والفحص عليه بلا معنى.
    if silk_i18n.is_rtl(lang):
...
_ACADEMIC_MAIN_REC = {
    ...
    # الموجة Z (Z-03): تصانيفُ كانت بلا نصٍّ هنا فتُسقِط التقرير الأكاديمي
    # بـKeyError على نفس الحالات التي أسقطت تقرير العميل.
    "preliminary": "المضي المبدئي في دخول السوق مع استكمال المعطيات غير "
                   "المتاحة قبل أي التزام إنتاجي أو شحني.",
    "inconclusive": "إرجاء الحسم حتى تُغلَق الفجوات المعلنة أدناه — "
                    "المرصود لا يكفي لترجيح الدخول أو تركه.",
    "data_complete": "اكتمل جمع الأدلة؛ يلزم تقييمٌ تجاريٌّ قبل إصدار توصية "
                     "دخول أو تركٍ للسوق.",
    "data_partial": "استكمال الأدلة الناقصة أدناه قبل إصدار توصية — "
                    "المرصود جزئيّ.",
    "data_absent": "إعادة جمع الأدلة من مصادرها؛ المتاح منها لا يكفي لأي "
                   "توصية.",
}
...
def _academic_summary(doc, view: dict, dr: dict, vtxt: str) -> None:
    ...
    if dr.get("hs_flagged"):
        # البند 18 (موجة سدّ الفجوات الثانية): تحذيرُ الرمز مرةً واحدة —
        # نصُّه الكامل في §٢ المنهجية حصراً، وهنا إحالةٌ لا تكرار (كانت
        # الجملة السياقية مزدوجة في القالب نفسه — نمط حادثة #10 تحت خط
        # نظر فحص caveat_repetition الذي يمسح نص الكاتب لا الـdocx).
        doc.add_paragraph(
            "ويحكم قراءةَ الأرقام الجمركية في هذه الدراسة قيدٌ منهجي "
            "جوهري مفصَّل في قسم «المنهجية ومصادر البيانات».")
...
def _pdf_via_tmp_docx(builder, view: dict, path: str, prefix: str,
                      name: str) -> str:
    """جسم مشترك لأغلفة الـPDF الثلاثة — §58 M5: مجلد docx الوسيط كان يُترك
    في /tmp **بنسخة كاملة من تقرير العميل** تتراكم حتى إعادة التشغيل؛ يُنظَّف
    الآن بـfinally مهما كانت نتيجة التحويل."""
    import os
    import shutil
    import tempfile
    td = tempfile.mkdtemp(prefix=prefix)
    try:
        tmp_docx = os.path.join(td, name)
        builder(view, tmp_docx)
        # الموجة ٠: لغة التقرير تصل خطوةَ التحويل أيضاً — خطواتُ الـPDF
        # الخاصّة بالعربية (تجريد الحركات، فحص الأقواس) لا تُطبَّق على مستندٍ
        # إنجليزيّ (بلا أثرٍ في الحالة الأفضل، ووقتٌ ضائعٌ في الأسوأ).
        return docx_to_pdf(tmp_docx, path, lang=_view_lang(view))
    finally:
...
def render_academic_pdf(view: dict, path: str) -> str:
    ...
    return _pdf_via_tmp_docx(render_academic_docx, view, path,
                             "silk_acad_", "academic.docx")
...
def render_client_pdf(view: dict, path: str) -> str:
    ...
    return _pdf_via_tmp_docx(render_client_docx, view, path,
                             "silk_pdf_", "client.docx")


def render_research_pdf(view: dict, path: str) -> str:
    """§3: تقرير المدقّق PDF — يُبنى تقرير البحث docx (RTL) ثم يُحوَّل."""
    return _pdf_via_tmp_docx(render_docx, view, path,
                             "silk_pdf_", "research.docx")
...
def render_docx(view: dict, path: str) -> str:
    ...
    if countries:
        ...
        doc.add_paragraph("بيانات التجارة الثنائية غير متاحة لهذا التحليل "
                          "(تتطلب with_research/UN Comtrade).")
    ...
    for i, m in enumerate((view.get("markets") or [])[:8], 1):
        ...
        if lines:
            ...
            doc.add_paragraph("مكوّنات هذا السوق غير متاحة")
    ...
    if prov:
        ...
        for b in prov:
            # T-10: ذيلُ الإسناد (رابط · مدى ثقة · أدلة) **مبنيٌّ في نموذج
            # العرض** (`silk_render._provenance`) — هنا عرضٌ لا بناء
            # (الدرس ١٠٥)، فيقرؤه العارضون الثلاثة بلغةِ التقرير نفسِها.
...
def _docx_glossary(doc, dr: dict, sanitize=None, lang: str = "ar") -> None:
    """B1 (SPEC-v2): اعرض «مسرد المصطلحات» في مستند Word (المدقّق والعميل)
    من بنية النموذج. `sanitize` اختياري (مُطهِّر تقرير العميل).

    **الموجة ٠.** المسرد شرحٌ عربيٌّ لمصطلحاتٍ إنجليزية — بنيةٌ لا معنى لها في
    تقريرٍ إنجليزيّ (العقد الإنجليزيّ يشرح المصطلح عند أوّل ورودٍ في النثر
    نفسه، §34). فيُسقَط كاملاً في الإنجليزية: **غيابٌ مقصود لا فجوةٌ صامتة** —
    الشرحُ موجودٌ في المتن، وحقنُ مسردٍ عربيٍّ كان يمنع التقرير من الخروج."""
    if silk_i18n.normalize(lang) == "en":
        return
    from silk_style_contract import GLOSSARY_HEADING
    gloss = dr.get("glossary") or []
    ...
    for g in gloss:
        line = _glossary_line(g, plain=bool(sanitize))
...
def _glossary_line(entry: dict, plain: bool) -> str:
    ...
    from silk_style_contract import (KEEP_WITH_SHORT_DESC, PLAIN_GLOSS,
                                     PLAIN_TERMS)
    term = entry.get("term", "")
    if plain and term in PLAIN_TERMS:
        return f"{PLAIN_TERMS[term]}: {PLAIN_GLOSS.get(term, entry.get('gloss', ''))}"
    # أسماءُ الأنظمة تبقى بالاسم، ووصفُها القصير هنا (لا في المتن) — هذا
    # هو المستهلك الحيّ لـKEEP_WITH_SHORT_DESC بعد إخراجه من المتن.
    if plain and term in KEEP_WITH_SHORT_DESC:
        return f"{term}: {KEEP_WITH_SHORT_DESC[term]}"
    return f"{term}: {entry.get('gloss', '')}"
...
def _docx_leads(doc, dr: dict, sanitize=None, lang: str = "ar",
    ...
    الشركات وعناوينُها وأرقامُها تبقى كما رُصدت (أسماءُ علَمٍ لا تُترجَم)."""
    from silk_gmaps import maps_disclaimer
    MAPS_DISCLAIMER = maps_disclaimer(dr.get("product"), lang)  # Wave 2 + لغة
    leads, note = _leads_data(dr)
...
def _md_deep_research(view: dict, prefix: list[str]) -> str:
    ...
    # البند 18 (موجة سدّ الفجوات F5): صندوق التحذير الواحد أعلى التقرير —
    # بُني حتمياً في build_view؛ يُعرَض مرة واحدة هنا لا مكرراً في الأقسام.
    _cbox = str(dr.get("caveat_box") or "")
    if _cbox:
        L += [f"> ⚠ {_cbox}", ""]
    ...
    # موجة سدّ الفجوات الثانية (G7): فحص اتساق التشغيلات (البند 7) كان
    # يُحتسب للبوابة وحدها بلا سطح بشري — الغياب يُعلن كما تُعلن النتيجة.
    _vc = dr.get("verdict_consistency") or {}
    if _vc:
        if _vc.get("inconsistent"):
            L.append("- فحص اتساق التشغيلات: **ترقية حكم مع تدهور الدليل "
                     f"مقارنةً بالتحليل #{_vc.get('previous_analysis_id', '؟')}"
                     "** — راجع بوابة الجودة.")
        elif _vc.get("comparable") is False:
            L.append("- فحص اتساق التشغيلات: لا تشغيلة سابقة مكتملة لنفس "
                     "المنتج والسوق — المقارنة غير متاحة.")
        else:
            L.append("- فحص اتساق التشغيلات: متسق مع أحدث تشغيلة سابقة "
                     "لنفس المنتج والسوق.")
    ...
    _rep_md = dr.get("report") or {}
    report_text = _rep_md.get("text")
    if report_text:
        # بلاغ تحليل 20: شارة «غير مكتمل» تُصاغ هنا بصيغة markdown (لا تُحقَن في
        # النصّ الواحد كي لا تظهر حرفياً في docx) — قبل السرد كي يراها القارئ فوراً.
        if _rep_md.get("incomplete"):
            from silk_render import _incomplete_banner
            L += [_incomplete_banner(_rep_md.get("missing_sections") or [],
                                     _view_lang(view)), ""]
    ...
    # ── النموذج الاقتصادي (الموجة ٣ §5.4) — قبل ملحق الأدلة ─────────────────
    L += _economics_md_lines(dr)
    ...
    # ── البند ٦: فرضيات الاستدلال العابر للمهمات — بحقولها الأربعة ─────────
    _hyps = dr.get("hypotheses") or []
    if _hyps:
        L += ["## قراءات تلتقي عندها الأدلة — فرضيات للتحقق", ""]
        for h in _hyps[:5]:
            ...
            exps = h.get("explanations") or []
            if len(exps) >= 2:
                L.append(f"- تفسيران مرشحان: (١) {exps[0]}؛ (٢) {exps[1]}.")
            L.append(f"- الأرجح: {h.get('likelier')} — {h.get('why')}")
            L.append(f"- ما يحسم: {h.get('settling_check')}")
            L.append("")
    ...
    if prov:
        for b in prov:
            L.append(f"- {b['source']}: أسهم {b['contributed']} من "
                     f"{b['attempted']} محاولة" + str(b.get("tail") or ""))
...
def render_markdown(view: dict) -> str:
    ...
    if ed is None:
        ...
        from silk_decision import _WEIGHT_LABEL_AR
        from silk_narrative import confidence_phrase, verdict_ar
        # الموجة ٥ (الجزء ٦-١): قاعدة القرار تُعرض **قبل** الحكم.
        if ed.get("decision_rule"):
            L += [str(ed["decision_rule"]), ""]
        sbo = ed.get("scores_by_option") or {}
        weights_label = ed.get("weights_label") or ed.get("weights_option") or ""
        ...
        L += ["", "| العمود | القيمة | الأساس | مكوّنات غائبة |",
              "| --- | --- | --- | --- |"]
        for key, p in (ed.get("pillars") or {}).items():
            missing = _missing_ar(p.get("missing"))
            L.append(f"| {_pillar_ar(key)} | {_fmt(p.get('value'))} | "
        ...
        # الموجة ٥ (الجزء ٦-٥): الحجة المضادة في أقوى صورها + سبب ردّها.
        cc = ed.get("counter_case") or {}
        if cc.get("case"):
            L += ["", f"**الحجة المضادة:** {cc['case']}"]
            if cc.get("rebuttal"):
                L.append(cc["rebuttal"])
        if ed.get("note"):
    ...
    if r_bundle is None:
        ...
        for g in ms.get("gaps") or []:
            L.append(f"- معطى ناقص: {_gap_ar(g)}")
        L.append("")
    ...
    if other_markets:
        ...
        for m in other_markets:
            ...
            if lines:
                ...
                L.append("- مكوّنات هذا السوق غير متاحة")
    ...
    if comp:
        ...
        for metric in ("hhi", "top_supplier_share_pct", "saudi_share_pct"):
            ...
            if f and f.get("value") is not None:
                ...
                L.append(f"- {internal_ar(metric)}: غير متاح")
        ...
        if ents_rc:
            ...
            L.append("- لا كيانات مسمّاة — القائمة غير متاحة (أسماء الأعمال "
        ...
        L += [r_absent or "وكيل المنافسة بلا اكتشافات — بياناته غير متاحة", ""]
    ...
    if r_bundle is None:
        ...
        if vals:
            ...
            if gap:
                L.append(f"- معطى ناقص: {_gap_ar(gap)}")
        else:
        ...
        if refs:
            ...
            L.append("- مراجع الأسعار: غير متاحة")
        L.append("")
    ...
    for key, title in (("saudi", "**مورّدون ومصنّعون سعوديون:**"),
        ...
        items = sd.get(key) or []
        if not items:
            L.append("- لا مرشّحين لهذا الدليل — القائمة غير متاحة")
        for e in items[:10]:
    ...
    if r_bundle is None or not reg:
        ...
        if checklist:
            ...
            L.append("- بنود الاشتراطات غير متاحة في مرجع L1 لهذا السوق")
    ...
    for dp in top_m.get("risk") or []:
        if dp.get("value") is not None:
            src = str(dp.get("source") or "غير متاح")
