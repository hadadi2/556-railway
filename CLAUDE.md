...
### ٤) أسلوب الكتابة · writing style (تفضيل مالك مُنفَّذ)

ردودك للمالك تتبع مهارة `token-saver` **دائماً**: مباشرة بلا مقدمات، بلا
إعادة صياغة سؤاله، بلا تكرار ما قيل، بتنسيق أدنى (لا عناوين ولا جداول ولا
**bold** إلا لحاجة حقيقية)، والنقاط عند تجاوز ثلاثة عناصر فقط. لا يُضحّى
بالدقة ولا بتحذير مهم من أجل الاختصار، وإذا طلب تفصيلاً فقدّمه.

**الإنفاذ:** hook من نوع `UserPromptSubmit` في `.claude/settings.json` يحقن
التذكير في كل دور (`.claude/hooks/writing-style-reminder.sh`). السبب: المالك
ضبط المهارة عالمياً بوسم «استخدمها دائماً»، ومرّت جلسة كاملة بردود طويلة
مليئة بالعناوين والتكرار — تفضيلٌ يعتمد على تذكّر النموذج ليس تفضيلاً
مُنفَّذاً. القاعدة نفسها تسري على نصوص المنتج الموجَّهة للعميل: لغة الزائر
العادي لا مصطلحات الكود (سابقة «لا اختلاق» → «لا نخمّن الأرقام»، PR #213).

### ٥) اقرأ ما يحتاجه الطلب · read what the task needs

لا تُحمّل وثائق المشروع أو سجلّ الانحدار الكامل افتراضياً. استخدمها حسب النطاق:

- `docs/LESSONS.md` عند تشخيص انحدار معروف أو إضافة درس/حارس.
- `docs/LIVE_PROOF_RUNBOOK.md` عند التحقق بخادم حقيقي أو متصفح أو نشر.
- `tests/test_regression_registry.py` عند تعديل سجل الدروس أو حراس الانحدار.
- `.claude/skills/README.md` لاختيار مهارة مرتبطة مباشرةً بالمهمة؛ لا تحمل
  مهارات احتياطية.

ابدأ من الملفات التي سمّاها الطلب أو التي يثبت البحث أنها تنفّذ المسار المتأثر،
ثم وسّع القراءة فقط عند ظهور تبعية أو غموض.

---

## قوانين غير قابلة للكسر · unbreakable rules (اقرأ أولاً · read first)

السجلّ الكامل (٢٦٠ درساً، كل واحد بأداة إنفاذ) في `docs/LESSONS.md`، مُنفَّذ
ميكانيكياً عبر `tests/test_lessons_enforcement.py`. اقرأ البنود المرتبطة بالمسار
المتأثر عند الحاجة. الخمسة الأحرج ثابتة في كل تغيير:
...
python3 -m pytest tests/ -q                    # full hermetic suite (~7-8 min, ~4,000 tests, no network needed)
...
CI (`.github/workflows/ci.yml`) runs a syntax/undefined-name gate (`ruff check --select E9,F63,F7,F82` — no style rules) and `pip-audit` on the pinned production requirements, then exactly `python -m pytest tests/ -q`. There is no linter config file (R0, audit 2026-09-01). `e2e-live-shape.yml` also builds the deploy image and hits `/health` inside it (`docker-health` job).
...
> **Scope note.** This section documents **pipeline 1 (`/analyze`)** only. A second pipeline — **`/research`** (the 12-mission deep-research path, waves 6–13) — and the client-vs-operator report split and grounded chat (`/ask`) landed after this file was written. (The quick snapshot `silk_snapshot.py` was later **deleted entirely by owner decision** — `docs/DECISIONS.md` A1/#107; do not rebuild it.) A third surface — the multi-tenant factories platform `silk_platform/` under `/platform` — is **market-studies-only since the 2026-08-17 owner pivot**: a factory launches a study and `engine_bridge.py` runs `silk_engine.analyze` automatically (quota-gated, «الباقة فقط» — no per-study wallet charge); all prospecting/outreach code (prospects, drafts, funnels, tenant SMTP, email queue, kill-switch) was **deleted outright by owner decision** — do not rebuild it here (guard: `tests/test_platform_pivot_deletion_guard.py`). Its source of truth is `docs/PLATFORM_ROADMAP.md`. They share the one view (`build_view`) and one verdict (`synthesize`) documented here, but their own mechanics live in `docs/ARCHITECTURE.md` and the `docs/DEEP_RESEARCH_DECISIONS.md` ledger (see its «ما بعد الموجة ١٣» section for PRs #76–#83). `docs/PLATFORM_ANALYSIS.md` is the current full reference.
