...
def mirror_saudi_export(hs_code: str, target_m49: object, target_iso3: str,
    ...
    try:  # المخزن أولاً — صف X (reporter=SAU) مخزّن = صفر نداء خارجي.
        ...
        if row and row.get("value_usd") is not None:
            ...
            # سياسة persist-4 نفسها المطبَّقة على واردات السوق: العتيقة
            # تُخدم فوراً **معلَّمة** stale مع تحديثٍ بالخلفية — كانت هذه
            # الضربة تُخدَم بثقة 0.9 كاملة بلا وسم قِدَم (صيد الفجوات ٣).
            suffix, status = silk_store.staleness_suffix(
                row.get("retrieved_at"), "trade")
            if status == "stale":
                _refresh_mirror_in_background(hs_code, target_m49,
                                              target_iso3, int(year))
            return DataPoint(
                ...
                + (f"، جُلبت أصلاً {day}" if day else "") + suffix,
                row.get("retrieved_at") or _today(), status=status)
    ...
    return _mirror_live(hs_code, target_m49, target_iso3, year, src)


# ── منع تكرار خيوط SWR · SWR in-flight de-duplication ───────────────────────
#
# **تدقيق 2026-08-27 (البند ١٠).** كل إصابة عتيقة كانت تفرّخ خيطاً جديداً بلا
# أي فحص «هل يجري تحديثٌ لهذا المفتاح أصلاً؟». والإصابة العتيقة **جماعية
# بطبعها**: البعثات الاثنتا عشرة المتوازية تسأل عن نفس (رمز، سوق، سنة) في
# نفس اللحظة، فيقع ١٢ خيطاً يجلبون **نفس** القيمة من كومتريد — ١٢ نداءً من
# ميزانية يومية محدودة، و١٢ كتابة متنافسة على المخزن نفسه (يتقاطع مع البند ٢).
# المجموعة أدناه تجعل أول طالبٍ يفوز والبقية no-op صامت — وهو بالضبط عقد SWR:
# القيمة العتيقة **قد خُدمت بالفعل**، فالتحديث الثاني بلا معنى.
# First requester wins; the rest are silent no-ops (SWR already served a value).
_swr_inflight: set = set()
# القفل يُنشأ **عند تحميل الوحدة** لا كسولاً (مراجعة §58): الإنشاء الكسول
# نفسه سباقٌ — خيطان أوّلان متزامنان يبنيان قفلين مختلفين فتصير المجموعة بلا
# تزامن **في نفس لحظة الاندفاع الاثني عشري** التي وُجد الحارس لأجلها.
# `threading` مكتبة قياسية بلا كلفة استيراد تُذكَر، فلا مبرّر للكسل هنا.
# Created at import: lazy creation is itself the race it was meant to prevent.
import threading as _threading

_swr_lock = _threading.Lock()


def _swr_begin(key: tuple) -> bool:
    """احجز مفتاحاً للتحديث — True إن لم يكن قيد التحديث أصلاً."""
    with _swr_lock:
...
def _swr_done(key: tuple) -> None:
    """حرّر المفتاح — always in a finally, so a crash can't wedge it forever."""
    with _swr_lock:
        _swr_inflight.discard(key)


def _refresh_mirror_in_background(hs_code: str, target_m49: object,
                                  target_iso3: str, year: int) -> None:
    """تحديث مرآة عتيقة بالخلفية — SWR للمرآة (نفس مفتاح SILK_SWR=0)."""
    import os
    if os.environ.get("SILK_SWR", "").strip() == "0":
        return
    import threading
    src = "UN Comtrade (تقرير سعودي مباشر — مرآة)"
    key = ("mirror", str(hs_code), str(target_iso3), int(year))
    if not _swr_begin(key):
        return   # تحديثٌ لنفس المفتاح يجري الآن — لا خيط ثانٍ ولا نداء ثانٍ

    def _run() -> None:
        try:  # النداء الحي يكتب للمخزن بنفسه (write-through داخل _mirror_live)
            _mirror_live(hs_code, target_m49, target_iso3, year, src)
        except Exception as e:  # noqa: BLE001 — تحديثُ خلفيةٍ لا يُسقط شيئاً
            log.debug("mirror SWR refresh failed (%s %s %s): %s",
                      hs_code, target_iso3, year, e)
        finally:
            _swr_done(key)

    try:
        threading.Thread(target=_run, name="silk-swr-mirror", daemon=True).start()
    except Exception:  # noqa: BLE001 — تعذّر إنشاء خيط: حرّر المفتاح ولا تعلق
        _swr_done(key)   # وإلا بقي المفتاح محجوزاً للأبد بلا أيّ تحديثٍ لاحق
        raise
...
def _warn_once(key: str, msg: str, *args) -> None:
    """تحذيرٌ واحد لكلّ عائلة عطلٍ في العملية (EXT-20) — كتابةُ المخزن العابرة تفشل
    لكلّ صفٍّ حين يعطب المخزن، فكان السجلُّ يُغرَق بالسطر نفسه."""
    if key in _warned_once:
        log.debug(msg, *args)
        return
    _warned_once.add(key)
    log.warning(msg + " (يُسجَّل مرّةً واحدة لكلّ عملية)", *args)


def _mirror_live(hs_code: str, target_m49: object, target_iso3: str,
                 year: int, src: str) -> DataPoint:
    """المسار الحي للمرآة + الكتابة العابرة — the live mirror fetch."""
    recs = comtrade_trade(hs_code, _SAUDI_M49, year, flow="X",
                          partner=target_m49)
    if recs is None:   # EXT-19: تعذّر الجلب ≠ «لا تقرير سعودي مباشر»
        return DataPoint(
            None, src, 0.0,
            f"HS{hs_code}→{target_iso3} {year}: تعذّر جلب المرآة (حد معدل/شبكة) — أعد المحاولة",
    ...
    try:  # كتابة عابرة — التحليل التالي لنفس الثلاثية يقرأ من المخزن مجاناً.
        ...
        _warn_once("mirror_write", "mirror write-through failed (%s %s %s): %s",
                    hs_code, target_iso3, year, e)
    return DataPoint(
...
def market_competitors(hs_code: str, market_m49: object, year: int) -> list[DataPoint]:
    ...
    EXT-19 (تدقيق 2026-09-01): «تعذّر الجلب» لم يعد يُعرَض «لا منافسين» — الفشلُ يعود
    **نقطةً معلَنة** (`value=None`, `status="fetch_failed"`) لا قائمةً فارغة، فيبقى مقعدُ
    الوكلاء (`silk_agents.market_competitors`) كما هو ويقرأ المستهلكُ التمييزَ من الحقل.
    """
    mi = market_imports(hs_code, market_m49, year)
    comps = mi.get("competitors") or []
    if not comps and mi.get("fetch_failed"):
        return [DataPoint(None, "UN Comtrade", 0.0,
                          f"HS{hs_code} منافسو السوق {market_m49} {year}: تعذّر الجلب "
                          "(حد معدل/شبكة) — أعد المحاولة",
                          _today(), status="fetch_failed")]
    return comps


def market_competitors_status(hs_code: str, market_m49: object,
                              year: int) -> tuple[list, bool]:
    """(المنافسون، هل تعذّر الجلب؟) — EXT-19: `market_competitors` كان يعيد [] لكليهما."""
    mi = market_imports(hs_code, market_m49, year)
    return mi.get("competitors") or [], bool(mi.get("fetch_failed"))
...
def _write_through_market(mi: dict, hs_code: str, market_iso3: str,
    ...
    try:
        ...
        _warn_once("store_write", "fact-store write-through failed (%s %s %s): %s",
...
def _refresh_in_background(hs_code: str, market_m49: object, market_iso3: str,
    ...
    منع التكرار (البند ١٠): مفتاحٌ قيد التحديث لا يُفرَّخ له خيطٌ ثانٍ — راجع
    `_swr_begin`. البعثات المتوازية تسأل عن نفس المفتاح في نفس اللحظة.
    """
    import os
    if os.environ.get("SILK_SWR", "").strip() == "0":
        return
    import threading
    key = ("imports", str(hs_code), str(market_iso3), int(year))
    if not _swr_begin(key):
        return

    def _run() -> None:
        try:
            _swr_refresh(hs_code, market_m49, market_iso3, year, live)
        finally:
            _swr_done(key)

    try:
        threading.Thread(target=_run, name="silk-swr", daemon=True).start()
    except Exception:  # noqa: BLE001 — تعذّر إنشاء خيط: حرّر المفتاح ولا تعلق
        _swr_done(key)   # وإلا بقي المفتاح محجوزاً للأبد بلا أيّ تحديثٍ لاحق
        raise
...
def market_imports_cached(hs_code: str, market_m49: object, market_iso3: str,
    ...
    try:  # 1) المخزن أولاً — the warm store
        ...
        if got["total_usd"] is not None or got["partners"]:
            ...
            suffix, st = silk_store.staleness_suffix(fetched, "trade")
            stale = st == "stale"
            stale_note = suffix + ("؛ يجري تحديثها بالخلفية" if stale else "")
            competitors = []
            if grand:
                for p in valued:
