...
def _fetch_jsonstat(dataset: str, params: dict) -> dict | None:
    ...
    try:
        ...
        # المسار المقوّى (البند ٨) — راجع `silk_data_layer.throttled_request`.
        r = silk_data_layer.throttled_get(url, params=q, timeout=_TIMEOUT)
        r.raise_for_status()
        payload = r.json()
...
def _first_value(payload: dict) -> float | None:
    ...
    # استجابة غير محصورة إلى خلية واحدة لا تمثل إجمالي البلد.
    import math
    if payload.get("size") and math.prod(payload["size"]) != 1:
        return None
    if isinstance(raw, (dict, list)) and len(raw) != 1:
        return None
...
def foreign_born_population_count(iso3: str, iso2: str | None,
    ...
    if not geo:
        return DataPoint(None, "Eurostat migration", 0.0,
                         f"لا رمز جغرافي ليوروستات لـ{iso3}", _today())
    params = {"geo": geo, "c_birth": "FOR", "sex": "T", "age": "TOTAL",
              "unit": "NR", **_time_params(year)}
