...
def _market_size_component(total_usd: object, hs_code: str, m49: str,
                           ...
                           store_meta: dict | None = None) -> DataPoint:
    ...
    قيمة مخدومة من المخزن تحتفظ بتاريخ جلبها الأصلي وتحمل «من المخزن» في
    الملاحظة (كان المنافسون من نفس الضربة يحملون الإسناد بينما يُختم هذا
    الرقم — رقم العنوان — بتاريخ اليوم كجلب حي؛ صيد الفجوات ٣)."""
    ...
    retrieved, status = _today(), ""
    if store_meta and store_meta.get("served_from") == "store":
        retrieved = store_meta.get("retrieved_at") or retrieved
        note += " — " + (store_meta.get("provenance_note") or "من المخزن")
        if store_meta.get("freshness") == "stale":
            status = "stale"
    return DataPoint(float(total_usd), "UN Comtrade", conf, note=note,
                     retrieved_at=retrieved, status=status,
                     data_year=_dyear(year))
...
def _imports_with_fallback(hs_code: str, m49: str, iso3: str,
    ...
    for back in range(_MAX_YEAR_FALLBACK + 1):
        ...
        try:
            ...
            # 1b: استثناء جلبٍ ≠ غياب سجل — بدون العلم كان الانهيار النهائي
            # يُوسَم no_record («لا سجل») بينما المنصةُ هي التي عجزت (صيد ٣).
            mi = {"total_usd": None, "competitors": [], "fetch_failed": True}
            saw_fetch_failure = True
            continue
        if mi.get("total_usd") is not None or mi.get("competitors"):
            return mi, y, (y != year)
    if (saw_fetch_failure and mi.get("total_usd") is None
            and not mi.get("competitors")):
        mi = dict(mi)
        mi["fetch_failed"] = True   # سنةٌ في النافذة تعذّر جلبها ⇒ لا يقين غياب
...
def rank_markets(hs_code: str, countries: list[dict] | None = None,
    ...
    # R1 (تدقيق 2026-09-01، CONC-1): نسخةُ سياقٍ لكل سوق تُؤخَذ هنا في خيط
    # المنادي — العمّال كانوا يرون الافتراضيات صامتاً (لا حجبَ إضافات كلود، لا
    # لوحةَ وكلاء، ولا عدّادَ اقتصاد بيانات فتضيع نداءاتُهم الحيّة من الحساب).
    import silk_context
    with ThreadPoolExecutor(max_workers=workers) as ex:
        rows: list[dict] = list(silk_context.map_with_context(
            ex,
