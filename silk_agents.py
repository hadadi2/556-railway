...
class TradeFlowAgent(BaseAgent):
    ...
    def _execute(self, task: dict) -> AgentReport:
        ...
        today = datetime.date.today().isoformat()   # ختم جلب لكل نقطة (صيد ٣)
        ...
        for flow, label in (("M", "imports"), ("X", "exports")):
            ...
            if row is not None:
                ...
                findings.append(DataPoint(
                    ...
                    + (suffix + " (يحدّثها التحديث الدوري)" if suffix else ""),
            ...
            if not recs:
                findings.append(DataPoint(
                    ...
                    today, status="no_record"))
            elif not vals:
                findings.append(DataPoint(
                    None, "UN Comtrade", 0.0,
                    f"HS{hs} {label} for market {market} {year}: "
                    f"سجلات بلا قيم رقمية ({dropped}) — payload records lacked "
                    "numeric primaryValue, no value to report", today))
            else:
...
class CompetitionAgent(BaseAgent):
    ...
    def _execute(self, task: dict) -> AgentReport:
        ...
        # EXT-19: تعذّرُ الجلب يعود نقطةً معلَنة من طبقة البيانات — تُسلَّم كما هي
        # (فجوةٌ موسومة) بدل «لا بيانات» التي تُقرأ سوقاً فارغة.
        if comps and all(getattr(c, "status", "") == "fetch_failed" for c in comps):
            return AgentReport(self.name, list(comps), True,
                               "تعذّر جلب المنافسين — fetch failed, not absence")
        if not comps:
            return AgentReport(
                self.name,
...
# ── حالةُ تغطية الأدلة · evidence-coverage state (الموجة D، البند V-02) ─────
#
# **ليست حكماً تجارياً.** هذه الحالةُ تصف كم من البعثات نجح، لا كم السوقُ جذّاب.
# الحكمُ التجاريّ من `silk_decision` حصراً (D-01). وُجِدت هنا كي يتوقّف
# المستهلكون عن **مطابقةِ نصٍّ** لاستنتاجها: مطابقةُ سلسلةٍ مختلطة أنتجت
# مفكّكَين مستقلَّين قد يعطيان نبرتين لنفس الحكم.
COVERAGE_NONE = "coverage_none"        # لا بندَ مساهِمٌ واحد
COVERAGE_PARTIAL = "coverage_partial"  # فشل وكيلٌ أو أكثر
COVERAGE_FULL = "coverage_full"        # نجح الجميع
COVERAGE_STATES = (COVERAGE_NONE, COVERAGE_PARTIAL, COVERAGE_FULL)
...
class JuryCommittee:
    ...
    def evaluate(reports: list[AgentReport]) -> dict:
        ...
        # **الموجة D (البند V-02).** السلسلةُ المختلطة أدناه (`"NO-GO
        # (insufficient data) — قرار مؤجّل…"`) كان يفكّكها **مفكّكان مختلفان
        # بمطابقةٍ نصّية فرعية** (`silk_render._verdict_tone` و
        # `silk_narrative.verdict_ar`) — فقد يظهر الحكمُ الواحد بنبرتين حسب
        # أيّ مفكّكٍ وصله أوّلاً، وتغييرُ حرفٍ في النصّ يكسر أحدَهما بصمت.
        #
        # الرمزُ المُعدَّد `coverage_state` هو المصدرُ البنيويّ الآن؛ والسلسلةُ
        # تبقى **حرفياً** كما هي فلا ينكسر أيُّ مستهلكٍ قائم (تكافؤٌ رجعيّ).
        if not contributing:
            state = COVERAGE_NONE
        ...
        return {
            "verdict": verdict,
            # رمزُ حالةِ التغطية — يُقرأ بنيوياً لا بمطابقةِ نصّ (V-02).
            "coverage_state": state,
            # **الموجة B (البند V-01).** هذا الفرعُ يقيس **تغطيةَ الأدلة** لا
            # جاذبيةَ السوق: لا هامشَ ولا منافسةَ ولا حاجزَ تنظيميّ يدخله.
            # كان مُخرَجُه يُطبَع للمصنع بتسمياتٍ تجارية («توصية أولية
            # بالدخول» / «عدم الدخول حالياً»)، فيُقرأ نجاحُ خطّ الأنابيب
            # توصيةً بدخول سوق، وانقطاعُ كلود نصيحةً بهجرها. الأساسُ يُعلَن
            # هنا بنيوياً كي تختار طبقةُ العرض تسميةً **صادقة** — والحكمُ
            # التجاريّ الحقيقيّ يبقى مسؤوليةَ `silk_decision` (الموجة D).
            "basis": "data_coverage",
            "preliminary": True,
