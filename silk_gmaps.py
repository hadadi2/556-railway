...
# R9 (تدقيق 2026-09-01، CONC-9/CONC-5): حدثُ إيقافٍ يوقظ عمّالَ الاستطلاع فوراً عند
# الإغلاق — كانوا ينامون `time.sleep(10)` فلا يرون الإغلاقَ قبل انقضاء نومهم، وSIGKILL
# Railway بعد المهلة هو ما كان ينهيهم.
_STOP = _threading.Event()


def stop_all() -> None:
    """أوقف كلَّ استطلاعٍ جارٍ — يُنادى من `silk_research_runtime.shutdown()`."""
    _STOP.set()
...
def maps_disclaimer(product: str = "", lang: str = "ar") -> str:
    """سطر إخلاء المسؤولية لجدول الخرائط، مُشتقٌّ من المنتج المدروس (لا مثبَّت).

    الموجة ٠: يصل **مُسلَّم العميل** تحت جدول جهات الاتصال، فيتبع لغة التقرير
    — تحفّظٌ لا يُقرأ بلغة القارئ تحفّظٌ لم يُقَل."""
    prod = (product or "").strip()
    if str(lang or "ar").lower() == "en":
        tail = (f"not that it imports “{prod}”" if prod
                else "not that it imports the product under study")
        return ("A Google Maps listing proves only that the business exists "
                "and how to reach it, " + tail
                + " — commercial verification is the next step.")
...
def localized_queries(product: str, market_ref) -> list:
    ...
    queries = [f"{prod} {terms[0]} {country}".strip(),
               f"{prod} {terms[1]}".strip(),
               f"{terms[2]} {country}".strip(),
               terms[3]]
    # §6 من أمر سد الفجوات (المرحلة 0): توسيع البحث بمصطلحات الفئة بالعربية
    # والإنجليزية معاً — خلف صمّام الطبقة (الإطفاء = استعلامات اليوم حرفياً).
    try:
        import silk_gap_recovery
        if silk_gap_recovery.enabled():
            queries += [
                f"موزع أغذية {country}".strip(),
                f"مستورد {prod}".strip(),
                f"تجارة مواد غذائية {country}".strip(),
                f"FMCG distributor {country}".strip(),
                f"food importer {country}".strip(),
            ]
    except Exception:
        pass
    return queries
...
def poll_leads(job_id: str, deadline_monotonic: float):
    ...
    while _time.monotonic() < deadline_monotonic:
        ...
        if status in ("failed", "error", "cancelled"):
            return None
        if _STOP.wait(min(_POLL_INTERVAL_S,
                          max(0.0, deadline_monotonic - _time.monotonic()))):
            return None  # إغلاقٌ — العامل يخرج فوراً (R9، CONC-9)
...
def submit_scrape_async(product: str, market_ref):
    ...
    if cache_get(iso3, queries) is not None:
        ...
        fut = ex.submit(lambda: {"_cached": queries})
        ex.shutdown(wait=False)   # R5 (CONC-9): العاملُ يخرج بعد المهمّة لا عند الإغلاق
        return fut
    ...
    ex.shutdown(wait=False)       # R5 (CONC-9): لا خيطَ عاملٍ يثبّت خروج العملية
    return fut
