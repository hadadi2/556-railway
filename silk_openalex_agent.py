...
def openalex_search(query: str, max_records: int = 5) -> list[DataPoint]:
    ...
    try:
        ...
        # المسار المقوّى (تدقيق 2026-08-27، البند ٨): مباعدة لكل مضيف + إعادة
        # على 429/5xx + قاطع دائرة — بدلاً من نداء عارٍ يحوّل ارتجاجةً عابرة
        # إلى فجوة معلنة. Hardened path instead of a bare request.
        resp = silk_data_layer.throttled_get(_ENDPOINT, params=params,
