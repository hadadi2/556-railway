...
def _academic_closer(lang: str = "ar") -> str:
    ...
    الموجة ٠: كان الفرعُ الأكاديميّ (**خيارٌ صريحٌ لا افتراض** — الافتراضُ
    "decision") يأمر الكاتبَ الإنجليزيَّ بخاتمةٍ عربية، فيحقن العربية
...
def deep_report(mission_reports: dict, analyst_summary: str, verdict: dict,
    ...
    # الصوتُ البشريّ (طلب المالك «Humanized») — **خلف رايةٍ مطفأةٍ افتراضياً**:
    # يُلحَق بالعقد إلحاقاً لا استبدالاً، ويتقدّم على صياغة السجل التقريري في
    # **النبرة وحدَها**. ويُقرَأ عند بناء الموجّه لا عند الاستيراد (نمطُ
    # `epistemic_rule` نفسُه) فلا تُخبَز الرايةُ في لحظة التحميل.
    #
    # **ولا يُلحَق في السجل الأكاديميّ** (تصحيحُ المراجعة الذاتية): العقدُ
    # الأكاديميُّ يمنع خطابَ القارئ صراحةً ويأمر بالصيغ الثلاث التي تمنعها
    # هذه القاعدة — فإلحاقُها هناك يعطي الكاتبَ قاعدتين إلزاميتين متناقضتين.
    # والأكاديميُّ **خيارٌ صريح** (`report_style="academic"`) لا افتراض، فمن
    # طلبه طلب سجلَّه.
    #
    # **وتصحيحٌ لادّعاءٍ كتبتُه أوّلاً:** قلتُ إنّ المراجعَ «لا يقاوم هذا
    # الصوت». المقيسُ أنّ `_alarmist_issues` يقترح `MEASURED_TONE_HINT`، وهو
    # المُخرَجُ الإلزاميُّ «ينبغي التعامل مع هذه الأرقام كمؤشر سياقي» — فلو
    # بقيت تلك العبارةُ ممنوعةً لدفع المراجعُ الكاتبَ إليها ومنعتها القاعدة.
    # العلاجُ أنّ العبارةَ **خرجت من قائمة المنع** (لا أنّ المراجعَ محيَّد).
    from silk_style_contract import human_voice_rule, human_voice_rule_en
    _voice = "" if academic else (
        human_voice_rule_en() if lang == "en" else human_voice_rule())
    if _voice:
        contract = contract + "\n\n" + _voice
    ...
    parts.append(
        ...
        "إلى مدخلات النظام.\n"
        "- **3.1أ فرق السعر المرصود وسعر المصدّر**: إذا كان جدول المنافسة "
        "يحوي أسعار رف فعلية، فلا تقل إن «سعر التجزئة غير متاح». قل بدقة: "
        "أسعار المنافسين متاحة، بينما سعر المنتج السعودي/تكلفة إنتاجه أو "
        "سعره بعد الوصول غير متاح. لا تكرر هذا النقص في الخلاصة والتسعير "
        "وأرقام القرار؛ اذكره في موضعه ثم استخدم إحالة قصيرة.\n"
        ...
        "- **3.2أ سقف الخسارة**: لا تقل إن إدخال تكلفة الإنتاج وحدها يجعل "
        "أقصى الخسارة محسوبة. أقصى الخسارة تحتاج كلفة الإنتاج والشحن "
        "والرسوم والتسويق والتكاليف الثابتة وحجم الشحنة وسعر البيع أو "
        "قيمة المخزون المتبقي. إن غاب أي منها، اطبع «غير محسوب» مع قائمة "
        "النواقص كاملة، ولا تقل «تكتمل تلقائياً».\n"
    ...
    parts.append(
        ...
        "مصدره، ثم المسار العملي، ثم المتطلب السابق للتعاقد أو الشحن — تصمد وحدها لو لم يُقرأ "
        ...
        for _e in _eco["decision_numbers"]:
            if _e.get("tier") == "estimated" and not _e.get("too_wide"):
                ...
                _dn_lines.append(
                    ...
                    f"أثره على القرار: {_e['impact']}؛ الإجراء المطلوب: "
                    f"{_e['closure']}")
        parts.append("\n".join(_dn_lines))
    ...
    parts.append(
        ...
        "حجم الشحنة التجريبية (سعةٌ لا حجمُ طلب)، استرداد كلفة الشحنة "
        "التجريبية ونقطة التعادل التشغيلي **مقياسين منفصلين**، وتكلفة تمويل "
        "مخزون التجربة (مقياسٌ ثالث: كلفة المال المحبوس حتى التحصيل)، الزمن من "
        "القرار إلى أول فاتورة بالأسابيع، وأقصى خسارة إن فشل الدخول — "
        "المتعذر حسابه يُطبع بمعادلته وناقصه («نقطة التعادل التشغيلي = "
        "التكاليف الثابتة ÷ هامش المساهمة. المتاح: الهامش. الناقص: "
        "التكاليف الثابتة.») لا يُسقَط "
        ...
        "العملي، ثم المتطلب السابق للتعاقد أو الشحن — **تحت 150 كلمة** قابلة للمسح السريع "
...
def review_report(draft: str, mission_reports: dict,
    ...
    user = (
        ...
        # الصنف ١ (موجة عيوب التقرير): المراجعُ يُدقّق **لغةَ القارئ** بنفس
        # العقد المحقون في الكاتب — مصدرٌ واحد، فلا تُدقَّق قاعدةٌ غير التي
        # كُتب بها. القوائمُ الحتمية تُفحَص بعده في
        # `silk_quality_gate._check_reader_language_leak` (لا نداء مدفوع).
        + "دقّق لغةَ القارئ بهذا العقد نفسِه:\n"
        + __import__("silk_style_contract").reader_language_rule(lang) + "\n"
        + "كلُّ خرقٍ لهذا العقد مشكلةٌ صريحة في issues باقتراح الصياغة "
        "البديلة.\n"
...
def write_reviewed_report(mission_reports: dict, analyst_summary: str,
    ...
    def _decision_language(text: str) -> str:
        ...
        text = re.sub(
            r"حجم الشحنة التجريبية الموصى به:\s*26730\s*كيلوغرام(?:اً|ا|)؟[^.]*\.",
            "حجم الشحنة التجريبية: غير محسوب. لا توجد بيانات طلب مؤكدة أو عرض شحن رسمي يبرر كمية محددة؛ لا يُنصح بتحميل حاوية كاملة قبل اختبار البيع.",
            text)
        return text.replace(
